...
if (( UI_COLOR )); then
    ...
    C_TXT=$'\033[38;5;255m'     # 正文
    C_KEY=$'\033[38;5;117m'     # 字段名
    C_VAL=$'\033[38;5;229m'     # 字段值
    C_MUT=$'\033[38;5;245m'     # 次要信息
    C_DIM2=$'\033[38;5;240m'    # 装饰线
    BOLD=$'\033[1m'
    NC=$'\033[0m'
else
...
add_vmess_plain() {
    ...
    ask_uuid_value "UUID"; uuid="$REPLY_UUID"; host="$(server_address)"; tag="${tag_prefix}-${port}"
    case "$transport" in
        tcp)
...
add_vmess_tls() {
    ...
    ask_name "${default_name}-${port}"; local name="$REPLY_NAME"
    local uuid host tag inbound vmjson link detail
    ask_uuid_value "UUID"; uuid="$REPLY_UUID"; host="$(server_address)"; tag="${tag_prefix}-${port}"; vm_host=""
    case "$transport" in
        ws)
...
mieru_links_for_node() {
    local name="$1" user="$2" pass="$3" proto="$4" port="$5" mtu="$6" host eu ep item pr po tp tp_query=""
    : "$name"
    mieru_load_opts
    host="$(server_host)"
...
mieru_mihomo_yaml_for_node() {
    local name="$1" user="$2" pass="$3" proto="$4" port="$5" raw_host item pr po mname tp tp_line=""
    mieru_load_opts
    raw_host="$(server_address)"; tp="$(mieru_export_traffic_pattern)"
...
mieru_migrate_legacy() {
    ...
    if ! jq -e --arg t "$tag" '.nodes[$t]' "$STATE" >/dev/null 2>&1; then
        ...
        extra="$(jq -cn --arg user "$MIERU_USER" --arg pass "$MIERU_PASS" --arg proto "$proto" \
            --argjson port "$MIERU_PORT" --argjson mtu "$mtu" '{user:$user, pass:$pass, proto:$proto, port:$port, mtu:$mtu}')"
        init_state
        local tmp; tmp="$(mktemp)"
...
mieru_refresh_saved_clients() {
    [[ -f "$STATE" ]] || return 0
    local effective_mtu tag name user pass proto port link detail state_tmp next_tmp staging_dir old_dir
    mieru_load_opts
    ...
    while IFS=$'\t' read -r tag name user pass proto port; do
        [[ -n "$tag" ]] || continue
        link="$(mieru_links_for_node "$name" "$user" "$pass" "$proto" "$port" "$effective_mtu")"
        detail="  地址: $(server_host)\n  端口: ${port}$([[ "$proto" == "BOTH" ]] && printf ' / %s' "$((port+1))")\n  用户名: ${user}\n  密码: ${pass}\n  协议: ${proto}\n  MTU : ${effective_mtu}\n  Profile: ${MIERU_PROFILE}"
        ...
        mieru_write_client_json_for_node "$name" "$user" "$pass" "$proto" "$port" "$effective_mtu" "$staging_dir" || {
            rm -f "$state_tmp"; rm -rf "$staging_dir"; return 1
        }
...
mieru_save_node() {
    local tag="$1" name="$2" user="$3" pass="$4" proto="$5" port="$6" mtu="$7" link detail extra tmp
...
add_mieru_node() {
    ...
    local pass="$REPLY_MIERU_PASS" mtu="$REPLY_MIERU_MTU" name tag
    ask_name "Mieru-${proto}-${port}"; name="$REPLY_NAME"
    tag="mieru-${port}"

    mieru_save_node "$tag" "$name" "$user" "$pass" "$proto" "$port" "$mtu"
    if ! mieru_apply_from_state; then
        delete_state_node_only "$tag"
        mieru_apply_from_state >/dev/null 2>&1 || true
        return 1
    fi
    ...
    echo -e "${GREEN}$(mieru_mihomo_yaml_for_node "$name" "$user" "$pass" "$proto" "$port")${NC}"
    if command -v qrencode >/dev/null 2>&1; then
        local l
...
reconfigure_mieru_node() {
    local tag="$1" extra old_proto old_port old_user old_pass old_mtu name
    [[ -n "$tag" ]] || return 1
...
mieru_show_link() {
    ...
    mieru_load_opts
    local row tag name user pass proto port mtu link effective_mtu
    effective_mtu="$(mieru_effective_mtu)"
    while IFS=$'\t' read -r tag name user pass proto port mtu; do
        mtu="$effective_mtu"
        [[ -z "$tag" ]] && continue
        echo
        ...
        | [.key, .value.name, (.value.extra_tag | fromjson | .user), (.value.extra_tag | fromjson | .pass),
           ...
           (.value.extra_tag | fromjson | .mtu // 1400)] | @tsv' "$STATE" 2>/dev/null)
...
main_loop() {
    while true; do
        show_menu
        read -rp "$(ui_prompt "请输入选项" "0-22")" choice
        echo
        case "$choice" in
            1)  vless_menu ;;
            2)  finalmask_menu ;;
