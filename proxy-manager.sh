...
add_vmess_plain() {
    ...
    uuid="$(gen_uuid)"; host="$(server_host)"; tag="${tag_prefix}-${port}"
    inbound="$(jq -n --arg tag "$tag" --argjson port "$port" --arg uuid "$uuid" --argjson stream "$stream" '{
        tag:$tag, listen:"0.0.0.0", port:$port, protocol:"vmess",
...
add_vmess_tls() {
    ...
    ask_name "${default_name}-${port}"; local name="$REPLY_NAME"
    local uuid host tag inbound vmjson link detail
    uuid="$(gen_uuid)"; host="$(server_host)"; tag="${tag_prefix}-${port}"; vm_host=""
    case "$transport" in
        ws)
            path="/$(openssl rand -hex 4)"; vm_path="$path"; vm_host="$sni"
            ...
            ;;
        grpc)
            svc="grpc$(openssl rand -hex 3)"; vm_path="$svc"
...
add_trojan_tls() {
    ...
    pw="$(gen_pass 16)"; host="$(server_host)"; tag="${tag_prefix}-${port}"
...
ensure_time_sync() {
    ...
    if command -v timedatectl >/dev/null 2>&1; then
        ...
        sleep 2
        time_synced && { ok "已通过 systemd-timesyncd 启用时间同步。"; return 0; }
    fi
    ...
    command -v chronyc >/dev/null 2>&1 && chronyc -a makestep >/dev/null 2>&1 || true
    sleep 2
...
add_shadowsocks() {
    ...
    local pw tag; pw="$(gen_pass 16)"; tag="ss-${port}"
    local inbound
...
add_ss2022() {
    ...
    local pw tag
    pw="$(gen_b64 "$(ss2022_key_bytes "$method")")"; tag="ss2022-${port}"
    local inbound
    inbound="$(jq -n --arg tag "$tag" --argjson port "$port" --arg m "$method" --arg pw "$pw" '{
        tag:$tag, listen:"0.0.0.0", port:$port, protocol:"shadowsocks",
        settings:{ method:$m, password:$pw, network:"tcp,udp" } }')"
...
add_http_proxy() {
    ...
    local user pass; user="http$(openssl rand -hex 2)"; pass="$(gen_pass 8)"
    local tag="http-${port}"
    local inbound
    inbound="$(jq -n --arg tag "$tag" --argjson port "$port" --arg u "$user" --arg p "$pass" '{
...
install_snell()    { install_snell_version "4" "v4.1.1"; }
install_snell_v5() { install_snell_version "5" "v5.0.1"; }

snell_show_link() {
    [[ -f "$SNELL_STATE" ]] || {
        local port psk
        port="$(grep -oP 'listen = .*:\K[0-9]+' "$SNELL_CONF" 2>/dev/null)"
        psk="$(grep -oP '^psk *= *\K.+' "$SNELL_CONF" 2>/dev/null)"
        [[ -n "$port$psk" ]] || { warn "未找到 Snell 配置。"; return 0; }
        SNELL_VERSION="4"; SNELL_PORT="$port"; SNELL_PSK="$psk"
    }
...
show_menu() {
    ...
    echo -e " ${GREEN}33.${NC} Snell v4"
    echo -e " ${GREEN}34.${NC} Snell v5"
    echo -e " ${GREEN}35.${NC} Mieru"
    echo -e " ${GREEN}36.${NC} WireGuard"
    echo -e "${YELLOW}${BOLD}─── 节点与系统管理 ───${NC}"
    echo -e " ${GREEN}37.${NC} 查看全部节点"
    echo -e " ${GREEN}38.${NC} 编辑节点配置"
    echo -e " ${GREEN}39.${NC} 删除指定节点"
    echo -e " ${GREEN}40.${NC} 内核与服务管理"
    echo -e " ${RED}41.${NC} 卸载"
...
main_loop() {
    while true; do
        show_menu
        read -rp "$(echo -e "${CYAN}请输入选项 [0-41]: ${NC}")" choice
        echo
        case "$choice" in
