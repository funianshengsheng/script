...
add_ss2022() {
    ...
    echo -e "${DIM}SS2022 对客户端要求较新, 高阻断环境可启用 ShadowTLS v3 插件模式。${NC}"
...
EOF
    ...
    local detail="  地址: ${host}\n  端口: ${port}\n  PSK : ${psk}\n  版本: ${major}\n  服务: ${svc}\n  配置: ${conf}"
    save_node "snell-managed" "$tag" "Snell v${major}" "$name" "$surge" "$detail"
...
install_snell() {
    ...
    echo -e "  ${GREEN}3.${NC} Snell v4 + ShadowTLS"
    echo -e "  ${GREEN}4.${NC} Snell v5 + ShadowTLS"
    echo -e "  ${GREEN}0.${NC} 返回"
    local c; read -rp "$(echo -e "${CYAN}请选择 (默认: 2): ${NC}")" c
    case "$c" in
        1) install_snell_v4 ;;
        3) install_snell_shadowtls_v4 ;;
        4) install_snell_shadowtls_v5 ;;
        0) return 0 ;;
        2|"") install_snell_v5 ;;
        *) err "无效选项。" ;;
    esac
...
add_mieru_node() {
    ...
    echo -e "${BOLD}手动参数:${NC}"; echo -e "${detail}"
    hr
}
...
ptm_build_port_user_message() {
    ...
    local remark status usage quota remaining expire reset_day usage_percent
    remark=$(jq -r ".ports.\"$port\".remark // \"\"" "$PTM_CONFIG_FILE" 2>/dev/null)
    status=$(ptm_format_running_status "$(ptm_get_port_running_status "$port")")
    usage=$(ptm_format_bytes "$(ptm_get_port_monthly_usage "$port" 2>/dev/null || echo 0)")
    quota=$(jq -r ".ports.\"$port\".quota.monthly_limit // \"unlimited\"" "$PTM_CONFIG_FILE" 2>/dev/null)
...
ptm_build_ports_summary_text() {
    ...
    for port in $ports; do
        local remark status usage quota remaining expire
        remark=$(jq -r ".ports.\"$port\".remark // \"\"" "$PTM_CONFIG_FILE" 2>/dev/null)
        status=$(ptm_format_running_status "$(ptm_get_port_running_status "$port")")
        usage=$(ptm_format_bytes "$(ptm_get_port_monthly_usage "$port" 2>/dev/null || echo 0)")
        quota=$(jq -r ".ports.\"$port\".quota.monthly_limit // \"unlimited\"" "$PTM_CONFIG_FILE" 2>/dev/null)
        ...
        msg+=$'\n'"状态：${status} | 已用：${usage} | 配额：${quota} | 剩余：${remaining}"
        msg+=$'\n'"到期：${expire}"$'\n'
    done
    echo "$msg"
}
...
ptm_send_daily_telegram_reports() {
    local enabled
    enabled=$(jq -r '.notify.telegram.daily_report_enabled // false' "$PTM_CONFIG_FILE" 2>/dev/null)
    ...
    for chat_id in $(jq -r '.ports | to_entries[] | .value.telegram_chat_id // empty' "$PTM_CONFIG_FILE" 2>/dev/null | sort -u); do
        [ -n "$chat_id" ] || continue
        local ports msg
        ports=$(ptm_get_telegram_bound_ports "$chat_id")
        [ -n "$ports" ] || continue
        msg=$(ptm_build_user_ports_summary_text "每日使用情况" "$ports")
        ptm_send_telegram_long "$chat_id" "$msg" || true
    done
    ...
    if [ -n "$admin_chat" ] && [ "$admin_chat" != "null" ] && [ -n "$all_ports" ]; then
        admin_msg=$(ptm_build_ports_summary_text "车主每日概览" "$all_ports")
        ptm_send_telegram_long "$admin_chat" "$admin_msg" || true
    fi
}
...
$(ptm_telegram_help_text "$role")"
        ...
        /report)
            if [ "$role" = "admin" ]; then
                ptm_send_daily_telegram_reports
                ptm_send_telegram_to_chat "$chat_id" "已触发一次每日用量报告。"
            else
...
ptm_install_telegram_bot_service() {
    if ! command -v systemctl >/dev/null 2>&1; then
        echo -e "${gl_huang}⚠ 当前系统未检测到 systemctl，无法安装 Telegram Bot 常驻服务${gl_bai}"
        return 1
    fi
...
ptm_menu_add_port() {
    ...
    port_json=$(jq -n \
        ...
        '{remark: $remark, billing_mode: $mode, email: "", telegram_chat_id: "", created_at: $created,
          expiration_date: "",
...
ptm_menu_merge_ports() {
    ...
    # 继承第一个端口的配置作为模板
    local quota_config bandwidth_config remark expiration_date email telegram_chat_id
...
ptm_menu_configure_telegram() {
    ...
    read -e -p "是否开启 Telegram 每日用量报告？[y/n] (留空不改): " daily_choice
    ...
    case "$daily_choice" in
        y|Y) ptm_update_config ".notify.telegram.daily_report_enabled = true" ;;
        n|N) ptm_update_config ".notify.telegram.daily_report_enabled = false" ;;
    esac
...
ptm_menu_bind_telegram_chat() {
    ...
    if [ "$user_chat" = "d" ]; then
        ptm_update_config ".ports.\"$port\".telegram_chat_id = \"\""
...
ptm_menu_configure_notify() {
    ...
    while true; do
        clear
        echo -e "\n${BLUE}${BOLD}=== 通知管理 ===${NC}"
        local email_enabled tg_enabled tg_admin tg_daily tg_service
        ...
        tg_daily=$(jq -r '.notify.telegram.daily_report_enabled // false' "$PTM_CONFIG_FILE" 2>/dev/null)
        if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet "$PTM_TG_SERVICE" 2>/dev/null; then
            tg_service="运行中"
        else
            tg_service="未运行"
        fi
        echo "邮箱通知：$email_enabled"
        echo "Telegram：$tg_enabled | 管理员: ${tg_admin:-未设置} | 每日报告: $tg_daily | 服务: $tg_service"
        echo ""
        echo -e "  ${GREEN}1.${NC} 邮件通知设置"
        echo -e "  ${GREEN}2.${NC} Telegram Bot 设置"
        echo -e "  ${GREEN}3.${NC} 绑定端口 Telegram 用户"
        echo -e "  ${GREEN}4.${NC} 发送 Telegram 测试通知"
        echo -e "  ${GREEN}5.${NC} 启动/重启 Telegram Bot 服务"
        echo -e "  ${GREEN}6.${NC} 停止 Telegram Bot 服务"
        echo -e "  ${GREEN}0.${NC} 返回"
        local choice
        read -e -p "$(echo -e "${CYAN}请选择 [0-6]: ${NC}")" choice
        case "$choice" in
            1) ptm_menu_configure_email ;;
            2) ptm_menu_configure_telegram ;;
            3) ptm_menu_bind_telegram_chat ;;
            4) ptm_menu_test_telegram ;;
            5) ptm_install_telegram_bot_service; break_end ;;
            6) ptm_remove_telegram_bot_service; echo -e "${gl_lv}✓ Telegram Bot 服务已停止${gl_bai}"; break_end ;;
            0) return ;;
            *) err "无效选项。"; sleep 1 ;;
        esac
    done
