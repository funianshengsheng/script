...
ptm_report_line() {
    local label="$1" value="${2:-}" fw_space="　"
    local label_width=6 label_len=6 pad_len=0 pad=""
    case "$label" in
        端口|备注|状态|节点|序号) label_len=2 ;;
        计费模式|已用流量|流量配额|剩余流量|使用比例|到期时间) label_len=4 ;;
        流量重置日期) label_len=6 ;;
        *) label_len=5 ;;
    esac
    pad_len=$((label_width - label_len))
