...
mieru_show_link() {
    ...
    for item in "${plist[@]}"; do
        ...
        printf '  - name: %s\n    type: mieru\n    server: %s\n    port: %s\n    transport: %s\n    username: %s\n    password: %s\n    multiplexing: MULTIPLEXING_LOW\n' \
            "$(yaml_quote "$mname")" "$(yaml_quote "$raw_host")" "$po" "$pr" "$(yaml_quote "$MIERU_USER")" "$(yaml_quote "$MIERU_PASS")"
    done
    echo -e "${NC}"
    if command -v qrencode >/dev/null 2>&1; then
        for item in "${plist[@]}"; do
...
ptm_report_line() {
    # 等宽块保留全角空格，让中文标签在桌面端和移动端使用相同列宽。
    local label="$1" value="${2:-}" pad="" pad_char="　"
    case "$label" in
        端口|备注|状态|节点|序号|服务) pad="${pad_char}${pad_char}${pad_char}${pad_char}" ;;
        管理员|到期日) pad="${pad_char}${pad_char}${pad_char}" ;;
        计费模式|已用流量|流量配额|剩余流量|使用比例|到期时间|邮箱通知|概览推送|诊断结果) pad="${pad_char}${pad_char}" ;;
        流量重置日期) pad="" ;;
        *) pm_aligned_line "$label" "$value" 12; return ;;
    esac
    printf '%s%s:%s\n' "$label" "$pad" "$value"
}
