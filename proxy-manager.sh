...
show_core_service_info() {
    ...
    if xray_installed; then
        echo -e "  Xray     : $(service_state_text "$XRAY_SERVICE")  版本: ${GREEN}$(xray_version)${NC}  服务: ${XRAY_SERVICE}  配置: ${XRAY_CONFIG}"
    else
        echo -e "  Xray     : ${YELLOW}未安装${NC}"
    fi
    if sb_installed; then
        echo -e "  sing-box : $(service_state_text "$SB_SERVICE")  版本: ${GREEN}$(sb_version)${NC}  服务: ${SB_SERVICE}  配置: ${SB_CONFIG}"
    else
        echo -e "  sing-box : ${YELLOW}未安装${NC}"
    fi
    if snell_installed; then
        local sport; snell_load_state >/dev/null 2>&1; sport="${SNELL_PORT:-未知}"
        echo -e "  Snell    : $(service_state_text "$SNELL_SERVICE")  版本: ${GREEN}$(snell_version)${NC}  服务: ${SNELL_SERVICE}  端口: ${sport}"
    fi
    if mieru_installed; then
        local mport mproto
        mport="$(source "$MIERU_STATE" 2>/dev/null; echo "${MIERU_PORT:-未知}")"
        mproto="$(source "$MIERU_STATE" 2>/dev/null; echo "${MIERU_PROTO:-未知}")"
        echo -e "  Mieru    : $(service_state_text "$MITA_SERVICE")  版本: ${GREEN}$(mieru_version)${NC}  服务: ${MITA_SERVICE}  协议/端口: ${mproto}/${mport}"
    fi
    if wg_installed; then
        local wport; wport="$(source "$WG_STATE" 2>/dev/null; echo "${WG_PORT:-未知}")"
        echo -e "  WireGuard: $(service_state_text "wg-quick@${WG_IFACE}")  版本: ${GREEN}$(wg_version)${NC}  服务: wg-quick@${WG_IFACE}  端口: ${wport}"
    fi
    if [[ -f "$STATE" ]]; then
...
prompt_auto_update_setting() {
    ...
    if confirm "是否开启 pm 启动时自动同步最新版脚本? 开启后每次输入 pm 都会先拉取更新" "y"; then
        set_auto_update_setting true
        ok "已开启 pm 自动同步。"
    else
        set_auto_update_setting false
        warn "已关闭 pm 自动同步, 可在菜单中重新开启。"
    fi
}
...
auto_update_on_pm_start() {
    ...
    info "pm 自动同步: 正在检查脚本更新..."
    if ! download_latest_script "$tmp"; then
        warn "pm 自动同步失败, 继续使用本地脚本。"
        return 0
    fi
    apply_script_update "$target" "$tmp"
    case "$?" in
        0)
            ok "pm 自动同步完成, 正在加载最新版..."
...
# ---- cron 自动化：生成独立脚本(不反向依赖 proxy-manager.sh)，模式与 snell_install_daily_restart_cron 一致 ----
...
ptm_menu() {
    ...
    while true; do
        ...
        hr
        local choice
        read -e -p "$(echo -e "${CYAN}请选择 [0-5,99]: ${NC}")" choice
        case $choice in
...
show_menu() {
    ...
    echo -e " ${GREEN}18.${NC} 内核与服务管理"
    echo -e " ${GREEN}19.${NC} 更新脚本"
    echo -e " ${GREEN}20.${NC} pm 自动同步开关 ${DIM}(当前: $( [[ "$(auto_update_setting)" == "true" ]] && echo 开启 || echo 关闭 ))${NC}"
    echo -e " ${RED}21.${NC} 卸载"
    echo -e " ${GREEN}22.${NC} 端口流量计费与到期管理"
...
main_loop() {
    while true; do
        show_menu
        read -rp "$(echo -e "${CYAN}请输入选项 [0-22]: ${NC}")" choice
        echo
        case "$choice" in
            1)  vless_menu ;;
            2)  finalmask_menu ;;
