...
# --- 输入提示 ---------------------------------------------------------------
# 用法: read -rp "$(ui_prompt "请选择" "0-21")" choice
ui_prompt() {
    local text="$1" hint="${2:-}"
    if [[ -n "$hint" ]]; then
...
# 流量伪装 (trafficPattern) 自 mita v3.28.0 起支持, 旧版本会因未知字段 apply 失败。
mieru_supports_traffic_pattern() {
    local v; v="$(mieru_version)"
    [[ "$v" =~ ^v?[0-9] ]] || return 1
    v="${v#v}"
...
mieru_load_opts() {
    MIERU_TRAFFIC="conservative"; MIERU_TRAFFIC_SEED=""; MIERU_MTU="1400"
    if [[ -f "$MIERU_STATE" ]]; then
        # shellcheck disable=SC1090
        source "$MIERU_STATE" 2>/dev/null || true
    fi
    MIERU_TRAFFIC="${MIERU_TRAFFIC:-conservative}"
    MIERU_MTU="${MIERU_MTU:-1400}"
}

mieru_save_opts() {
    mkdir -p "$STATE_DIR"
    local legacy=""
    # 旧版单节点状态若尚未迁移, 保留其字段, 迁移函数会负责清理。
...
mieru_traffic_pattern_json() {
    mieru_load_opts
    local level="${MIERU_TRAFFIC:-conservative}" seed
    ...
    if [[ "$level" == "aggressive" ]]; then
        jq -cn --argjson seed "$seed" '{
            seed:$seed, unlockAll:true,
...
ask_mieru_traffic_pattern() {
    ...
    if [[ "$MIERU_TRAFFIC" != "off" ]] && mieru_installed && ! mieru_supports_traffic_pattern; then
        warn "当前 mita 版本不支持流量伪装 (需 >= 3.28.0), 本次将跳过该字段; 可在菜单重新安装 Mieru 升级内核。"
    fi
    mieru_save_opts
}
...
mieru_mihomo_yaml_for_node() {
    local name="$1" user="$2" pass="$3" proto="$4" port="$5" raw_host item pr po mname
    raw_host="$(server_address)"
    case "$proto" in
        BOTH) set -- "TCP:${port}" "UDP:$((port+1))" ;;
        *)    set -- "${proto}:${port}" ;;
    esac
    for item in "$@"; do
        ...
        printf '  - name: %s\n    type: mieru\n    server: %s\n    port: %s\n    transport: %s\n    udp: true\n    username: %s\n    password: %s\n    multiplexing: MULTIPLEXING_LOW\n' \
            "$(yaml_quote "$mname")" "$(yaml_quote "$raw_host")" "$po" "$pr" "$(yaml_quote "$user")" "$(yaml_quote "$pass")"
    done
}
...
ask_mieru_node_params() {
    ...
    ask_int_default "Mieru MTU" "$cur_mtu" 1280 9000; mtu="$REPLY_VALUE"
...
install_mieru() {
    ...
    info "获取 Mieru 最新版本..."
    local tag ver; tag="$(gh_latest_tag "$MITA_REPO")"
...
mieru_show_link() {
    ...
    mieru_load_opts
    local row tag name user pass proto port mtu link
...
edit_single_node() {
    ...
    EDIT_LABELS=()
    EDIT_FUNCS=()
    if [[ "$core" == "snell-managed" || "$core" == "snell-shadowtls" || "$core" == "wireguard-managed" ]]; then
...
show_menu() {
    ...
    echo
    ui_item 21 "卸载" "移除内核 / 节点 / 计费模块" "$C_ERRC"
    ui_item_back "退出"
    echo
    ui_rule "$G_H"
}
...
main_loop() {
    while true; do
        show_menu
        read -rp "$(ui_prompt "请输入选项" "0-21")" choice
        echo
        case "$choice" in
            1)  vless_menu ;;
            2)  finalmask_menu ;;
