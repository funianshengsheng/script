...
# 一、终端 UI 引擎 (色彩 / 图形 / 排版)
#
#   统一视觉语言: 渐变标题条 · 左导轨面板 · 编号胶囊 · 状态徽章 · 进度条。
#   设计取舍:
#     1) 面板一律只画左边框, 不画右边框 —— 中文/emoji 在不同终端下宽度不一致,
#        没有右边框就永远不会出现"框线错位", 可读性与鲁棒性同时成立。
#     2) 颜色使用真实 ESC 字符 (而非 '\033' 字面量), 因此 echo -e 与 printf
#        两种调用方式都能正确着色, 且宽度计算函数可以剥离颜色。
#     3) 非 TTY / NO_COLOR / TERM=dumb 时整体退化为纯文本; PM_ASCII=1 可强制
#        使用 ASCII 边框字符 (老旧终端、日志留存场景)。
# ---------------------------------------------------------------------------
...
if (( UI_COLOR )); then
    C_ACC=$'\033[38;5;51m'      # 主强调 霓虹青
    C_ACC2=$'\033[38;5;45m'     # 次强调 冰蓝
    C_VIO=$'\033[38;5;141m'     # 紫罗兰
    C_PINK=$'\033[38;5;211m'    # 品红
    C_OKC=$'\033[38;5;84m'      # 成功 荧光绿
    C_WARNC=$'\033[38;5;221m'   # 警告 琥珀
    C_ERRC=$'\033[38;5;203m'    # 错误 珊瑚红
    C_TXT=$'\033[38;5;255m'     # 正文 亮白
    C_KEY=$'\033[38;5;117m'     # 字段名 天蓝
    C_VAL=$'\033[38;5;229m'     # 字段值 暖黄
    C_MUT=$'\033[38;5;153m'     # 次要信息 雾蓝 (不用灰色, 黑底清晰可读)
    C_DIM2=$'\033[38;5;110m'    # 装饰线 钢蓝 (不用灰色, 黑底清晰可见)
    BOLD=$'\033[1m'
    NC=$'\033[0m'
else
    C_ACC=''; C_ACC2=''; C_VIO=''; C_PINK=''; C_OKC=''; C_WARNC=''; C_ERRC=''
    C_TXT=''; C_KEY=''; C_VAL=''; C_MUT=''; C_DIM2=''; BOLD=''; NC=''
fi

# 旧变量名保持可用, 全脚本上千处 echo -e 无需改写。
RED="$C_ERRC"; GREEN="$C_OKC"; YELLOW="$C_WARNC"
BLUE="$C_ACC2"; CYAN="$C_ACC"; MAGENTA="$C_VIO"; DIM="$C_MUT"

# 兼容从 net-tcp-tune 移植的 PTM 模块颜色变量名。
gl_hong="$RED"; gl_lv="$GREEN"; gl_huang="$YELLOW"; gl_bai="$NC"
gl_kjlan="$CYAN"; gl_zi="$MAGENTA"; gl_hui="$DIM"

# 渐变色阶 (256 色索引): 深蓝 → 青 → 薄荷白 / 靛蓝 → 紫 → 品红
UI_GRAD=(27 33 39 45 51 87 123 159)
UI_GRAD2=(63 99 135 141 177 213)

if [[ "${PM_ASCII:-0}" == "1" ]]; then
    G_H='-';  G_HH='='; G_V='|';  G_TL='+';  G_BL='+'
    G_BAR='#'; G_RAIL='|'; G_SEP='-'; G_ARROW='>'; G_TIP='>'
    G_OK='OK'; G_ERR='XX'; G_WARN='!!'; G_INFO='::'
    G_DOT='*'; G_RING='o'; G_FULL='#'; G_EMPTY='.'; G_ENTER='<-'
else
    G_H='─';  G_HH='━'; G_V='│';  G_TL='╭';  G_BL='╰'
    G_BAR='▊'; G_RAIL='▎'; G_SEP='·'; G_ARROW='❯'; G_TIP='▸'
    G_OK='✔'; G_ERR='✘'; G_WARN='▲'; G_INFO='◆'
    G_DOT='●'; G_RING='○'; G_FULL='▰'; G_EMPTY='▱'; G_ENTER='↵'
fi
...
ui_refresh_cols() {
    local c=""
    command -v tput >/dev/null 2>&1 && c="$(tput cols 2>/dev/null)"
    [[ "$c" =~ ^[0-9]+$ ]] || c="${COLUMNS:-80}"
    [[ "$c" =~ ^[0-9]+$ ]] || c=80
    (( c < 56 ))  && c=56
    (( c > 104 )) && c=104
...
# --- 显示宽度计算 -----------------------------------------------------------
# 结果写入全局 UI_WIDTH (用全局而非 stdout 是为了避免每行一次子进程 fork)。
# 规则: 先剥离 ANSI 序列; 再按 UTF-8 首字节判定宽度 —— 0xE3-0xE9 (CJK/假名)、
# 0xEF (全角形式)、0xF0-0xF4 (emoji) 记 2 列, 其余记 1 列。
UI_WIDTH=0
ui_width() {
    local s="$1" head tail
    while [[ "$s" == *$'\033['* ]]; do
        head="${s%%$'\033['*}"; tail="${s#*$'\033['}"; tail="${tail#*m}"; s="${head}${tail}"
    done
    local LC_ALL=C
    local chars="${s//[$'\x80'-$'\xbf']/}"
    local wide_cjk="${s//[!$'\xe3'-$'\xe9']/}"
    local wide_full="${s//[!$'\xef']/}"
    local wide_emoji="${s//[!$'\xf0'-$'\xf4']/}"
    UI_WIDTH=$(( ${#chars} + ${#wide_cjk} + ${#wide_full} + ${#wide_emoji} ))
}

# 重复字符 n 次, 结果写入 UI_REP。
UI_REP=''
ui_rep() {
    local n="${2:-0}" pad=''
    (( n < 0 )) && n=0
...
# --- 基础绘制 ---------------------------------------------------------------
# 渐变分隔线: 整行按色阶均分, 形成从左到右的光带。
ui_rule() {
    local ch="${1:-$G_H}" width="${2:-$UI_COLS}" out='' i n seg used
    n=${#UI_GRAD[@]}
    if (( ! UI_COLOR )); then
        ui_rep "$ch" "$width"; printf '%s\n' "$UI_REP"; return
    fi
    seg=$(( width / n )); used=0
    for (( i = 0; i < n; i++ )); do
        (( i == n - 1 )) && seg=$(( width - used ))
        ui_rep "$ch" "$seg"
        out+=$'\033[38;5;'"${UI_GRAD[i]}"'m'"$UI_REP"
        used=$(( used + seg ))
    done
    printf '%s%s\n' "$out" "$NC"
}

# 逐字符渐变着色 (仅用于 ASCII 标题, 不参与对齐计算)。
ui_grad_text() {
    local s="$1" i n len out='' idx
    if (( ! UI_COLOR )); then printf '%s' "$s"; return; fi
    n=${#UI_GRAD[@]}; len=${#s}
    (( len == 0 )) && return
    for (( i = 0; i < len; i++ )); do
        idx=$(( i * n / len ))
        out+=$'\033[38;5;'"${UI_GRAD[idx]}"'m'"${s:i:1}"
    done
    printf '%s%s' "$out" "$NC"
}

hr() { ui_rule "$G_H" "$UI_COLS"; }

# --- 消息行 (图标 + 文本, 建立强弱分层) -------------------------------------
ui_msg() {
    local icon="$1" icon_c="$2" text_c="$3"; shift 3
    local m="$*"
    m="${m#"${m%%[![:space:]]*}"}"      # 去掉调用方为对齐附加的前导空格
    printf '%b\n' " ${icon_c}${icon}${NC} ${text_c}${m}${NC}"
}
info() { ui_msg "$G_INFO" "$C_ACC"   "$C_TXT"   "$*"; }
ok()   { ui_msg "$G_OK"   "$C_OKC"   "$C_OKC"   "$*"; }
warn() { ui_msg "$G_WARN" "$C_WARNC" "$C_WARNC" "$*"; }
err()  { ui_msg "$G_ERR"  "$C_ERRC"  "$C_ERRC"  "$*"; }
die()  { err "$*"; exit 1; }
...
# 分组标题: ▊ 标题 ───────────  (右侧横线自动补满整行)
ui_head() {
    local title="$1" col="${2:-$C_ACC}" n
    ui_width "$title"
    n=$(( UI_COLS - UI_WIDTH - 5 )); (( n < 2 )) && n=2
    ui_rep "$G_H" "$n"
    printf '%b\n' "\n ${col}${G_BAR}${NC} ${BOLD}${col}${title}${NC} ${C_DIM2}${UI_REP}${NC}"
}

# 整屏标题 (clear 之后的第一屏): 渐变线包裹的屏名
ui_screen() {
    ui_rule "$G_HH"
    if [[ -n "${2:-}" ]]; then
        printf '%b\n' "  ${BOLD}${C_ACC}${1}${NC}   ${C_DIM2}${2}${NC}"
    else
        printf '%b\n' "  ${BOLD}${C_ACC}${1}${NC}"
    fi
    ui_rule "$G_H"
...
# 次级标题 (流程内的小节): ▎ 标题
ui_sub() {
    printf '%b\n' "\n ${C_ACC2}${G_RAIL}${NC} ${BOLD}${C_TXT}${1}${NC}"
}

# 向导步骤标记: ▎ 步骤 2/4 · 标题
ui_step() {
    printf '%b\n' "\n ${C_ACC}${G_RAIL}${NC} ${BOLD}${C_ACC}步骤 ${1}/${2}${NC} ${C_DIM2}${G_SEP}${NC} ${BOLD}${C_TXT}${3}${NC}"
}
...
# 卡片顶/行/底: 只有左导轨, 内容再长也不会破框
ui_card_top() {
    local title="$1" col="${2:-$C_ACC}" n
    ui_width "$title"
    n=$(( UI_COLS - UI_WIDTH - 6 )); (( n < 2 )) && n=2
    ui_rep "$G_H" "$n"
    printf '%b\n' " ${col}${G_TL}${G_H}${NC} ${BOLD}${col}${title}${NC} ${C_DIM2}${UI_REP}${NC}"
}
ui_card_row() { printf '%b\n' " ${C_DIM2}${G_V}${NC}  $*"; }
ui_card_gap() { printf '%b\n' " ${C_DIM2}${G_V}${NC}"; }
ui_card_end() {
    ui_rep "$G_H" $(( UI_COLS - 2 ))
    printf '%b\n' " ${C_DIM2}${G_BL}${UI_REP}${NC}"
}

# --- 菜单项 -----------------------------------------------------------------
# 单列: ' 01 · 标题        提示'
ui_item() {
    local num="$1" text="$2" hint="${3:-}" col="${4:-$C_ACC}" tag='' w
    printf -v tag '%2s' "$num"
    if [[ -n "$hint" ]]; then
        w=$(( UI_COLS - 36 )); (( w > 34 )) && w=34; (( w < 20 )) && w=20
        ui_pad "$text" "$w"
        [[ -z "$UI_PAD" ]] && UI_PAD='  '
        printf '%b\n' "  ${BOLD}${col}${tag}${NC} ${C_DIM2}${G_SEP}${NC} ${C_TXT}${text}${NC}${UI_PAD}${C_DIM2}${hint}${NC}"
    else
        printf '%b\n' "  ${BOLD}${col}${tag}${NC} ${C_DIM2}${G_SEP}${NC} ${C_TXT}${text}${NC}"
    fi
}

# 双列: 窄终端 (<76 列) 自动回退为两行单列, 保证不折行
ui_item2() {
    local n1="$1" t1="$2" n2="$3" t2="$4" col="${5:-$C_ACC}" tag1='' tag2='' w
    if (( UI_COLS < 76 )); then
        ui_item "$n1" "$t1" "" "$col"
        [[ -n "$n2" ]] && ui_item "$n2" "$t2" "" "$col"
        return
    fi
    w=$(( UI_COLS / 2 - 8 )); (( w < 18 )) && w=18
    printf -v tag1 '%2s' "$n1"
    if [[ -z "$n2" ]]; then
        printf '%b\n' "  ${BOLD}${col}${tag1}${NC} ${C_DIM2}${G_SEP}${NC} ${C_TXT}${t1}${NC}"
        return
    fi
    printf -v tag2 '%2s' "$n2"
    ui_pad "$t1" "$w"
    [[ -z "$UI_PAD" ]] && UI_PAD='  '
    printf '%b\n' "  ${BOLD}${col}${tag1}${NC} ${C_DIM2}${G_SEP}${NC} ${C_TXT}${t1}${NC}${UI_PAD}${BOLD}${col}${tag2}${NC} ${C_DIM2}${G_SEP}${NC} ${C_TXT}${t2}${NC}"
}

# 返回项统一样式
ui_item_back() { ui_item "0" "${1:-返回}" "" "$C_MUT"; }

# 节点行: 编号 + 类型标签 + 备注名 + 端口, 三段按显示宽度对齐
ui_node_row() {
    local idx="$1" type="$2" name="$3" port="${4:-}" tag='' pad_type
    printf -v tag '%2s' "$idx"
    ui_pad "$type" 24; pad_type="$UI_PAD"
    [[ -z "$pad_type" ]] && pad_type=' '
    ui_pad "$name" 22
    [[ -z "$UI_PAD" ]] && UI_PAD=' '
    if [[ -n "$port" ]]; then
        printf '%b\n' "  ${BOLD}${C_ACC}${tag}${NC} ${C_DIM2}${G_SEP}${NC} ${C_VIO}${type}${NC}${pad_type}${C_TXT}${name}${NC}${UI_PAD}${C_DIM2}端口 ${port}${NC}"
    else
        printf '%b\n' "  ${BOLD}${C_ACC}${tag}${NC} ${C_DIM2}${G_SEP}${NC} ${C_VIO}${type}${NC}${pad_type}${C_TXT}${name}${NC}"
    fi
}

# --- 键值对 / 徽章 / 进度条 -------------------------------------------------
ui_kv() {
    local k="$1" v="$2" w="${3:-12}"
    ui_pad "$k" "$w"
    [[ -z "$UI_PAD" ]] && UI_PAD='  '        # 键名超宽时至少保留一个视觉间隙
    printf '%b\n' "   ${C_KEY}${k}${NC}${UI_PAD}${C_VAL}${v}${NC}"
}

# 状态徽章: on 运行中 / off 已停止 / none 未安装 / set 已配置 / warn 注意
ui_badge() {
    local state="$1" text="${2:-}" c g label
    case "$state" in
        on)   c="$C_OKC";   g="$G_DOT";  label="运行中" ;;
        off)  c="$C_ERRC";  g="$G_DOT";  label="已停止" ;;
        none) c="$C_MUT";   g="$G_RING"; label="未安装" ;;
        set)  c="$C_ACC";   g="$G_DOT";  label="已配置" ;;
        warn) c="$C_WARNC"; g="$G_DOT";  label="注意"   ;;
        *)    c="$C_MUT";   g="$G_RING"; label="未知"   ;;
    esac
    [[ -n "$text" ]] && label="$text"
    printf '%b' "${c}${g} ${label}${NC}"
}
...
ui_svc_row() {
    ...
    ui_pad "$name" 12
    [[ -z "$UI_PAD" ]] && UI_PAD=' '
    printf '%b\n' "   ${C_TXT}${name}${NC}${UI_PAD}$(ui_badge "$state")   ${C_DIM2}${extra}${NC}"
}

# 用量进度条: <70% 绿 / <90% 琥珀 / >=90% 红
ui_bar() {
    local p="$1" w="${2:-16}" filled col fill empty
    [[ "$p" =~ ^[0-9]+$ ]] || p=0
    (( p > 100 )) && p=100
    filled=$(( p * w / 100 ))
    if   (( p >= 90 )); then col="$C_ERRC"
    elif (( p >= 70 )); then col="$C_WARNC"
    else col="$C_OKC"; fi
    ui_rep "$G_FULL" "$filled";        fill="$UI_REP"
    ui_rep "$G_EMPTY" $(( w - filled )); empty="$UI_REP"
    printf '%b' "${col}${fill}${C_DIM2}${empty}${NC}"
}

# --- 输入提示 ---------------------------------------------------------------
# 用法: read -rp "$(ui_prompt "请选择" "0-22")" choice
ui_prompt() {
    local text="$1" hint="${2:-}"
    if [[ -n "$hint" ]]; then
        printf '%b' "\n ${C_ACC}${G_ARROW}${NC} ${BOLD}${C_TXT}${text}${NC} ${C_DIM2}[${hint}]${NC} ${C_ACC}${G_TIP}${NC} "
    else
        printf '%b' "\n ${C_ACC}${G_ARROW}${NC} ${BOLD}${C_TXT}${text}${NC} ${C_ACC}${G_TIP}${NC} "
    fi
}
# 危险/确认类提示 (琥珀色箭头)
ui_prompt_warn() {
    printf '%b' "\n ${C_WARNC}${G_ARROW}${NC} ${BOLD}${C_TXT}${1}${NC} ${C_DIM2}[${2}]${NC} ${C_WARNC}${G_TIP}${NC} "
}
...
XRAY_CN_BLOCK_TAG="proxy-manager-block-cn"
...
SNELL_V6_BIN="/usr/local/bin/snell-server-v6"
SNELL_V6_DIR="/etc/snell-v6"
SNELL_V6_VERSION="6.0.0rc"
SNELL_V6_SHA256_AMD64="21c4aa6b4a208236f33e9923603acd8a26534a02104aed40496ddf77949dfb4b"
SNELL_V6_SHA256_ARM64="2b47d111d648648cf6845886433a7a93404ffe8d68a8e447058de6f7eca0d1a7"
...
MIERU_CLIENT_DIR="/root/mieru-clients/current"
MIERU_STABLE_VERSION="3.35.0"
...
# 参考 233boy/Xray commit 5075a7c8d: 移除 Cloudflare 域名, 避免共享证书特征风险
REALITY_SNIS=(
    "www.microsoft.com" "www.apple.com" "www.amazon.com"
    "dl.google.com" "www.icloud.com"
...
# ---- GitHub Release / 下载校验 (参考 ike-sh 系列脚本) ----
# GitHub API 有速率限制且部分机房不可达, 失败时回退到 releases/latest 跳转解析。
gh_latest_tag() {
    local repo="$1" tag
    tag="$(curl -fsSL --max-time 15 "https://api.github.com/repos/${repo}/releases/latest" 2>/dev/null | jq -r '.tag_name // empty')"
    if [[ -z "$tag" ]]; then
        tag="$(curl -fsSLI -o /dev/null -w '%{url_effective}' --max-time 15 \
            "https://github.com/${repo}/releases/latest" 2>/dev/null | grep -oE '[^/]+$')"
        [[ "$tag" == "latest" ]] && tag=""
    fi
    printf '%s' "$tag"
}

# 校验下载文件的 SHA256; 校验文件不存在时跳过 (不阻断安装), 校验不一致则失败。
# 兼容 "<hash>  <file>" 与 Xray 的 .dgst ("SHA2-256= <hash>") 两种格式。
verify_sha256() {
    local file="$1" sha_url="$2" tmp expected actual
    command -v sha256sum >/dev/null 2>&1 || command -v shasum >/dev/null 2>&1 || return 0
    tmp="$(mktemp)"
    if ! curl -fsSL --max-time 30 "$sha_url" -o "$tmp" 2>/dev/null; then
        rm -f "$tmp"; return 0
    fi
    expected="$(grep -oiE '\b[0-9a-f]{64}\b' "$tmp" | head -n1)"
    rm -f "$tmp"
...
# ---- 本机防火墙自动放行 (ufw / firewalld / iptables) ----
# 输入为多行 "proto port", 例如 $'tcp 2080\nudp 2081'
pm_firewall_backend() {
...
fw_allow_pairs() {
    local pairs="$1" fw proto port
    [[ -n "$pairs" ]] || return 0
    fw="$(pm_firewall_backend)"
    [[ -n "$fw" ]] || { warn "未检测到本地防火墙工具, 请自行在云安全组放行端口。"; return 0; }
    while read -r proto port; do
        [[ -z "$port" ]] && continue
        case "$fw" in
            ufw) ufw allow "${port}/${proto}" >/dev/null 2>&1 || true ;;
            firewalld) firewall-cmd --permanent --add-port="${port}/${proto}" >/dev/null 2>&1 || true ;;
            iptables)
    ...
    [[ "$fw" == firewalld ]] && firewall-cmd --reload >/dev/null 2>&1
    pm_persist_iptables "$fw"
    cloud_firewall_hint "$pairs"
}
...
fw_deny_pairs() {
    ...
    while read -r proto port; do
        ...
        case "$fw" in
            ...
            firewalld) firewall-cmd --permanent --remove-port="${port}/${proto}" >/dev/null 2>&1 || true ;;
            iptables)
...
cloud_firewall_hint() {
    local pairs="$1" specs=() proto port
    while read -r proto port; do
        [[ -z "$port" ]] && continue
        specs+=("${port}/${proto}")
    done <<< "$pairs"
    (( ${#specs[@]} == 0 )) && return 0
    warn "云安全组提醒: 请在 VPS 控制台放行 $(IFS=','; echo "${specs[*]}")"
}
...
gen_uuid() {
    local u=""
    if [[ -x "$SB_BIN" ]];   then u="$("$SB_BIN" generate uuid 2>/dev/null)"; fi
    if [[ -z "$u" && -x "$XRAY_BIN" ]]; then u="$("$XRAY_BIN" uuid 2>/dev/null)"; fi
    if [[ -z "$u" && -r /proc/sys/kernel/random/uuid ]]; then u="$(cat /proc/sys/kernel/random/uuid)"; fi
    if [[ -z "$u" ]] && command -v uuidgen >/dev/null 2>&1; then u="$(uuidgen)"; fi
    if [[ -z "$u" ]]; then
        u="$(openssl rand -hex 16 | sed 's/\(........\)\(....\)\(....\)\(....\)\(............\)/\1-\2-\3-\4-\5/')"
    fi
    printf '%s\n' "$u"
}
...
ask_port_with_default() {
    ...
    while true; do
        read -rp "$(ui_prompt "$prompt" "回车=$def")" p
        [[ -z "$p" ]] && p="$def"
...
ask_sni() {
    ...
    read -rp "$(ui_prompt "Reality 伪装域名 SNI" "回车=$def")" s
    [[ -z "$s" ]] && s="$def"
...
ask_tls_sni() {
    ...
    read -rp "$(ui_prompt "TLS SNI" "回车=$def")" s
    REPLY_SNI="${s:-$def}"
...
ask_name() {
    local def="$1" n
    read -rp "$(ui_prompt "节点备注名" "回车=$def")" n
    REPLY_NAME="${n:-$def}"
}
...
ask_value_default() {
    ...
    read -rp "$(ui_prompt "$prompt" "回车=$def")" v
    REPLY_VALUE="${v:-$def}"
}
...
ask_secret_value() {
    ...
    while true; do
        ask_value_default "$prompt" "$def"; v="$REPLY_VALUE"
        [[ "$v" == *":"* || "$v" == *"/"* || "$v" == *"?"* || "$v" == *"&"* || "$v" == *"@"* || "$v" == *"#"* || "$v" == *"%"* || "$v" == *" "* ]] && { err "${prompt}不能包含 :、/、?、&、@、#、% 或空格。"; continue; }
...
confirm() {
    ...
    if [[ "$def" == "y" ]]; then read -rp "$(ui_prompt_warn "$prompt" "Y/n")" ans; ans="${ans:-y}"
    else read -rp "$(ui_prompt_warn "$prompt" "y/N")" ans; ans="${ans:-n}"; fi
    [[ "$ans" =~ ^[Yy]$ ]]
}

pause() {
    echo
    read -n1 -s -r -p "$(printf '%b' "   ${C_DIM2}${G_ENTER} 按任意键返回菜单${NC}")"
    echo
}
break_end() { ok "操作完成"; pause; }

# 渐进披露: 每个参数都有安全的随机/推荐默认值, 默认零交互直接生成;
# 只有当用户回答 "y" 时才逐项进入 ask_xxx 手动模式。
ask_customize() {
    confirm "是否自定义其余参数 (默认全部随机生成, 回车跳过)" "n"
}

# --- 自动生成 (auto_*) 与 按需询问 (choose_*) 双轨 ---------------------------
auto_uuid()      { gen_uuid; }
auto_short_id()  { gen_short_id; }
auto_path()      { printf '/%s' "$(openssl rand -hex 4)"; }
auto_service()   { printf 'grpc%s' "$(openssl rand -hex 3)"; }
auto_sni()       { printf '%s' "${REALITY_SNIS[$((RANDOM % ${#REALITY_SNIS[@]}))]}"; }
auto_secret()    { gen_pass "${1:-16}"; }
auto_username()  { printf '%s%s' "${1:-user}" "$(openssl rand -hex 2)"; }

# choose_xxx: 一键模式下静默生成; 自定义模式下复用原有 ask_xxx 校验交互。
choose_uuid()    { if [[ "${1:-}" == "-i" ]]; then shift; ask_uuid_value "${1:-UUID}"; REPLY_UUID; else auto_uuid; fi; }
choose_short_id(){ if [[ "${1:-}" == "-i" ]]; then ask_short_id_value; REPLY_SHORT_ID; else auto_short_id; fi; }
choose_path()    { if [[ "${1:-}" == "-i" ]]; then shift; ask_path_value "${1:-Path}"; REPLY_PATH; else auto_path; fi; }
choose_service() { if [[ "${1:-}" == "-i" ]]; then shift; ask_service_value "${1:-gRPC serviceName}"; REPLY_SERVICE; else auto_service; fi; }
choose_sni()     { if [[ "${1:-}" == "-i" ]]; then ask_sni; REPLY_SNI; else auto_sni; fi; }
choose_tls_sni() { if [[ "${1:-}" == "-i" ]]; then shift; ask_tls_sni "$@"; REPLY_SNI; else printf 'www.bing.com'; fi; }
choose_secret()  { if [[ "${1:-}" == "-i" ]]; then shift; ask_secret_value "$@"; REPLY_PASS; else auto_secret 16; fi; }
choose_username(){ if [[ "${1:-}" == "-i" ]]; then shift; ask_username_value "$@"; REPLY_USER; else auto_username; fi; }

choose_reality_keys() {
    if [[ "${1:-}" == "-i" ]]; then
        ask_xray_reality_keys || return 1
    else
...
install_singbox() {
    ...
    [[ -z "$tag" ]] && { err "无法获取 sing-box 版本 (GitHub 访问失败), 请检查网络。"; return 1; }
    ver="${tag#v}"
    ...
    verify_sha256 "$tmp/sb.tar.gz" "${url}.sha256" || { rm -rf "$tmp"; return 1; }
...
require_singbox() {
    if sb_installed; then
        if [[ ! -f "$SB_CONFIG" ]]; then
            warn "检测到 sing-box 内核但配置文件缺失, 正在初始化基础配置..."
            mkdir -p "$SB_DIR" || { err "无法创建 sing-box 配置目录: $SB_DIR"; return 1; }
            init_sb_config || { err "无法初始化 sing-box 配置: $SB_CONFIG"; return 1; }
        fi
        return 0
    fi
    warn "尚未安装 sing-box, 现在开始安装..."
    install_singbox
}
...
install_xray() {
    ...
    [[ -z "$tag" ]] && { err "无法获取 Xray 版本 (GitHub 访问失败), 请检查网络。"; return 1; }
    ...
    verify_sha256 "$tmp/xray.zip" "${url}.dgst" || { rm -rf "$tmp"; return 1; }
...
require_xray() {
    if xray_installed; then
        # 二进制可能由其他安装方式保留，但本脚本管理的配置文件尚未创建。
        if [[ ! -f "$XRAY_CONFIG" ]]; then
            warn "检测到 Xray 内核但配置文件缺失, 正在初始化基础配置..."
            mkdir -p "$XRAY_DIR" || { err "无法创建 Xray 配置目录: $XRAY_DIR"; return 1; }
            init_xray_config || { err "无法初始化 Xray 配置: $XRAY_CONFIG"; return 1; }
        fi
        return 0
    fi
    warn "尚未安装 Xray, 现在开始安装..."
    install_xray
}
...
xray_cn_block_mode() {
    [[ -f "$XRAY_CONFIG" ]] || { printf 'off'; return 0; }
    local has_ip=false has_domain=false
    jq -e --arg tag "$XRAY_CN_BLOCK_TAG" '
        any(.routing.rules[]?; . == {type:"field", ip:["geoip:cn"], outboundTag:$tag})' \
        "$XRAY_CONFIG" >/dev/null 2>&1 && has_ip=true
    jq -e --arg tag "$XRAY_CN_BLOCK_TAG" '
        any(.routing.rules[]?; . == {type:"field", domain:["geosite:cn"], outboundTag:$tag})' \
        "$XRAY_CONFIG" >/dev/null 2>&1 && has_domain=true
    if $has_ip && $has_domain; then printf 'enhanced'
    elif $has_ip; then printf 'basic'
    else printf 'off'; fi
}

xray_cn_block_status() {
    case "$(xray_cn_block_mode)" in
        basic) printf '基础模式 (geoip:cn)' ;;
        enhanced) printf '增强模式 (geoip:cn + geosite:cn)' ;;
        *) printf '关闭' ;;
    esac
}

xray_cn_block_check_assets() {
    local mode="$1" missing=()
    [[ -f "$XRAY_SHARE/geoip.dat" ]] || missing+=("geoip.dat")
    [[ "$mode" != "enhanced" || -f "$XRAY_SHARE/geosite.dat" ]] || missing+=("geosite.dat")
    (( ${#missing[@]} == 0 )) && return 0
    err "缺少 Xray 路由资源: ${missing[*]}"
    warn "请先在「内核与服务管理」中安装 / 更新 Xray 内核。"
    return 1
}

set_xray_cn_block() {
    local mode="$1" tmp backup state_tmp
    case "$mode" in off|basic|enhanced) ;; *) err "未知屏蔽模式: $mode"; return 1 ;; esac
    require_xray || return 1
    [[ "$mode" == "off" ]] || xray_cn_block_check_assets "$mode" || return 1

    tmp="$(mktemp_json)" || { err "创建临时配置失败。"; return 1; }
    backup="$(mktemp_json)" || { rm -f "$tmp"; err "创建配置备份失败。"; return 1; }
    cp "$XRAY_CONFIG" "$backup" || { rm -f "$tmp" "$backup"; err "备份 Xray 配置失败。"; return 1; }

    if ! jq --arg mode "$mode" --arg tag "$XRAY_CN_BLOCK_TAG" '
        def managed_cn_rule:
            . == {type:"field", ip:["geoip:cn"], outboundTag:$tag} or
            . == {type:"field", domain:["geosite:cn"], outboundTag:$tag};
        .outbounds = (.outbounds // []) |
        .routing = (.routing // {}) |
        .routing.rules = ((.routing.rules // []) | map(select((managed_cn_rule) | not))) |
        if $mode == "off" then
            .outbounds |= map(select(.tag != $tag))
        else
            (if any(.outbounds[]?; .tag == $tag) then .
             else .outbounds += [{protocol:"blackhole", tag:$tag}] end) |
            .routing.rules = (
    ...
    ' "$XRAY_CONFIG" > "$tmp"; then
        rm -f "$tmp" "$backup"; err "生成 Xray 屏蔽规则失败。"; return 1
    fi
    if ! "$XRAY_BIN" -test -config "$tmp" >/tmp/pm_check.log 2>&1; then
        err "Xray 配置校验未通过, 已放弃修改:"
        sed 's/^/    /' /tmp/pm_check.log
        rm -f "$tmp" "$backup"
        return 1
    fi

    mv "$tmp" "$XRAY_CONFIG"; chmod 600 "$XRAY_CONFIG"
    if ! restart_xray; then
        warn "Xray 重启失败, 正在恢复原配置..."
        cp "$backup" "$XRAY_CONFIG"; chmod 600 "$XRAY_CONFIG"
    ...
    init_state
    state_tmp="$(mktemp)"
    if jq --arg mode "$mode" '.meta.xray_cn_block_mode=$mode' "$STATE" > "$state_tmp"; then
        mv "$state_tmp" "$STATE"; chmod 600 "$STATE"
    else
        rm -f "$state_tmp"
        warn "规则已生效, 但状态备注写入失败。"
    fi
    case "$mode" in
        basic) ok "已开启大陆直连屏蔽基础模式。" ;;
        enhanced) ok "已开启大陆直连屏蔽增强模式。" ;;
        off) ok "已关闭大陆直连屏蔽。" ;;
    esac
}

configure_xray_cn_block() {
    ui_head "屏蔽中国大陆直连" "$C_WARNC"
    ui_note "当前: $(xray_cn_block_status)"
    ui_item 1 "开启基础模式" "仅阻断 geoip:cn IP" "$C_WARNC"
    ui_item 2 "开启增强模式" "阻断 geoip:cn IP 与 geosite:cn 域名" "$C_WARNC"
    ui_item 3 "关闭"
    ui_item_back "保持当前状态"
    warn "Reality 类协议需服务端直连 SNI target; 若目标命中规则, 节点可能无法连接。"
    local c; read -rp "$(ui_prompt "请选择" "0-3")" c
    case "$c" in
        1) set_xray_cn_block basic ;;
        2) set_xray_cn_block enhanced ;;
        3) set_xray_cn_block off ;;
        *) return 0 ;;
    esac
}

# ---------------------------------------------------------------------------
#  出站 IP 协议族优先 (参考 233boy/Xray commit c35214c48)
#  通过 Xray freedom 出站的 domainStrategy 控制域名解析的地址族优先级:
#    AsIs       自动 (删除 domainStrategy, 由系统决定)
#    UseIPv4v6  优先 IPv4 (先 A 后 AAAA)
#    UseIPv6v4  优先 IPv6 (先 AAAA 后 A)
#  sing-box direct 出站则对应 domain_strategy: prefer_ipv4 / prefer_ipv6。
# ---------------------------------------------------------------------------
xray_outbound_strategy() {
    [[ -f "$XRAY_CONFIG" ]] || { echo "未配置"; return; }
    jq -r '(.outbounds[]? | select(.tag == "direct" and .protocol == "freedom") | .settings.domainStrategy) // "AsIs"' \
        "$XRAY_CONFIG" 2>/dev/null
}

sb_outbound_strategy() {
    [[ -f "$SB_CONFIG" ]] || { echo "未配置"; return; }
    jq -r '(.outbounds[]? | select(.tag == "direct" and .type == "direct") | .domain_strategy) // "自动"' \
        "$SB_CONFIG" 2>/dev/null
}

set_xray_outbound_strategy() {
    local strategy="$1" tmp
    xray_installed || { warn "未安装 Xray, 跳过。"; return 0; }
    tmp="$(mktemp_json)"
    if [[ "$strategy" == "AsIs" ]]; then
        jq '(.outbounds[]? | select(.tag == "direct" and .protocol == "freedom")) |=
            (.settings = ((.settings // {}) | del(.domainStrategy)) | if .settings == {} then del(.settings) else . end)' \
            "$XRAY_CONFIG" > "$tmp" || { rm -f "$tmp"; err "配置更新失败。"; return 1; }
    else
        jq --arg s "$strategy" \
            '(.outbounds[]? | select(.tag == "direct" and .protocol == "freedom") | .settings.domainStrategy) = $s' \
            "$XRAY_CONFIG" > "$tmp" || { rm -f "$tmp"; err "配置更新失败。"; return 1; }
    fi
    mv "$tmp" "$XRAY_CONFIG"; chmod 600 "$XRAY_CONFIG"
    restart_xray >/dev/null 2>&1 || true
...
set_sb_outbound_strategy() {
    local ds="$1" tmp
    sb_installed || { warn "未安装 sing-box, 跳过。"; return 0; }
    tmp="$(mktemp_json)"
    if [[ -z "$ds" ]]; then
        jq '(.outbounds[]? | select(.tag == "direct" and .type == "direct")) |= del(.domain_strategy)' \
            "$SB_CONFIG" > "$tmp" || { rm -f "$tmp"; err "配置更新失败。"; return 1; }
    else
        jq --arg s "$ds" \
            '(.outbounds[]? | select(.tag == "direct" and .type == "direct") | .domain_strategy) = $s' \
            "$SB_CONFIG" > "$tmp" || { rm -f "$tmp"; err "配置更新失败。"; return 1; }
    fi
    mv "$tmp" "$SB_CONFIG"; chmod 600 "$SB_CONFIG"
    restart_singbox >/dev/null 2>&1 || true
}

configure_outbound_strategy() {
    ui_head "出站 IP 协议优先" "$C_ACC2"
    xray_installed && ui_note "Xray 当前: $(xray_outbound_strategy)"
    sb_installed   && ui_note "sing-box 当前: $(sb_outbound_strategy)"
    ui_item 1 "自动"       "默认 · 由系统解析顺序决定" "$C_ACC2"
    ui_item 2 "优先 IPv4"  "UseIPv4v6 / prefer_ipv4"    "$C_ACC2"
    ui_item 3 "优先 IPv6"  "UseIPv6v4 / prefer_ipv6"    "$C_ACC2"
    ui_item_back "保持当前设置"
    local c; read -rp "$(ui_prompt "请选择" "0-3")" c
    local xs="" sbds="" label=""
    case "$c" in
        1) xs="AsIs";       sbds="";            label="自动" ;;
        2) xs="UseIPv4v6";  sbds="prefer_ipv4"; label="优先 IPv4" ;;
        3) xs="UseIPv6v4";  sbds="prefer_ipv6"; label="优先 IPv6" ;;
        *) return 0 ;;
    esac
    xray_installed && set_xray_outbound_strategy "$xs"
    sb_installed   && set_sb_outbound_strategy "$sbds"
    ok "出站 IP 优先级已更新为: ${label}"
}
...
tls_cert_sha256() {
    local hash
    [[ -f "$SB_CERT" ]] || return 1
    hash="$(openssl x509 -fingerprint -noout -sha256 -in "$SB_CERT" 2>/dev/null | sed 's/.*=//' | tr -d ':' | tr 'A-F' 'a-f')"
...
# 从 inbound JSON 解析 "proto port" 行 (用于防火墙自动放行)
inbound_port_pairs() {
    local core="$1" inb="$2"
    if [[ "$core" == singbox ]]; then
        printf '%s' "$inb" | jq -r '
            select(.listen_port != null)
            | (if (.type // "") | test("hysteria2|tuic|naive") then "udp" else "tcp" end) + " " + (.listen_port|tostring)' 2>/dev/null
    else
        printf '%s' "$inb" | jq -r '
            select(.port != null)
            | (if ((.streamSettings.network // "") == "kcp") then "udp" else "tcp" end) + " " + (.port|tostring)' 2>/dev/null
    fi
}
...
core_add_inbounds() {
    ...
    # 新节点端口自动放行本机防火墙 (仅对外监听的 inbound; 127.0.0.1 内层服务跳过)
    local pairs=""
    for arg in "$@"; do
        [[ "$(printf '%s' "$arg" | jq -r '.listen // "0.0.0.0"')" == 127.0.0.1 ]] && continue
        pairs+="$(inbound_port_pairs "$core" "$arg")"$'\n'
    done
    fw_allow_pairs "$(printf '%s' "$pairs" | sed '/^$/d')"
    return 0
}
...
choose_same_type_policy() {
    ...
    ui_item 1 "新增共存" "默认, 不影响已有节点"
    ui_item 2 "覆盖旧节点" "删除已有同类型节点后保留新节点"
    read -rp "$(ui_prompt "请选择" "回车=1")" choice
    [[ "$choice" == "2" ]] || return 0
...
ask_xray_reality_keys() {
    local priv pub
    read -rp "$(ui_prompt "Reality PrivateKey" "回车自动生成")" priv
    if [[ -z "$priv" ]]; then
        gen_xray_reality_keys || return 1
        ok "  Reality key 已自动生成。"
        return 0
    fi
    read -rp "$(ui_prompt "Reality PublicKey (手动 PrivateKey 时必填)")" pub
...
ask_xray_vless_enc() {
    local dec enc
    read -rp "$(ui_prompt "VLESS decryption" "回车自动生成")" dec
    if [[ -z "$dec" ]]; then
        gen_xray_vless_enc || return 1
        ok "  VLESS 加密参数已自动生成。"
        return 0
    fi
    read -rp "$(ui_prompt "VLESS encryption (手动 decryption 时必填)")" enc
...
# 八、结果展示
# ---------------------------------------------------------------------------
pm_label_display_width() {
    local label="$1"
    case "$label" in
        端口|备注|状态|节点|序号|地址|密码|类型|加密|协议|服务|配置|版本|插件|公钥|指纹|传输|安全|证书)
            echo 4 ;;
        SNI|PSK|MTU|DNS)
            echo 3 ;;
        UUID|Flow|ALPN)
        ...
        用户名|管理员|到期日)
            echo 6 ;;
        计费模式|已用流量|流量配额|剩余流量|使用比例|到期时间|邮箱通知|概览推送|诊断结果|后端端口|本机端口|对外端口|拥塞控制)
            echo 8 ;;
        服务端端口|客户端私钥|客户端公钥|encryption)
            echo 10 ;;
        流量重置日期|握手域名\ SNI)
            echo 12 ;;
        客户端内网\ IP|ShadowTLS\ SNI)
            echo 13 ;;
        Snell\ 后端端口|ShadowTLS\ 密码)
            echo 14 ;;
        gRPC\ serviceName)
            echo 16 ;;
        encryption\(客户端\))
...
pm_aligned_line() {
    local label="$1" value="${2:-}" label_width="${3:-12}"
    local display_width pad_len pad=""
    display_width=$(pm_label_display_width "$label")
    [ "$display_width" -gt "$label_width" ] && label_width="$display_width"
    pad_len=$((label_width - display_width))
...
pm_format_detail_text() {
    local detail="$1" line label value
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [[ "$line" == *:* ]]; then
            ...
            if [ -n "$label" ] && [ -n "$value" ]; then
                pm_aligned_line "$label" "$value" 18
                continue
            fi
        fi
        printf '%s\n' "$line"
...
show_result() {
    ...
    echo
    ui_card_top "${G_OK} 部署成功  ${G_SEP}  ${name}" "$C_OKC"
    ui_card_gap
    ui_card_row "${BOLD}${C_KEY}分享链接${NC} ${C_DIM2}(整行复制到客户端导入)${NC}"
    ui_card_row "${BOLD}${C_OKC}${link}${NC}"
    local cy; cy="$(clash_node_yaml "${LAST_NODE_TYPE:-}" "$name" "$link")"
    if [[ -n "$cy" ]]; then
        ui_card_gap
        ui_card_row "${BOLD}${C_KEY}Mihomo / Clash 配置${NC}"
        local yline
        while IFS= read -r yline; do
            [[ -z "$yline" ]] && continue
            ui_card_row "${C_VAL}${yline}${NC}"
        done <<< "$cy"
    fi
    ui_card_gap
    if command -v qrencode >/dev/null 2>&1; then
        ui_card_row "${BOLD}${C_KEY}二维码${NC} ${C_DIM2}(客户端扫码导入)${NC}"
        ui_card_end
        qrencode -t ANSIUTF8 "$link"
    else
        ui_card_row "${C_WARNC}${G_WARN} 未安装 qrencode, 跳过二维码。${NC}"
        ui_card_end
    fi
    return 0
}
...
add_xray_xhttp_reality() {
    require_xray || return 1
    ui_deploy_head "VLESS-XHTTP-Reality" "Xray 独有传输, 抗封锁抗探测"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid sid path
    if [[ $custom -eq 0 ]]; then
        ask_sni;              sni="$REPLY_SNI"
        ask_name "XHTTP-Reality-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_short_id_value;   sid="$REPLY_SHORT_ID"
        ask_path_value "XHTTP Path"; path="$REPLY_PATH"
        ask_xray_reality_keys || { err "Xray Reality 密钥生成失败。"; return 1; }
    else
        sni="$(auto_sni)";   name="XHTTP-Reality-${port}"
        uuid="$(auto_uuid)"; sid="$(auto_short_id)"; path="$(auto_path)"
        gen_xray_reality_keys || { err "Xray Reality 密钥生成失败。"; return 1; }
        info "  SNI ${sni} · UUID/ShortID/Path 均已随机生成"
    fi
...
add_xray_vision_reality() {
    require_xray || return 1
    ui_deploy_head "VLESS-Reality-Vision" "抗封锁首选"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid sid
    if [[ $custom -eq 0 ]]; then
        ask_sni;             sni="$REPLY_SNI"
        ask_name "Xray-Vision-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_short_id_value;  sid="$REPLY_SHORT_ID"
        ask_xray_reality_keys || { err "Xray Reality 密钥生成失败。"; return 1; }
    else
        sni="$(auto_sni)";   name="Xray-Vision-${port}"
        uuid="$(auto_uuid)"; sid="$(auto_short_id)"
        gen_xray_reality_keys || { err "Xray Reality 密钥生成失败。"; return 1; }
        info "  SNI ${sni} · UUID/ShortID 均已随机生成"
    fi
...
add_xray_vless_encryption() {
    require_xray || return 1
    ui_deploy_head "VLESS-Encryption" "Xray 原生加密, 需 v25.9.5+ 客户端"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local name uuid
    if [[ $custom -eq 0 ]]; then
        ask_name "VLESS-Enc-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_xray_vless_enc || { err "生成 VLESS 加密参数失败, 请确认 Xray 版本 >= v25.9.5 (菜单可更新 Xray)。"; return 1; }
    else
        name="VLESS-Enc-${port}"; uuid="$(auto_uuid)"
        gen_xray_vless_enc || { err "生成 VLESS 加密参数失败, 请确认 Xray 版本 >= v25.9.5 (菜单可更新 Xray)。"; return 1; }
        info "  UUID 已随机生成 · VLESS 加密参数已自动生成"
    fi
...
add_vless_grpc_reality() {
    require_xray || return 1
    ui_deploy_head "VLESS-gRPC-Reality"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid sid svc
    if [[ $custom -eq 0 ]]; then
        ask_sni;             sni="$REPLY_SNI"
        ask_name "VLESS-gRPC-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_short_id_value;  sid="$REPLY_SHORT_ID"
        ask_service_value "gRPC serviceName"; svc="$REPLY_SERVICE"
        ask_xray_reality_keys || { err "Reality 密钥生成失败。"; return 1; }
    else
        sni="$(auto_sni)";     name="VLESS-gRPC-${port}"
        uuid="$(auto_uuid)";   sid="$(auto_short_id)"; svc="$(auto_service)"
        gen_xray_reality_keys || { err "Reality 密钥生成失败。"; return 1; }
        info "  SNI ${sni} · UUID/ShortID/serviceName 均已随机生成"
    fi
...
add_vless_encryption_xhttp() {
    require_xray || return 1
    ui_deploy_head "VLESS-Encryption-XHTTP" "需 Xray/v2rayN v25.9.5+ 客户端"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local name uuid path
    if [[ $custom -eq 0 ]]; then
        ask_name "VLESS-Enc-XHTTP-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_path_value "XHTTP Path"; path="$REPLY_PATH"
        ask_xray_vless_enc || { err "VLESS 加密参数生成失败, 请确认 Xray >= v25.9.5。"; return 1; }
    else
        name="VLESS-Enc-XHTTP-${port}"; uuid="$(auto_uuid)"; path="$(auto_path)"
        gen_xray_vless_enc || { err "VLESS 加密参数生成失败, 请确认 Xray >= v25.9.5。"; return 1; }
        info "  UUID/Path 已随机生成 · VLESS 加密参数已自动生成"
    fi
...
add_vless_tls() {
    ...
    local label default_name tag_prefix cert_sha
    cert_sha="$(tls_cert_sha256)" || { err "无法计算 TLS 证书 SHA256 指纹。"; return 1; }
    case "$transport" in
    ...
    ui_deploy_head "${label}" "自签证书模式, 分享链接将携带证书指纹"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "${default_name}-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
    else
        sni="www.bing.com";  name="${default_name}-${port}"; uuid="$(auto_uuid)"
        info "  TLS SNI ${sni} · UUID 已随机生成"
    fi
    ...
    case "$transport" in
        ws)
            if [[ $custom -eq 0 ]]; then ask_path_value "WebSocket Path"; path="$REPLY_PATH"; else path="$(auto_path)"; fi
            ...
            link="vless://${uuid}@${host}:${port}?encryption=none&security=tls&sni=$(url_encode "$sni")&pcs=${cert_sha}&type=ws&host=$(url_encode "$sni")&path=$(url_encode "$path")#$(url_encode "$name")"
            transport_desc="WebSocket  Path: ${path}"
            ;;
        grpc)
            if [[ $custom -eq 0 ]]; then ask_service_value "gRPC serviceName"; svc="$REPLY_SERVICE"; else svc="$(auto_service)"; fi
            ...
            link="vless://${uuid}@${host}:${port}?encryption=none&security=tls&sni=$(url_encode "$sni")&pcs=${cert_sha}&type=grpc&serviceName=$(url_encode "$svc")&mode=gun#$(url_encode "$name")"
            transport_desc="gRPC serviceName: ${svc}"
            ;;
        xhttp)
            if [[ $custom -eq 0 ]]; then ask_path_value "XHTTP Path"; path="$REPLY_PATH"; else path="$(auto_path)"; fi
            stream="$(jq -cn --arg sni "$sni" --arg cert "$SB_CERT" --arg key "$SB_KEY" --arg path "$path" '{
                network:"xhttp", security:"tls",
                tlsSettings:{alpn:["h2","http/1.1"], certificates:[{certificateFile:$cert, keyFile:$key}]},
                xhttpSettings:{path:$path, mode:"auto"}
            }')"
            link="vless://${uuid}@${host}:${port}?encryption=none&security=tls&sni=$(url_encode "$sni")&pcs=${cert_sha}&type=xhttp&path=$(url_encode "$path")&mode=auto#$(url_encode "$name")"
    ...
    local detail="  地址: ${host}\n  端口: ${port}\n  UUID: ${uuid}\n  传输: ${transport_desc}\n  TLS SNI: ${sni}\n  证书: 自签名, 分享链接已包含 pcs 指纹; Mihomo 使用跳过证书验证"
...
add_vless_enc_xhttp_finalmask() {
    require_xray || return 1
    ui_deploy_head "VLESS-Encryption-XHTTP-FinalMask" "需 Xray v26.3.27+ 客户端"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local name uuid path
    if [[ $custom -eq 0 ]]; then
        ask_name "ENC-XHTTP-FM-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_path_value "XHTTP Path"; path="$REPLY_PATH"
        ask_xray_vless_enc || { err "VLESS 加密参数生成失败, 需 Xray >= v25.9.5。"; return 1; }
    else
        name="ENC-XHTTP-FM-${port}"; uuid="$(auto_uuid)"; path="$(auto_path)"
    ...
    local host; host="$(server_address)"
...
add_vless_enc_finalmask() {
    require_xray || return 1
    ui_deploy_head "VLESS-Encryption-FinalMask sudoku" "需 Xray v26.3.27+ 客户端"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local name uuid spw
    if [[ $custom -eq 0 ]]; then
        ask_name "ENC-FM-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_xray_vless_enc || { err "VLESS 加密参数生成失败, 需 Xray >= v25.9.5。"; return 1; }
        ask_secret_value "FinalMask sudoku 密码" "$(gen_pass 16)"; spw="$REPLY_PASS"
    else
        name="ENC-FM-${port}"; uuid="$(auto_uuid)"; spw="$(auto_secret 16)"
        gen_xray_vless_enc || { err "VLESS 加密参数生成失败, 需 Xray >= v25.9.5。"; return 1; }
        info "  UUID/sudoku 密码 已随机生成 · VLESS 加密参数已自动生成"
    fi
...
add_vless_fullstack() {
    require_xray || return 1
    ui_deploy_head "FullStack REALITY+XHTTP+加密+FinalMask" "需 Xray v26.3.27+ 客户端"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid path sid
    if [[ $custom -eq 0 ]]; then
        ask_sni;             sni="$REPLY_SNI"
        ask_name "FullStack-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_path_value "XHTTP Path"; path="$REPLY_PATH"
        ask_short_id_value;  sid="$REPLY_SHORT_ID"
        ask_xray_vless_enc || { err "VLESS 加密参数生成失败, 需 Xray >= v25.9.5。"; return 1; }
        ...
        sni="$(auto_sni)";   name="FullStack-${port}"; uuid="$(auto_uuid)"
        path="$(auto_path)"; sid="$(auto_short_id)"
        gen_xray_vless_enc || { err "VLESS 加密参数生成失败, 需 Xray >= v25.9.5。"; return 1; }
        gen_xray_reality_keys || { err "Reality 密钥生成失败。"; return 1; }
        info "  SNI ${sni} · UUID/ShortID/Path 已随机生成 · 加密与密钥均已自动生成"
    fi
    local fm='{"tcp":[{"type":"fragment","settings":{"packets":"tlshello","length":"100-200","delay":"10-20","maxSplit":"3-6"}}]}'
    local fm_uri; fm_uri="$(jq -crn --argjson v "$fm" '$v|tojson|@uri')"
...
add_vmess_ws() {
    require_xray || return 1
    ui_deploy_head "VMess-WebSocket"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local name uuid path
    if [[ $custom -eq 0 ]]; then
        ask_name "VMess-WS-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
        ask_path_value "WebSocket Path"; path="$REPLY_PATH"
    else
        name="VMess-WS-${port}"; uuid="$(auto_uuid)"; path="$(auto_path)"
        info "  UUID/Path 均已随机生成"
    fi
...
choose_vmess_header() {
    ...
    case "$transport" in
        tcp)
            ui_sub "TCP 伪装类型"
            ui_item 1 "none" "默认 · 不伪装"
            ui_item 2 "http" "伪装成普通 HTTP 流量"
            read -rp "$(ui_prompt "请选择" "回车=1")" c
...
add_vmess_plain() {
    ...
    ui_deploy_head "${label}"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    if [[ "$transport" == "tcp" && $custom -eq 0 ]]; then
        choose_vmess_header "$transport"; header="$REPLY_HEADER"; vm_type="$header"
    else
        header="none"; vm_type="none"
    fi
    local name uuid host tag inbound vmjson link detail
    if [[ $custom -eq 0 ]]; then
        ask_name "${default_name}-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
    else
        name="${default_name}-${port}"; uuid="$(auto_uuid)"
        info "  UUID 已随机生成"
    fi
    host="$(server_address)"; tag="${tag_prefix}-${port}"
    case "$transport" in
        tcp)
            ...
            ;;
        kcp)
            stream='{"network":"kcp","security":"none","kcpSettings":{}}'
            transport_desc="mKCP UDP"
            ;;
    esac
    inbound="$(jq -n --arg tag "$tag" --argjson port "$port" --arg uuid "$uuid" --argjson stream "$stream" '{
        tag:$tag, listen:"0.0.0.0", port:$port, protocol:"vmess",
...
add_vmess_tls() {
    local transport="$1"
    require_xray || return 1; ensure_cert
    local label default_name tag_prefix path svc stream net vm_type vm_path vm_host transport_desc cert_sha
    cert_sha="$(tls_cert_sha256)" || { err "无法计算 TLS 证书 SHA256 指纹。"; return 1; }
    case "$transport" in
    ...
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid host tag inbound vmjson link detail
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "${default_name}-${port}"; name="$REPLY_NAME"
        ask_uuid_value "UUID"; uuid="$REPLY_UUID"
    ...
    host="$(server_address)"; tag="${tag_prefix}-${port}"; vm_host=""
    case "$transport" in
        ws)
            if [[ $custom -eq 0 ]]; then ask_path_value "WebSocket Path"; path="$REPLY_PATH"; else path="$(auto_path)"; fi
            vm_path="$path"; vm_host="$sni"
    ...
    vmjson="$(jq -cn --arg ps "$name" --arg add "$host" --arg port "$port" --arg id "$uuid" \
        --arg net "$net" --arg type "$vm_type" --arg host "$vm_host" --arg path "$vm_path" --arg sni "$sni" --arg pcs "$cert_sha" '{
        v:"2", ps:$ps, add:$add, port:$port, id:$id, aid:"0", scy:"auto",
        net:$net, type:$type, host:$host, path:$path, tls:"tls", sni:$sni, pcs:$pcs
    }')"
    link="vmess://$(b64_line "$vmjson")"
    detail="  地址: ${host}\n  端口: ${port}\n  UUID: ${uuid}\n  传输: ${transport_desc}\n  TLS SNI: ${sni}\n  证书: 自签名, 分享链接已包含 pcs 指纹; Mihomo 使用跳过证书验证"
    save_node "xray" "$tag" "$label" "$name" "$link" "$detail"
...
add_trojan_reality() {
    require_xray || return 1
    ui_deploy_head "Trojan-Reality"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name pw sid
    if [[ $custom -eq 0 ]]; then
        ask_sni;             sni="$REPLY_SNI"
        ask_name "Trojan-Reality-${port}"; name="$REPLY_NAME"
        ask_secret_value "Trojan 密码" "$(gen_pass 16)"; pw="$REPLY_PASS"
        ask_short_id_value;  sid="$REPLY_SHORT_ID"
        ask_xray_reality_keys || { err "Reality 密钥生成失败。"; return 1; }
    else
        sni="$(auto_sni)";   name="Trojan-Reality-${port}"
        pw="$(auto_secret 16)"; sid="$(auto_short_id)"
        gen_xray_reality_keys || { err "Reality 密钥生成失败。"; return 1; }
        info "  SNI ${sni} · 密码/ShortID 均已随机生成"
    fi
...
add_trojan_tls() {
    local transport="$1"
    require_xray || return 1; ensure_cert
    local label default_name tag_prefix path svc stream link transport_desc cert_sha
    cert_sha="$(tls_cert_sha256)" || { err "无法计算 TLS 证书 SHA256 指纹。"; return 1; }
    case "$transport" in
    ...
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name pw host tag inbound detail
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "${default_name}-${port}"; name="$REPLY_NAME"
        ask_secret_value "Trojan 密码" "$(gen_pass 16)"; pw="$REPLY_PASS"
    else
        sni="www.bing.com";  name="${default_name}-${port}"; pw="$(auto_secret 16)"
        info "  TLS SNI ${sni} · 密码 已随机生成"
    fi
    host="$(server_host)"; tag="${tag_prefix}-${port}"
    ...
    case "$transport" in
        tcp)
            ...
            link="trojan://${pw}@${host}:${port}?security=tls&sni=$(url_encode "$sni")&pcs=${cert_sha}&type=tcp#$(url_encode "$name")"
            transport_desc="TCP"
            ;;
        ws)
            if [[ $custom -eq 0 ]]; then ask_path_value "WebSocket Path"; path="$REPLY_PATH"; else path="$(auto_path)"; fi
            ...
            link="trojan://${pw}@${host}:${port}?security=tls&sni=$(url_encode "$sni")&pcs=${cert_sha}&type=ws&host=$(url_encode "$sni")&path=$(url_encode "$path")#$(url_encode "$name")"
            transport_desc="WebSocket  Path: ${path}"
            ;;
        grpc)
            if [[ $custom -eq 0 ]]; then ask_service_value "gRPC serviceName"; svc="$REPLY_SERVICE"; else svc="$(auto_service)"; fi
            ...
            link="trojan://${pw}@${host}:${port}?security=tls&sni=$(url_encode "$sni")&pcs=${cert_sha}&type=grpc&serviceName=$(url_encode "$svc")&mode=gun#$(url_encode "$name")"
            transport_desc="gRPC serviceName: ${svc}"
            ;;
    esac
...
choose_ss_method() {
    ui_sub "Shadowsocks 加密方式"
    ui_item 1 "aes-128-gcm"       "速度优先"
    ui_item 2 "aes-256-gcm"       "默认 · 均衡"
    ui_item 3 "chacha20-poly1305" "移动端/无 AES 加速更快"
    local c; read -rp "$(ui_prompt "请选择" "回车=2")" c
    case "$c" in
...
choose_ss2022_method() {
    ui_sub "Shadowsocks 2022 加密方式"
    ui_item 1 "2022-blake3-aes-128-gcm"        "速度优先"
    ui_item 2 "2022-blake3-aes-256-gcm"        "默认 · 均衡"
    ui_item 3 "2022-blake3-chacha20-poly1305"  "移动端更友好"
    local c; read -rp "$(ui_prompt "请选择" "回车=2")" c
    case "$c" in
...
choose_ss_plugin() {
    ...
    ui_sub "Shadowsocks 插件 / 增强模式"
    ui_item 1 "不启用"       "默认 · 纯 SS"
    ui_item 2 "HTTP obfs"    "simple-obfs 伪装"
    [[ "$allow_shadowtls" == "true" ]] && ui_item 3 "ShadowTLS v3" "再套一层 TLS 伪装"
    read -rp "$(ui_prompt "请选择" "回车=1")" c
    case "$c" in
        2)
...
add_shadowsocks() {
    require_xray || return 1
    ui_deploy_head "Shadowsocks" "经典 AEAD"
    ask_port "监听端口"; local port="$REPLY_PORT"
    choose_ss_method; local method="$REPLY_METHOD"
    local name pw
    ask_customize && {
        ask_name "SS-${port}"; name="$REPLY_NAME"
        ask_secret_value "Shadowsocks 密码" "$(gen_pass 16)"; pw="$REPLY_PASS"
    } || {
        name="SS-${port}"; pw="$(auto_secret 16)"
        info "  密码已随机生成"
    }
    local tag; tag="ss-${port}"
...
add_ss2022() {
    ui_deploy_head "Shadowsocks 2022"
    ...
    local name pw
    ask_customize && {
        ask_name "SS2022-${port}"; name="$REPLY_NAME"
        ask_ss2022_psk "$method"; pw="$REPLY_PASS"
    } || {
        name="SS2022-${port}"
        pw="$(gen_b64 "$(ss2022_key_bytes "$method")")"
        info "  PSK 已随机生成"
    }
    local tag; tag="ss2022-${port}"
...
shadowsocks_menu() {
    ui_head "Shadowsocks" "$C_ACC2"
    ui_item 1 "Shadowsocks AEAD" "兼容性最好 · 支持 obfs 插件" "$C_ACC2"
    ui_item 2 "Shadowsocks 2022" "新一代 · 抗重放更强"         "$C_ACC2"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-2")" c
    case "$c" in
        1) add_shadowsocks ;;
        2) add_ss2022 ;;
        *) return 0 ;;
...
add_hysteria2() {
    ...
    ui_deploy_head "Hysteria2" "基于 QUIC, 弱网强"
    ask_port "监听端口 UDP"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name pw
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "Hysteria2-${port}"; name="$REPLY_NAME"
        ask_secret_value "Hysteria2 密码" "$(gen_pass 16)"; pw="$REPLY_PASS"
    else
        sni="www.bing.com";  name="Hysteria2-${port}"; pw="$(auto_secret 16)"
        info "  TLS SNI ${sni} · 密码 已随机生成"
    fi
...
add_tuic() {
    require_singbox || return 1; ensure_cert
    ui_deploy_head "TUIC v5" "基于 QUIC"
    ask_port "监听端口 UDP"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name uuid pw
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "TUIC-${port}"; name="$REPLY_NAME"
        ask_uuid_value "TUIC UUID"; uuid="$REPLY_UUID"
        ask_secret_value "TUIC 密码" "$(gen_pass 16)"; pw="$REPLY_PASS"
    else
        sni="www.bing.com";  name="TUIC-${port}"
        uuid="$(auto_uuid)"; pw="$(auto_secret 16)"
        info "  TLS SNI ${sni} · UUID/密码 已随机生成"
    fi
    local tag="tuic-${port}"
...
add_anytls() {
    require_singbox || return 1; ensure_cert
    ui_deploy_head "AnyTLS" "新型抗指纹协议, 需 sing-box 1.12+ 客户端"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name pw
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "AnyTLS-${port}"; name="$REPLY_NAME"
        ask_secret_value "AnyTLS 密码" "$(gen_pass 16)"; pw="$REPLY_PASS"
    else
        sni="www.bing.com";  name="AnyTLS-${port}"; pw="$(auto_secret 16)"
        info "  TLS SNI ${sni} · 密码 已随机生成"
    fi
...
add_shadowtls() {
    ...
    ui_deploy_head "ShadowTLS v3"
    ask_port "对外监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name stls_pw ss_pw inner_port method
    if [[ $custom -eq 0 ]]; then
        ask_sni;             sni="$REPLY_SNI"
        ask_name "ShadowTLS-${port}"; name="$REPLY_NAME"
        ask_secret_value "ShadowTLS 密码" "$(gen_pass 16)"; stls_pw="$REPLY_PASS"
    else
        sni="$(auto_sni)";   name="ShadowTLS-${port}"; stls_pw="$(auto_secret 16)"
        info "  SNI ${sni} · ShadowTLS 密码已随机生成"
    fi
    ensure_time_sync || return 1
    ...
    if [[ $custom -eq 0 ]]; then
        ...
        ss_pw="$(gen_b64 "$(ss2022_key_bytes "$method")")"
    fi
...
add_socks() {
    require_xray || return 1
    ui_deploy_head "SOCKS5" "明文, 建议仅内网或中转使用"
    ask_port "监听端口"; local port="$REPLY_PORT"
    local name user pass
    ask_customize && {
        ask_name "SOCKS5-${port}"; name="$REPLY_NAME"
        ask_username_value "SOCKS5 用户名" "user$(openssl rand -hex 2)"; user="$REPLY_USER"
        ask_secret_value "SOCKS5 密码" "$(gen_pass 8)"; pass="$REPLY_PASS"
    } || {
        name="SOCKS5-${port}"; user="$(auto_username user)"; pass="$(auto_secret 8)"
        info "  用户名/密码 已随机生成"
    }
...
add_http_proxy() {
    require_xray || return 1
    ui_deploy_head "HTTP Proxy" "明文, 建议仅内网或中转使用"
    ask_port "监听端口"; local port="$REPLY_PORT"
    local name user pass
    ask_customize && {
        ask_name "HTTP-${port}"; name="$REPLY_NAME"
        ask_username_value "HTTP 用户名" "http$(openssl rand -hex 2)"; user="$REPLY_USER"
        ask_secret_value "HTTP 密码" "$(gen_pass 8)"; pass="$REPLY_PASS"
    } || {
        name="HTTP-${port}"; user="$(auto_username http)"; pass="$(auto_secret 8)"
        info "  用户名/密码 已随机生成"
    }
    local tag="http-${port}"
    local inbound
...
add_naive() {
    require_singbox || return 1; ensure_cert
    ui_deploy_head "NaïveProxy"
    ask_port "监听端口"; local port="$REPLY_PORT"
    ask_customize; local custom=$?
    local sni name user pass
    if [[ $custom -eq 0 ]]; then
        ask_tls_sni;          sni="$REPLY_SNI"
        ask_name "Naive-${port}"; name="$REPLY_NAME"
        ask_username_value "NaïveProxy 用户名" "naive$(openssl rand -hex 3)"; user="$REPLY_USER"
        ask_secret_value "NaïveProxy 密码" "$(gen_pass 12)"; pass="$REPLY_PASS"
    else
        sni="www.bing.com";  name="Naive-${port}"
        user="$(printf 'naive%s' "$(openssl rand -hex 3)")"; pass="$(auto_secret 12)"
        info "  TLS SNI ${sni} · 用户名/密码 已随机生成"
    fi
...
install_shadowtls_binary() {
    ...
    tag="$(gh_latest_tag "$SHADOWTLS_REPO")"
    [[ -z "$tag" ]] && { err "无法获取 ShadowTLS 版本 (GitHub 访问失败)。"; return 1; }
...
snell_mihomo_yaml() {
    ...
    host="${host#[}"; host="${host%]}"
    printf '  - name: %s\n    type: snell\n    server: %s\n    port: %s\n    psk: %s\n    version: %s\n    tfo: true\n' \
        "$(yaml_quote "$name")" "$(yaml_quote "$host")" "$port" "$(yaml_quote "$psk")" "$ver"
...
install_snell_version() {
    local major="$1" ver="$2"
    ui_deploy_head "Snell v${major}" "Surge 官方协议"
...
add_snell_shadowtls_version() {
    ...
    ui_deploy_head "Snell v${major} + ShadowTLS v3"
...
EOF
    ...
    local host; host="$(server_host)"
    fw_allow_pairs "tcp ${port}"
    local link="${name} = snell, ${host}, ${port}, psk=${psk}, version=${major}, reuse=true, tfo=true, shadow-tls-password=${stls_pw}, shadow-tls-sni=${sni}, shadow-tls-version=3"
    ...
    echo -e "${BOLD}连接信息:${NC}"
    echo -e "${DIM}$(pm_format_detail_text "$detail")${NC}"
    hr
}

snell_v6_managed_exists() {
    [[ -f "$STATE" ]] && jq -e '.nodes | to_entries[] | select(.value.core == "snell-v6-managed")' "$STATE" >/dev/null 2>&1
}
snell_v6_service() { printf 'snellv6-%s' "$1"; }
snell_v6_conf() { printf '%s/snell-%s.conf' "$SNELL_V6_DIR" "$1"; }

install_snell_v6_binary() {
    local sha url tmp actual
    case "$ARCH" in
        amd64) sha="$SNELL_V6_SHA256_AMD64" ;;
        arm64) sha="$SNELL_V6_SHA256_ARM64" ;;
        *) err "Snell v6 RC 仅支持 amd64 / arm64, 当前: ${ARCH}。"; return 1 ;;
    esac
    if [[ -x "$SNELL_V6_BIN" && -f "$SNELL_V6_DIR/.binary-version" ]] \
        && [[ "$(cat "$SNELL_V6_DIR/.binary-version" 2>/dev/null)" == "$SNELL_V6_VERSION" ]]; then
        return 0
    fi
    command -v unzip >/dev/null 2>&1 || pkg_install unzip
    command -v unzip >/dev/null 2>&1 || { err "缺少 unzip, 无法安装 Snell v6。"; return 1; }
    command -v sha256sum >/dev/null 2>&1 || { err "缺少 sha256sum, 无法校验 Snell v6 安装包。"; return 1; }
    url="https://dl.nssurge.com/snell/snell-server-v${SNELL_V6_VERSION}-linux-${SNELL_ARCH}.zip"
    tmp="$(mktemp -d)" || { err "创建临时目录失败。"; return 1; }
    info "下载 Snell v${SNELL_V6_VERSION} (${SNELL_ARCH})..."
    if ! curl -fL --retry 2 --max-time 120 "$url" -o "$tmp/snell.zip"; then
        err "下载失败: $url"; rm -rf "$tmp"; return 1
    fi
    actual="$(sha256sum "$tmp/snell.zip" | awk '{print $1}')"
    if [[ "$actual" != "$sha" ]]; then
        err "Snell v6 安装包 SHA256 校验失败, 已停止安装。"
        rm -rf "$tmp"; return 1
    fi
    unzip -t "$tmp/snell.zip" >/dev/null 2>&1 \
        && unzip -oq "$tmp/snell.zip" -d "$tmp" \
        || { err "Snell v6 压缩包损坏或解压失败。"; rm -rf "$tmp"; return 1; }
    [[ -f "$tmp/snell-server" ]] || { err "压缩包中未找到 snell-server。"; rm -rf "$tmp"; return 1; }
    install -m 755 "$tmp/snell-server" "$SNELL_V6_BIN" \
        || { err "安装 Snell v6 二进制失败。"; rm -rf "$tmp"; return 1; }
    mkdir -p "$SNELL_V6_DIR"
    printf '%s\n' "$SNELL_V6_VERSION" > "$SNELL_V6_DIR/.binary-version"
    chmod 600 "$SNELL_V6_DIR/.binary-version"
    rm -rf "$tmp"
    ok "Snell v${SNELL_V6_VERSION} 内核安装完成。"
}

install_snell_v6() {
    ui_deploy_head "Snell v6 RC" "Surge 预发布协议"
    warn "Snell v6 仍是预发布版本; 客户端需 Surge Mac Beta 或 iOS TestFlight。"
    pm_service_supported || { err "Snell v6 需要 systemd 或 OpenRC。"; return 1; }
    install_snell_v6_binary || return 1

    ask_port "Snell v6 监听端口"; local port="$REPLY_PORT"
    ask_name "Snell-v6-${port}"; local name="$REPLY_NAME"
    ask_secret_value "Snell v6 PSK" "$(gen_pass 16)"; local psk="$REPLY_PASS"

    local c listen_mode listen_addr ip_suffix mode dns_pref="default"
    ui_sub "监听模式"
    ui_item 1 "仅 IPv4" "默认"
    ui_item 2 "仅 IPv6"
    ui_item 3 "IPv4 + IPv6 双栈"
    read -rp "$(ui_prompt "选择" "回车=1")" c
    case "$c" in
        2) listen_mode="ipv6"; listen_addr="[::]:${port}"; ip_suffix=", ip-version=v6-only" ;;
        3) listen_mode="dual"; listen_addr="0.0.0.0:${port},[::]:${port}"; ip_suffix="" ;;
        *) listen_mode="ipv4"; listen_addr="0.0.0.0:${port}"; ip_suffix=", ip-version=v4-only" ;;
    esac

    ui_sub "Snell v6 模式"
    ui_item 1 "default" "混淆 + AES, 推荐"
    ui_item 2 "unshaped" "仅 AES, 速度约快 10%"
    ui_item 3 "unsafe-raw" "关闭混淆与加密, 不推荐"
    read -rp "$(ui_prompt "选择" "回车=1")" c
    case "$c" in
        2) mode="unshaped" ;;
        3)
            confirm "unsafe-raw 不提供流量混淆与加密, 确认继续?" "n" || return 0
            mode="unsafe-raw"
            ;;
        *) mode="default" ;;
    esac

    local tag="snell-v6-${port}" conf svc svc_file
    conf="$(snell_v6_conf "$port")"; svc="$(snell_v6_service "$port")"
    svc_file="/etc/systemd/system/${svc}.service"
    mkdir -p "$SNELL_V6_DIR"
    cat > "$conf" <<EOF
[snell-server]
...
EOF
    ...
    if pm_has_systemd; then
        cat > "$svc_file" <<EOF
[Unit]
Description=Snell v6 Proxy Server (${port})
...
EOF
    ...
    pm_write_openrc_service "$svc" "Snell v6 Proxy Server (${port})" "$SNELL_V6_BIN" "-c ${conf}" $'respawn\nrespawn_delay 10'
    systemctl daemon-reload
    if ! systemctl enable --now "$svc" >/dev/null 2>&1; then
        err "Snell v6 服务启动失败。"
        journalctl -u "$svc" -n 15 --no-pager 2>/dev/null | sed 's/^/    /'
        delete_snell_v6_runtime "$port"
        return 1
    fi
    sleep 2
    if ! systemctl is-active --quiet "$svc" 2>/dev/null; then
        err "Snell v6 启动后异常退出:"
    ...
    configured_host="$(jq -r '.meta.host // empty' "$STATE" 2>/dev/null)"
    if [[ "$listen_mode" == "ipv6" ]]; then
        if [[ -n "$configured_host" ]]; then
            host="$(server_host)"
        else
            detect_ip
            [[ -n "$PUBLIC_IPV6" ]] || {
                err "未检测到公网 IPv6, 不能为 IPv6-only Snell v6 生成可用节点。"
                delete_snell_v6_runtime "$port" >/dev/null 2>&1 || true
                return 1
            }
            host="$PUBLIC_IPV6"
            host="${host#[}"; host="${host%]}"
        ...
        if [[ -n "$configured_host" ]]; then
            ...
            detect_ip
            [[ -n "$PUBLIC_IPV4" ]] || {
                err "未检测到公网 IPv4, 不能为 IPv4-only Snell v6 生成可用节点。"
                delete_snell_v6_runtime "$port" >/dev/null 2>&1 || true
                return 1
            }
            host="$PUBLIC_IPV4"
    ...
    local link surge detail extra
    link="$(snell_uri "$name" "$host" "$port" "$psk" "6")"
    surge="${name} = snell, ${host}, ${port}, psk=${psk}, version=6, reuse=true${ip_suffix}"
    detail="  地址: ${host}\n  端口: ${port}\n  PSK : ${psk}\n  版本: 6 (${SNELL_V6_VERSION})\n  监听: ${listen_mode}\n  模式: ${mode}\n  服务: ${svc}\n  配置: ${conf}\n  Surge: ${surge}"
    extra="$(jq -cn --arg psk "$psk" --arg listen "$listen_mode" --arg mode "$mode" --arg surge "$surge" \
        --argjson port "$port" '{psk:$psk, port:$port, listen:$listen, mode:$mode, surge:$surge}')"
    fw_allow_pairs "tcp ${port}"
    save_node "snell-v6-managed" "$tag" "Snell v6 RC" "$name" "$link" "$detail" "$extra"
    if ! jq -e --arg tag "$tag" '.nodes[$tag].core == "snell-v6-managed"' "$STATE" >/dev/null 2>&1; then
        fw_deny_pairs "tcp ${port}"
        delete_snell_v6_runtime "$port"
        err "保存 Snell v6 节点状态失败, 已回滚本次安装。"
        return 1
    fi

    echo; hr; ok "  ✅ Snell v6 RC 部署成功!"; hr
    echo -e "${BOLD}Surge Beta / TestFlight 配置:${NC}"
    echo -e "${GREEN}${surge}${NC}"
    echo -e "${DIM}App Store 正式版 Surge 目前无法连接 Snell v6。${NC}"
    hr
}
...
install_snell() {
    ui_head "Snell" "$C_ACC2"
    ui_item 1 "Snell v4"
    ui_item 2 "Snell v5" "默认"
    ui_item 3 "Snell v6 RC" "需 Surge Beta / TestFlight"
    ui_item_back "返回"
    local c major ver; read -rp "$(ui_prompt "请选择版本" "回车=2")" c
    case "$c" in
        0) return 0 ;;
        1) major="4"; ver="v4.1.1" ;;
        2|"") major="5"; ver="v5.0.1" ;;
        3) install_snell_v6; return $? ;;
        *) err "无效选项。"; return 1 ;;
    esac
...
uninstall_snell() {
    snell_installed || snell_managed_exists || snell_v6_managed_exists || \
        jq -e '.nodes | to_entries[] | select(.value.core == "snell-shadowtls")' "$STATE" >/dev/null 2>&1 || \
        shadowtls_installed || [[ -x "$SNELL_V6_BIN" ]] \
        || { warn "Snell 未安装。"; return 0; }
    [[ "$1" == "quiet" ]] || confirm "确认卸载全部 Snell v4/v5/v6 节点?" "n" || return 0
    if [[ -f "$STATE" ]]; then
        local t
        while IFS= read -r t; do
        ...
        while IFS= read -r t; do
            [[ -n "$t" ]] && delete_snell_v6_managed_node "$t" >/dev/null 2>&1 || true
        done <<< "$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-v6-managed") | .key' "$STATE" 2>/dev/null)"
    fi
    systemctl stop "$SNELL_SERVICE" 2>/dev/null; systemctl disable "$SNELL_SERVICE" 2>/dev/null
    rm -f "$SNELL_SERVICE_FILE" "$SNELL_BIN" "$SNELL_V6_BIN" "$SNELL_STATE" "$SNELL_CONF" \
        /usr/local/bin/snell-server-v4 /usr/local/bin/snell-server-v5
    rm -rf "$SNELL_V6_DIR"
    pm_remove_openrc_service "$SNELL_SERVICE"
    rmdir "$(dirname "$SNELL_CONF")" 2>/dev/null || true
    systemctl daemon-reload 2>/dev/null || true
    ok "Snell v4/v5/v6 已卸载。"
}

# ===========================================================================
#  十一之二、Mieru / mita (独立内核, enfein/mieru)
# ===========================================================================
# 多节点共存策略 (参考 ike-sh/mieru-OneClick):
#   * mita 服务端只有一份全局配置, 多节点 = 多用户 + 多端口绑定, 一节点一端口。
#   * 所有节点存放在 state.json (core=mieru-managed), 每次增删改都由
#     mieru_apply_from_state 重新生成完整配置后 apply, 避免互相覆盖。
#   * 双协议 BOTH 占用 port 与 port+1; 端口/用户名冲突在添加时即拦截。
mita_bin() { command -v mita 2>/dev/null || { [[ -x /usr/local/bin/mita ]] && echo /usr/local/bin/mita; } || { [[ -x /usr/bin/mita ]] && echo /usr/bin/mita; }; }
...
# mita 守护进程需要写 /etc/mita/server.conf.pb; deb/rpm 会建 mita 用户, tar.gz 由脚本创建。
mieru_ensure_account() {
    id mita >/dev/null 2>&1 && return 0
    if command -v useradd >/dev/null 2>&1; then
        groupadd --system mita >/dev/null 2>&1 || true
        useradd --system -g mita -s /sbin/nologin -d /var/lib/mita mita >/dev/null 2>&1 || true
    elif command -v adduser >/dev/null 2>&1; then
        addgroup -S mita >/dev/null 2>&1 || true
        adduser -S -G mita -s /sbin/nologin -h /var/lib/mita mita >/dev/null 2>&1 || true
    fi
}

mieru_harden_permissions() {
    mkdir -p /etc/mita /var/lib/mita /var/run/mita 2>/dev/null || true
    if id mita >/dev/null 2>&1; then
        chown -R mita:mita /etc/mita /var/lib/mita /var/run/mita 2>/dev/null || true
        chmod 0750 /etc/mita 2>/dev/null || true
    else
        chmod 0750 /etc/mita 2>/dev/null || true
    fi
    chmod 0755 /var/lib/mita /var/run/mita 2>/dev/null || true
}
...
# 共享 mita 架构下只快照脚本持有的节点状态与全局选项；服务配置由恢复后重新 apply。
mieru_tx_snapshot() {
    local snapshot
    snapshot="$(mktemp -d "${TMPDIR:-/tmp}/proxy-manager-mieru-tx.XXXXXX")" || return 1
    : > "$snapshot/.proxy-manager-mieru-tx"
    if [[ -f "$STATE" ]]; then
        cp -p "$STATE" "$snapshot/state.json" || { rm -rf -- "$snapshot"; return 1; }
    else
        : > "$snapshot/state.absent"
    fi
    if [[ -f "$MIERU_STATE" ]]; then
        cp -p "$MIERU_STATE" "$snapshot/mieru.env" || { rm -rf -- "$snapshot"; return 1; }
    else
        : > "$snapshot/options.absent"
    fi
    printf '%s' "$snapshot"
}
...
mieru_tx_restore() {
    local snapshot="$1"
    [[ -f "$snapshot/.proxy-manager-mieru-tx" ]] || return 1
    mkdir -p "$STATE_DIR" || return 1
    if [[ -f "$snapshot/state.json" ]]; then
        install -m 600 "$snapshot/state.json" "$STATE" || return 1
    elif [[ -f "$snapshot/state.absent" ]]; then
        rm -f "$STATE"
    else
        return 1
    fi
    if [[ -f "$snapshot/mieru.env" ]]; then
        install -m 600 "$snapshot/mieru.env" "$MIERU_STATE" || return 1
    elif [[ -f "$snapshot/options.absent" ]]; then
        rm -f "$MIERU_STATE"
    else
        return 1
    fi
...
mieru_tx_rollback() {
    local snapshot="$1"
    if ! mieru_tx_restore "$snapshot"; then
        err "Mieru 状态恢复失败, 请勿继续修改节点。旧状态快照已保留: ${snapshot}"
        return 1
    fi
    if ! mieru_apply_from_state >/dev/null 2>&1; then
        err "旧 Mieru 配置已恢复到状态文件, 但重新应用失败, 请检查 mita 服务。"
        mieru_tx_commit "$snapshot" 2>/dev/null || true
        return 1
    fi
    mieru_tx_commit "$snapshot"
    warn "Mieru 修改失败, 已恢复修改前的节点配置。"
}

# trafficPattern 自 mita v3.28.0 起支持; lowEntropy 自 v3.35.0 起支持。
mieru_version_at_least() {
    local required="$1" v; v="$(mieru_version)"
    [[ "$v" =~ ^v?[0-9] ]] || return 1
    v="${v#v}"
    [[ "$(printf '%s\n%s' "$required" "$v" | sort -V 2>/dev/null | head -n1)" == "$required" ]]
}
mieru_supports_traffic_pattern() { mieru_version_at_least "3.28.0"; }
mieru_supports_low_entropy() { mieru_version_at_least "3.35.0"; }

mieru_load_opts() {
    local had_state=0
    [[ -f "$MIERU_STATE" ]] && had_state=1
    MIERU_PROFILE="balanced"; MIERU_CHANNEL="stable"; MIERU_MTU="1400"
    MIERU_TRAFFIC="conservative"; MIERU_TRAFFIC_SEED=""
    MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF"
    MIERU_MULTIPLEXING="MULTIPLEXING_OFF"; MIERU_HANDSHAKE="HANDSHAKE_NO_WAIT"
    if (( had_state )); then
        # shellcheck disable=SC1090
        source "$MIERU_STATE" 2>/dev/null || true
        # 旧脚本的分享链接固定使用 LOW / STANDARD; 迁移时保留实际行为。
        if ! grep -q '^MIERU_PROFILE=' "$MIERU_STATE" 2>/dev/null; then
            MIERU_PROFILE="custom"
            grep -q '^MIERU_MULTIPLEXING=' "$MIERU_STATE" 2>/dev/null || MIERU_MULTIPLEXING="MULTIPLEXING_LOW"
            grep -q '^MIERU_HANDSHAKE=' "$MIERU_STATE" 2>/dev/null || MIERU_HANDSHAKE="HANDSHAKE_STANDARD"
        fi
        grep -q '^MIERU_CHANNEL=' "$MIERU_STATE" 2>/dev/null || MIERU_CHANNEL="latest"
    fi
    case "$MIERU_PROFILE" in iplc|balanced|stealth|custom) ;; *) MIERU_PROFILE="custom" ;; esac
    case "$MIERU_CHANNEL" in stable|latest) ;; *) MIERU_CHANNEL="latest" ;; esac
    case "$MIERU_TRAFFIC" in off|conservative|aggressive) ;; *) MIERU_TRAFFIC="conservative" ;; esac
    case "$MIERU_LOW_ENTROPY" in
        LOW_ENTROPY_MODE_OFF|LOW_ENTROPY_MODE_56|LOW_ENTROPY_MODE_48|LOW_ENTROPY_MODE_40|LOW_ENTROPY_MODE_32) ;;
        *) MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF" ;;
    esac
    case "$MIERU_MULTIPLEXING" in
        MULTIPLEXING_OFF|MULTIPLEXING_LOW|MULTIPLEXING_MIDDLE|MULTIPLEXING_HIGH) ;;
        *) MIERU_MULTIPLEXING="MULTIPLEXING_OFF" ;;
    esac
    case "$MIERU_HANDSHAKE" in HANDSHAKE_NO_WAIT|HANDSHAKE_STANDARD) ;; *) MIERU_HANDSHAKE="HANDSHAKE_NO_WAIT" ;; esac
    [[ "$MIERU_MTU" =~ ^[0-9]+$ ]] || MIERU_MTU="1400"
    [[ "$MIERU_TRAFFIC" != "off" ]] || MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF"
}

mieru_save_opts() {
    mkdir -p "$STATE_DIR"
    local legacy=""
    [[ -f "$MIERU_STATE" ]] && legacy="$(grep -E '^MIERU_(USER|PASS|PORT|PROTO)=' "$MIERU_STATE" 2>/dev/null || true)"
    {
        printf "MIERU_PROFILE='%s'\n" "${MIERU_PROFILE:-custom}"
        printf "MIERU_CHANNEL='%s'\n" "${MIERU_CHANNEL:-stable}"
        printf "MIERU_MTU=%s\n" "${MIERU_MTU:-1400}"
        printf "MIERU_TRAFFIC='%s'\n" "${MIERU_TRAFFIC:-conservative}"
        printf "MIERU_LOW_ENTROPY='%s'\n" "${MIERU_LOW_ENTROPY:-LOW_ENTROPY_MODE_OFF}"
        printf "MIERU_MULTIPLEXING='%s'\n" "${MIERU_MULTIPLEXING:-MULTIPLEXING_OFF}"
        printf "MIERU_HANDSHAKE='%s'\n" "${MIERU_HANDSHAKE:-HANDSHAKE_NO_WAIT}"
        [[ -n "${MIERU_TRAFFIC_SEED:-}" ]] && printf "MIERU_TRAFFIC_SEED=%s\n" "$MIERU_TRAFFIC_SEED"
        [[ -n "$legacy" ]] && printf '%s\n' "$legacy"
    } > "$MIERU_STATE"
...
mieru_apply_profile() {
    MIERU_PROFILE="$1"; MIERU_MTU="1400"
    MIERU_MULTIPLEXING="MULTIPLEXING_OFF"; MIERU_HANDSHAKE="HANDSHAKE_NO_WAIT"
    MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF"
    case "$MIERU_PROFILE" in
        iplc) MIERU_TRAFFIC="off" ;;
        stealth) MIERU_TRAFFIC="aggressive" ;;
        *) MIERU_PROFILE="balanced"; MIERU_TRAFFIC="conservative" ;;
    esac
}

ask_mieru_client_modes() {
    local c def
    case "$MIERU_MULTIPLEXING" in MULTIPLEXING_LOW) def=2 ;; MULTIPLEXING_MIDDLE) def=3 ;; MULTIPLEXING_HIGH) def=4 ;; *) def=1 ;; esac
    ui_sub "多路复用"
    ui_item 1 "关闭" "推荐"
    ui_item 2 "低"
    ui_item 3 "中"
    ui_item 4 "高"
    read -rp "$(ui_prompt "选择" "回车=${def}")" c
    case "${c:-$def}" in
        2) MIERU_MULTIPLEXING="MULTIPLEXING_LOW" ;;
        3) MIERU_MULTIPLEXING="MULTIPLEXING_MIDDLE" ;;
        4) MIERU_MULTIPLEXING="MULTIPLEXING_HIGH" ;;
        *) MIERU_MULTIPLEXING="MULTIPLEXING_OFF" ;;
    esac
    def=1; [[ "$MIERU_HANDSHAKE" == "HANDSHAKE_STANDARD" ]] && def=2
    ui_sub "握手模式"
    ui_item 1 "NO_WAIT" "推荐, 0-RTT"
    ui_item 2 "STANDARD"
    read -rp "$(ui_prompt "选择" "回车=${def}")" c
    [[ "${c:-$def}" == "2" ]] && MIERU_HANDSHAKE="HANDSHAKE_STANDARD" || MIERU_HANDSHAKE="HANDSHAKE_NO_WAIT"
}

ask_mieru_low_entropy() {
    MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF"
    [[ "$MIERU_TRAFFIC" != "off" ]] || return 0
    confirm "是否启用 Low Entropy 低熵模式? 会增加流量与 CPU 开销" "n" || return 0
    ui_sub "Low Entropy 强度 (数字越小, 伪装越强, 开销越大)"
    ui_item 1 "56" "约增加 15% 流量"
    ui_item 2 "48" "约增加 34% 流量"
    ui_item 3 "40" "约增加 60% 流量"
    ui_item 4 "32" "约增加 100% 流量"
    local c; read -rp "$(ui_prompt "选择" "回车=1")" c
    case "$c" in
        2) MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_48" ;;
        3) MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_40" ;;
        4) MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_32" ;;
        *) MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_56" ;;
    esac
    warn "Low Entropy 需要 mita >= 3.35.0 和支持该能力的客户端; 不确定时请保持关闭。"
}

ask_mieru_traffic_pattern() {
    local cur="${MIERU_TRAFFIC:-conservative}" def c
    case "$cur" in aggressive) def=3 ;; off) def=1 ;; *) def=2 ;; esac
    ui_sub "流量伪装 (mita v3.28.0+ 支持)"
    ui_item 1 "关闭"
    ui_item 2 "保守" "推荐"
    ui_item 3 "激进" "更强流量特征处理, 开销更大"
    read -rp "$(ui_prompt "选择" "回车=${def}")" c
    case "${c:-$def}" in
        1) MIERU_TRAFFIC="off" ;;
        3) MIERU_TRAFFIC="aggressive" ;;
        *) MIERU_TRAFFIC="conservative" ;;
    esac
    [[ "$MIERU_TRAFFIC" != "off" ]] || MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF"
}
...
ask_mieru_profile() {
    mieru_load_opts
    local c def=2
    case "$MIERU_PROFILE" in iplc) def=1 ;; stealth) def=3 ;; custom) def=4 ;; esac
    ui_sub "配置 Profile (同一 mita 服务下全部节点共用)"
    ui_item 1 "IPLC / 专线性能" "OFF + NO_WAIT, 不启用流量伪装"
    ui_item 2 "普通公网" "推荐, 保守流量伪装"
    ui_item 3 "强化伪装" "激进流量伪装, Low Entropy 默认关闭"
    ui_item 4 "高级自定义"
    read -rp "$(ui_prompt "选择" "回车=${def}")" c
    case "${c:-$def}" in
        1) mieru_apply_profile iplc ;;
        3) mieru_apply_profile stealth ;;
        4)
            MIERU_PROFILE="custom"
            ask_mieru_traffic_pattern
            ask_mieru_low_entropy
            ask_mieru_client_modes
            ;;
        *) mieru_apply_profile balanced ;;
    esac
    if [[ "$MIERU_TRAFFIC" != "off" ]] && mieru_installed && ! mieru_supports_traffic_pattern; then
        warn "当前 mita 不支持 trafficPattern (需 >= 3.28.0), 应用时将忽略。"
    fi
    if [[ "$MIERU_LOW_ENTROPY" != "LOW_ENTROPY_MODE_OFF" ]] && mieru_installed && ! mieru_supports_low_entropy; then
        warn "当前 mita 不支持 Low Entropy (需 >= 3.35.0), 应用时将关闭。"
        MIERU_LOW_ENTROPY="LOW_ENTROPY_MODE_OFF"
    fi
    mieru_save_opts
}
...
mieru_traffic_seed() {
    if ! [[ "${MIERU_TRAFFIC_SEED:-}" =~ ^[0-9]+$ ]]; then
        MIERU_TRAFFIC_SEED="$(( ( $(od -An -N4 -tu4 /dev/urandom 2>/dev/null | tr -dc '0-9' || echo $RANDOM) ) % 2147483647 ))"
        [[ "$MIERU_TRAFFIC_SEED" =~ ^[0-9]+$ ]] || MIERU_TRAFFIC_SEED="$RANDOM"
        mieru_save_opts
    fi
    printf '%s' "$MIERU_TRAFFIC_SEED"
}

# 输出 trafficPattern JSON; 关闭或内核过旧时输出 null。
mieru_traffic_pattern_json() {
    mieru_load_opts
    local level="$MIERU_TRAFFIC" seed low="$MIERU_LOW_ENTROPY"
    [[ "$level" == "off" ]] && { printf 'null'; return 0; }
    mieru_supports_traffic_pattern || { printf 'null'; return 0; }
    [[ "$low" == "LOW_ENTROPY_MODE_OFF" ]] || mieru_supports_low_entropy || low="LOW_ENTROPY_MODE_OFF"
    seed="$(mieru_traffic_seed)"
    jq -cn --arg level "$level" --arg low "$low" --argjson seed "$seed" '
        {seed:$seed, unlockAll:false} +
        (if $low == "LOW_ENTROPY_MODE_OFF" then {} else {lowEntropy:{mode:$low}} end) +
...
mieru_export_traffic_pattern() {
    [[ "${MIERU_TRAFFIC:-off}" != "off" ]] || return 0
    local bin value; bin="$(mita_bin)"
    [[ -n "$bin" ]] || return 0
    value="$("$bin" export traffic-pattern 2>/dev/null | tr -d '\r\n' || true)"
    [[ -n "$value" && "$value" != *[!A-Za-z0-9+/=]* ]] && printf '%s' "$value"
}

mieru_normalize_display_host() {
    local host="$1"
    host="${host#[}"; host="${host%]}"; host="${host%.}"
    printf '%s' "$host"
}

mieru_valid_display_host() {
    local host a b c d extra segment rest label left right count=0
    local -a segments=()
    host="$(mieru_normalize_display_host "$1")"
    [[ -n "$host" && ${#host} -le 253 && "$host" != *[[:space:]]* ]] || return 1
    if [[ "$host" == *:* ]]; then
        ...
        if [[ "$host" == *::* ]]; then
            ...
            [[ "$right" != *::* ]] || return 1
            if [[ -n "$left" ]]; then
                IFS=: read -r -a segments <<< "$left"
                for segment in "${segments[@]}"; do [[ "$segment" =~ ^[0-9A-Fa-f]{1,4}$ ]] || return 1; done
                count=${#segments[@]}
            fi
            if [[ -n "$right" ]]; then
                IFS=: read -r -a segments <<< "$right"
                for segment in "${segments[@]}"; do [[ "$segment" =~ ^[0-9A-Fa-f]{1,4}$ ]] || return 1; done
                count=$((count + ${#segments[@]}))
            fi
            (( count < 8 )) || return 1
        else
            IFS=: read -r -a segments <<< "$host"
            (( ${#segments[@]} == 8 )) || return 1
            for segment in "${segments[@]}"; do [[ "$segment" =~ ^[0-9A-Fa-f]{1,4}$ ]] || return 1; done
        fi
        return 0
    ...
    if [[ "$host" =~ ^[0-9.]+$ ]]; then
        ...
        for segment in "$a" "$b" "$c" "$d"; do
            [[ "$segment" =~ ^[0-9]{1,3}$ ]] && (( 10#$segment <= 255 )) || return 1
        done
        return 0
    fi
...
mieru_display_host() {
    local host="${1:-}"
    [[ -n "$host" ]] || host="$(server_address)"
    mieru_normalize_display_host "$host"
}

mieru_display_url_host() {
    local host
    host="$(mieru_display_host "${1:-}")"
    if [[ "$host" == *:* ]]; then printf '[%s]' "$host"; else printf '%s' "$host"; fi
}

mieru_display_port() {
    local mode="$1" transport="$2" actual_port="$3" advertise_port="${4:-}" base
    base="${advertise_port:-$actual_port}"
    if [[ "$mode" == "BOTH" && "$transport" == "UDP" ]]; then printf '%s' "$((base+1))"; else printf '%s' "$base"; fi
}

REPLY_MIERU_ADVERTISE_HOST=""; REPLY_MIERU_ADVERTISE_PORT=""
ask_mieru_display_endpoint() {
    local cur_host="${1:-}" cur_port="${2:-}" actual_port="$3" proto="$4"
    local choice def=1 host port default_host default_port
    [[ -n "$cur_host" || -n "$cur_port" ]] && def=2
    ui_sub "客户端展示入口 (Display Endpoint)"
    ui_item 1 "自动" "使用脚本全局节点地址与实际端口"
    ui_item 2 "自定义" "仅影响链接/客户端 JSON/Mihomo, 不修改服务端监听"
    read -rp "$(ui_prompt "选择" "回车=${def}")" choice
    if [[ "${choice:-$def}" != "2" ]]; then
        REPLY_MIERU_ADVERTISE_HOST=""; REPLY_MIERU_ADVERTISE_PORT=""
        return 0
    fi

    default_host="${cur_host:-$(server_address)}"
    [[ "$default_host" == "YOUR_SERVER_IP" ]] && default_host=""
    while true; do
        if [[ -n "$default_host" ]]; then
            read -rp "$(ui_prompt "客户端入口域名或 IP" "回车=${default_host}")" host
            host="${host:-$default_host}"
        else
            read -rp "$(ui_prompt "客户端入口域名或 IP")" host
        fi
        host="$(mieru_normalize_display_host "$host")"
        mieru_valid_display_host "$host" && break
        err "  地址无效, 请输入合法 IPv4、IPv6 或域名。"
    done

    default_port="${cur_port:-$actual_port}"
    while true; do
        read -rp "$(ui_prompt "客户端展示基础端口" "回车=${default_port}")" port
        port="${port:-$default_port}"
        if ! [[ "$port" =~ ^[0-9]+$ ]] || (( port < 1 || port > 65535 )); then
            err "  展示端口无效, 请输入 1-65535。"; continue
        fi
        if [[ "$proto" == "BOTH" && "$port" -ge 65535 ]]; then
            err "  双协议展示端口需要预留端口+1, 基础端口不能为 65535。"; continue
        fi
        break
    done
    REPLY_MIERU_ADVERTISE_HOST="$host"; REPLY_MIERU_ADVERTISE_PORT="$port"
}

mieru_node_detail() {
    local user="$1" pass="$2" proto="$3" port="$4" mtu="$5" advertise_host="${6:-}" advertise_port="${7:-}"
    local host display_ports actual_ports
    mieru_load_opts
    host="$(mieru_display_url_host "$advertise_host")"
    if [[ "$proto" == "BOTH" ]]; then
        display_ports="$(mieru_display_port "$proto" TCP "$port" "$advertise_port") / $(mieru_display_port "$proto" UDP "$port" "$advertise_port")"
        actual_ports="${port} / $((port+1))"
    else
        display_ports="$(mieru_display_port "$proto" "$proto" "$port" "$advertise_port")"
        actual_ports="$port"
    fi
    printf '  地址: %s\n  端口: %s' "$host" "$display_ports"
    [[ -n "$advertise_host" ]] && printf '\n  实际监听端口: %s' "$actual_ports"
    printf '\n  用户名: %s\n  密码: %s\n  协议: %s\n  MTU : %s\n  Profile: %s' \
        "$user" "$pass" "$proto" "$mtu" "$MIERU_PROFILE"
}

mieru_links_for_node() {
    local name="$1" user="$2" pass="$3" proto="$4" port="$5" mtu="$6"
    local advertise_host="${7:-}" advertise_port="${8:-}" host eu ep item pr po tp tp_query=""
    : "$name"
    mieru_load_opts
    host="$(mieru_display_url_host "$advertise_host")"
    eu="$(url_encode "$user")"; ep="$(url_encode "$pass")"
    tp="$(mieru_export_traffic_pattern)"
    [[ -n "$tp" ]] && tp_query="&traffic-pattern=$(url_encode "$tp")"
    case "$proto" in
        TCP) set -- "TCP:${port}" ;; UDP) set -- "UDP:${port}" ;;
        BOTH) set -- "TCP:${port}" "UDP:$((port+1))" ;; *) set -- "${proto}:${port}" ;;
    esac
    for item in "$@"; do
        pr="${item%%:*}"
        po="$(mieru_display_port "$proto" "$pr" "$port" "$advertise_port")"
        printf 'mierus://%s:%s@%s?handshake-mode=%s&mtu=%s&multiplexing=%s&port=%s&profile=default&protocol=%s%s\n' \
            "$eu" "$ep" "$host" "$MIERU_HANDSHAKE" "$mtu" "$MIERU_MULTIPLEXING" "$po" "$pr" "$tp_query"
    done
}

mieru_mihomo_yaml_for_node() {
    local name="$1" user="$2" pass="$3" proto="$4" port="$5"
    local advertise_host="${6:-}" advertise_port="${7:-}" raw_host item pr po mname tp tp_line=""
    mieru_load_opts
    raw_host="$(mieru_display_host "$advertise_host")"; tp="$(mieru_export_traffic_pattern)"
    [[ -n "$tp" ]] && tp_line="\n    traffic-pattern: $(yaml_quote "$tp")"
    case "$proto" in BOTH) set -- "TCP:${port}" "UDP:$((port+1))" ;; *) set -- "${proto}:${port}" ;; esac
    for item in "$@"; do
        pr="${item%%:*}"
        po="$(mieru_display_port "$proto" "$pr" "$port" "$advertise_port")"
        mname="$name"; [[ "$proto" == "BOTH" ]] && mname="${name}-${pr}"
        printf '  - name: %s\n    type: mieru\n    server: %s\n    port: %s\n    transport: %s\n    udp: true\n    username: %s\n    password: %s\n    multiplexing: %s\n    handshake-mode: %s%b\n' \
            "$(yaml_quote "$mname")" "$(yaml_quote "$raw_host")" "$po" "$pr" "$(yaml_quote "$user")" \
            "$(yaml_quote "$pass")" "$MIERU_MULTIPLEXING" "$MIERU_HANDSHAKE" "$tp_line"
    done
}

# proto/port 组合, 每行 "tcp 端口", 供防火墙使用
mieru_port_pairs() {
...
# 旧版单节点状态 (MIERU_USER/PASS/PORT) 迁移为 state.json 中的托管节点
mieru_migrate_legacy() {
    [[ -f "$MIERU_STATE" ]] || return 0
    grep -q '^MIERU_USER=' "$MIERU_STATE" 2>/dev/null || return 0
    # shellcheck disable=SC1090
    source "$MIERU_STATE" 2>/dev/null || true
    [[ -n "${MIERU_USER:-}" && -n "${MIERU_PASS:-}" && -n "${MIERU_PORT:-}" ]] || return 0
    local proto="${MIERU_PROTO:-TCP}" mtu="${MIERU_MTU:-1400}"
    local tag="mieru-${MIERU_PORT}" name="Mieru-${proto}-${MIERU_PORT}" link detail extra
    if ! jq -e --arg t "$tag" '.nodes[$t]' "$STATE" >/dev/null 2>&1; then
        link="$(mieru_links_for_node "$name" "$MIERU_USER" "$MIERU_PASS" "$proto" "$MIERU_PORT" "$mtu" "" "")"
        detail="$(mieru_node_detail "$MIERU_USER" "$MIERU_PASS" "$proto" "$MIERU_PORT" "$mtu" "" "")"
        extra="$(jq -cn --arg user "$MIERU_USER" --arg pass "$MIERU_PASS" --arg proto "$proto" \
            --argjson port "$MIERU_PORT" --argjson mtu "$mtu" \
            '{user:$user, pass:$pass, proto:$proto, port:$port, mtu:$mtu, advertise_host:"", advertise_port:null}')"
        init_state
        local tmp; tmp="$(mktemp)"
        jq --arg tag "$tag" --arg name "$name" --arg link "$link" --arg detail "$detail" --arg extra "$extra" \
           '.nodes[$tag] = {core:"mieru-managed", type:"Mieru", name:$name, link:$link, detail:$detail, extra_tag:$extra}' \
           "$STATE" > "$tmp" && mv "$tmp" "$STATE"
        info "已将旧版 Mieru 单节点配置迁移为托管节点: ${tag}"
    fi
    MIERU_MTU="$mtu"
    unset MIERU_USER MIERU_PASS MIERU_PORT MIERU_PROTO
    mieru_load_opts; MIERU_MTU="$mtu"
    # 重写完整新状态, 去掉旧版单节点字段。
    rm -f "$MIERU_STATE"
    mieru_save_opts
}
...
mieru_effective_mtu() {
    local mtu
    mtu="$(mieru_state_nodes_json | jq -r 'if length == 0 then 1400 else ([.[] | (.mtu // 1400)] | min) end' 2>/dev/null)"
    [[ "$mtu" =~ ^[0-9]+$ ]] && printf '%s' "$mtu" || printf '1400'
}

mieru_write_client_json_for_node() {
    local name="$1" user="$2" pass="$3" proto="$4" port="$5" mtu="$6"
    local advertise_host="${7:-}" advertise_port="${8:-}" output_dir="${9:-$MIERU_CLIENT_DIR}"
    local host ip="" domain="" tp item pr po filename tmp
    mieru_load_opts
    host="$(mieru_display_host "$advertise_host")"; tp="$(mieru_traffic_pattern_json)"
    if [[ "$host" == *:* || "$host" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then ip="$host"; else domain="$host"; fi
    mkdir -p "$(dirname "$MIERU_CLIENT_DIR")"
    mkdir -p "$output_dir"; chmod 700 "$(dirname "$output_dir")" "$output_dir" 2>/dev/null || true
    case "$proto" in
        TCP) set -- "TCP:${port}" ;; UDP) set -- "UDP:${port}" ;;
        BOTH) set -- "TCP:${port}" "UDP:$((port+1))" ;; *) set -- "${proto}:${port}" ;;
    esac
    for item in "$@"; do
        pr="${item%%:*}"
        po="$(mieru_display_port "$proto" "$pr" "$port" "$advertise_port")"
        filename="$(printf '%s_%s_%s' "$name" "$po" "${pr,,}" | tr -cs 'A-Za-z0-9._-' '_').json"
        tmp="$(mktemp_json)" || return 1
        if ! jq -n --arg user "$user" --arg pass "$pass" --arg ip "$ip" --arg domain "$domain" \
            --arg proto "$pr" --arg mux "$MIERU_MULTIPLEXING" --arg handshake "$MIERU_HANDSHAKE" \
            --argjson port "$po" --argjson mtu "$mtu" --argjson tp "$tp" '
            {profiles:[{
                profileName:"default",
                user:{name:$user, password:$pass},
                servers:[{ipAddress:$ip, domainName:$domain, portBindings:[{port:$port, protocol:$proto}]}],
                mtu:$mtu,
                multiplexing:{level:$mux},
                handshakeMode:$handshake
            } + (if $tp == null then {} else {trafficPattern:$tp} end)],
            activeProfile:"default", rpcPort:8964, socks5Port:1080, loggingLevel:"INFO",
            socks5ListenLAN:false, httpProxyPort:8080, httpProxyListenLAN:false}' > "$tmp"; then
            rm -f "$tmp"; return 1
        fi
        mv "$tmp" "$output_dir/$filename"; chmod 600 "$output_dir/$filename"
    done
}

mieru_refresh_saved_clients() {
    [[ -f "$STATE" ]] || return 0
    local effective_mtu tag name user pass proto port advertise_host advertise_port link detail state_tmp next_tmp staging_dir old_dir
    mieru_load_opts
    effective_mtu="$(mieru_effective_mtu)"
    mkdir -p "$(dirname "$MIERU_CLIENT_DIR")" || return 1
    state_tmp="$(mktemp "${STATE}.tmp.XXXXXX")" || return 1
    staging_dir="$(mktemp -d "${MIERU_CLIENT_DIR}.staging.XXXXXX")" || {
        rm -f "$state_tmp"
        return 1
    }
    if ! jq '.' "$STATE" > "$state_tmp"; then
        rm -f "$state_tmp"; rm -rf "$staging_dir"; return 1
    fi

    while IFS=$'\t' read -r tag name user pass proto port advertise_host advertise_port; do
        [[ -n "$tag" ]] || continue
        link="$(mieru_links_for_node "$name" "$user" "$pass" "$proto" "$port" "$effective_mtu" "$advertise_host" "$advertise_port")"
        detail="$(mieru_node_detail "$user" "$pass" "$proto" "$port" "$effective_mtu" "$advertise_host" "$advertise_port")"
        next_tmp="$(mktemp)" || { rm -f "$state_tmp"; rm -rf "$staging_dir"; return 1; }
        if ! jq --arg tag "$tag" --arg link "$link" --arg detail "$detail" \
            '.nodes[$tag].link=$link | .nodes[$tag].detail=$detail' "$state_tmp" > "$next_tmp"; then
            rm -f "$state_tmp" "$next_tmp"; rm -rf "$staging_dir"; return 1
        fi
        mv "$next_tmp" "$state_tmp"
        mieru_write_client_json_for_node "$name" "$user" "$pass" "$proto" "$port" "$effective_mtu" \
            "$advertise_host" "$advertise_port" "$staging_dir" || {
            rm -f "$state_tmp"; rm -rf "$staging_dir"; return 1
        }
    done < <(jq -r '.nodes | to_entries[] | select(.value.core == "mieru-managed") |
        [.key, .value.name, (.value.extra_tag | fromjson | .user), (.value.extra_tag | fromjson | .pass),
         (.value.extra_tag | fromjson | .proto), (.value.extra_tag | fromjson | .port),
         (.value.extra_tag | fromjson | .advertise_host // ""), (.value.extra_tag | fromjson | .advertise_port // "")] | @tsv' "$STATE" 2>/dev/null)

    chmod 600 "$state_tmp"
    old_dir="${MIERU_CLIENT_DIR}.old.$$"
    rm -rf "$old_dir"
    if [[ -d "$MIERU_CLIENT_DIR" ]]; then mv "$MIERU_CLIENT_DIR" "$old_dir" || {
        rm -f "$state_tmp"; rm -rf "$staging_dir"; return 1
    }; fi
    if ! mv "$staging_dir" "$MIERU_CLIENT_DIR"; then
        [[ -d "$old_dir" ]] && mv "$old_dir" "$MIERU_CLIENT_DIR"
        rm -f "$state_tmp"; rm -rf "$staging_dir"; return 1
    fi
    if ! mv "$state_tmp" "$STATE"; then
        rm -f "$state_tmp"
        rm -rf "$MIERU_CLIENT_DIR"
        [[ -d "$old_dir" ]] && mv "$old_dir" "$MIERU_CLIENT_DIR"
        return 1
    fi
    rm -rf "$old_dir"
...
# 由 state.json 生成完整 mita 配置并应用 (多节点共存核心)
mieru_apply_from_state() {
    local bin nodes cfg count tp
    bin="$(mita_bin)"
    ...
    tp="$(mieru_traffic_pattern_json)"
    cfg="$(mktemp_json)" || { err "创建临时配置失败。"; return 1; }
    if ! printf '%s' "$nodes" | jq --argjson tp "$tp" '{
        portBindings: ([.[] | if .proto == "BOTH" then
                [{port:.port, protocol:"TCP"}, {port:(.port + 1), protocol:"UDP"}]
            else
                [{port:.port, protocol:(.proto // "TCP")}]
            end] | add | unique_by([.port, .protocol])),
        users: ([.[] | {name:.user, password:.pass}] | unique_by(.name)),
        loggingLevel: "INFO",
        mtu: ([.[] | (.mtu // 1400)] | min)
    } + (if $tp == null then {} else {trafficPattern: $tp} end)' > "$cfg"; then
        rm -f "$cfg"; err "生成 Mieru 配置失败。"; return 1
    fi

    mieru_harden_permissions
    systemctl enable "$MITA_SERVICE" >/dev/null 2>&1 || true
    systemctl start "$MITA_SERVICE" >/dev/null 2>&1 || systemctl restart "$MITA_SERVICE" >/dev/null 2>&1 || true
    wait_mita_socket 30 || warn "mita 管理套接字未就绪, 继续尝试 apply..."

    local attempt applied=0
    for attempt in 1 2 3; do
        if "$bin" apply config "$cfg" >/dev/null 2>&1; then applied=1; break; fi
        systemctl restart "$MITA_SERVICE" >/dev/null 2>&1 || true
        wait_mita_socket 10 || true
        sleep 2
    done
    # 兜底: 内核过旧不识别 trafficPattern 时, 去掉该字段再试, 避免整体失败
    if (( applied == 0 )) && [[ "$tp" != "null" ]]; then
        local stripped; stripped="$(mktemp_json)" || { rm -f "$cfg"; return 1; }
        if jq 'del(.trafficPattern)' "$cfg" > "$stripped" && "$bin" apply config "$stripped" >/dev/null 2>&1; then
            warn "当前 mita 不支持流量伪装(trafficPattern), 已忽略该设置并应用其余配置。"
            applied=1
        fi
        rm -f "$stripped"
    fi
    rm -f "$cfg"
    if (( applied == 0 )); then
        err "mita apply config 失败。最近日志:"
        journalctl -u "$MITA_SERVICE" -n 15 --no-pager 2>/dev/null | sed 's/^/    /'
        [[ -s /var/log/mita.err ]] && tail -n 8 /var/log/mita.err | sed 's/^/    /'
        return 1
    fi
    "$bin" start >/dev/null 2>&1 || true
    mieru_load_opts
    MIERU_MTU="$(mieru_effective_mtu)"
    mieru_save_opts
    mieru_refresh_saved_clients || warn "Mieru 客户端链接 / JSON 刷新失败, 服务端配置已正常生效。"
    mieru_harden_permissions
    return 0
}
...
mieru_user_exists() {
    local user="$1"
    [[ -f "$STATE" ]] || return 1
    jq -e --arg u "$user" '[.nodes | to_entries[] | select(.value.core == "mieru-managed")
        | (.value.extra_tag | fromjson | .user)] | index($u) != null' "$STATE" >/dev/null 2>&1
}

# 采集 Mieru 节点参数, 结果写入 REPLY_MIERU_* ; $1..$5 为当前值 (编辑时使用)
REPLY_MIERU_PROTO=""; REPLY_MIERU_PORT=""; REPLY_MIERU_USER=""; REPLY_MIERU_PASS=""; REPLY_MIERU_MTU=""
ask_mieru_node_params() {
    local cur_proto="${1:-TCP}" cur_port="${2:-}" cur_user="${3:-}" cur_pass="${4:-}" cur_mtu="${5:-}"
    local pc def_choice proto port user pass mtu
    mieru_load_opts
    [[ -z "$cur_mtu" ]] && cur_mtu="${MIERU_MTU:-1400}"

    ui_sub "请选择传输协议"
    ui_item 1 "TCP" "推荐"
    ui_item 2 "UDP"
    ui_item 3 "TCP+UDP 双协议"
    case "$cur_proto" in UDP) def_choice=2 ;; BOTH) def_choice=3 ;; *) def_choice=1 ;; esac
    read -rp "$(ui_prompt "选择" "回车=${def_choice}")" pc
    case "${pc:-$def_choice}" in 2) proto="UDP" ;; 3) proto="BOTH" ;; *) proto="TCP" ;; esac

    while true; do
        if [[ -n "$cur_port" ]]; then
            local p
            read -rp "$(ui_prompt "监听端口" "回车=${cur_port}")" p
            p="${p:-$cur_port}"
            if ! [[ "$p" =~ ^[0-9]+$ ]] || (( p < 1 || p > 65535 )); then err "  端口无效, 请输入 1-65535。"; continue; fi
        ...
        if [[ "$proto" == "BOTH" ]]; then
            if (( port >= 65535 )); then err "  双协议需要 端口+1 空闲, 监听端口不能为 65535。"; cur_port=""; continue; fi
            if [[ "$((port+1))" != "$cur_port" ]] && port_used "$((port+1))"; then
                err "  双协议需要 端口+1 (${port}→$((port+1))) 空闲, 请换一个端口。"; continue
            fi
        fi
        break
    done
    ...
    while true; do
        ask_username_value "Mieru 用户名" "${cur_user:-mieru$(openssl rand -hex 3)}"; user="$REPLY_USER"
        if [[ "$user" != "$cur_user" ]] && mieru_user_exists "$user"; then
            err "  用户名 ${user} 已被其他 Mieru 节点使用, 请换一个。"; continue
        fi
        break
    done
    ask_secret_value "Mieru 密码" "${cur_pass:-$(gen_pass 12)}"; pass="$REPLY_PASS"
    ask_int_default "Mieru MTU" "$cur_mtu" 1280 1500; mtu="$REPLY_VALUE"
    [[ "$mtu" == "1400" ]] || MIERU_PROFILE="custom"

    REPLY_MIERU_PROTO="$proto"; REPLY_MIERU_PORT="$port"
    REPLY_MIERU_USER="$user"; REPLY_MIERU_PASS="$pass"; REPLY_MIERU_MTU="$mtu"
    MIERU_MTU="$mtu"; mieru_save_opts
}

# 直接写入 state.json: Mieru 多节点默认共存, 不走 save_node 的"覆盖同类型"询问
mieru_save_node() {
    local tag="$1" name="$2" user="$3" pass="$4" proto="$5" port="$6" mtu="$7"
    local advertise_host="${8:-}" advertise_port="${9:-}" link detail extra tmp
    link="$(mieru_links_for_node "$name" "$user" "$pass" "$proto" "$port" "$mtu" "$advertise_host" "$advertise_port")"
    detail="$(mieru_node_detail "$user" "$pass" "$proto" "$port" "$mtu" "$advertise_host" "$advertise_port")"
    extra="$(jq -cn --arg user "$user" --arg pass "$pass" --arg proto "$proto" \
        --arg advertise_host "$advertise_host" --argjson port "$port" --argjson mtu "$mtu" \
        --argjson advertise_port "${advertise_port:-null}" \
        '{user:$user, pass:$pass, proto:$proto, port:$port, mtu:$mtu,
          advertise_host:$advertise_host, advertise_port:$advertise_port}')" || return 1
    init_state
    tmp="$(mktemp)" || return 1
    if ! jq --arg tag "$tag" --arg name "$name" --arg link "$link" --arg detail "$detail" --arg extra "$extra" \
       '.nodes[$tag] = {core:"mieru-managed", type:"Mieru", name:$name, link:$link, detail:$detail, extra_tag:$extra}' \
    ...
    chmod 600 "$STATE"
    MIERU_LAST_LINK="$link"
}

add_mieru_node() {
    local bin tx; bin="$(mita_bin)"
    [[ -n "$bin" ]] || { err "未找到 mita 二进制, 请先安装 Mieru。"; return 1; }
    mieru_migrate_legacy
    tx="$(mieru_tx_snapshot)" || { err "创建 Mieru 状态快照失败。"; return 1; }

    if mieru_managed_exists; then
        warn "Profile / 客户端模式是 mita 全局设置, 本次修改会同步影响全部 Mieru 节点。"
    fi
    ask_mieru_profile
    ask_mieru_node_params "TCP" "" "" "" "$MIERU_MTU"
    local proto="$REPLY_MIERU_PROTO" port="$REPLY_MIERU_PORT" user="$REPLY_MIERU_USER"
    local pass="$REPLY_MIERU_PASS" mtu="$REPLY_MIERU_MTU" name tag advertise_host advertise_port
    ask_name "Mieru-${proto}-${port}"; name="$REPLY_NAME"
    ask_mieru_display_endpoint "" "" "$port" "$proto"
    advertise_host="$REPLY_MIERU_ADVERTISE_HOST"; advertise_port="$REPLY_MIERU_ADVERTISE_PORT"
    tag="mieru-${port}"

    if ! mieru_save_node "$tag" "$name" "$user" "$pass" "$proto" "$port" "$mtu" "$advertise_host" "$advertise_port" \
       || ! mieru_apply_from_state; then
        mieru_tx_rollback "$tx" || true
        return 1
    fi
    mieru_tx_commit "$tx"
    MIERU_LAST_LINK="$(jq -r --arg t "$tag" '.nodes[$t].link // empty' "$STATE" 2>/dev/null)"
    fw_allow_pairs "$(mieru_port_pairs "$proto" "$port")"
    echo; hr; ok "  ✅ Mieru 节点已添加!"; hr
    echo -e "${BOLD}分享链接:${NC}"; echo -e "${GREEN}${MIERU_LAST_LINK}${NC}"
    echo; echo -e "${BOLD}Mihomo 配置:${NC}"
    echo -e "${GREEN}$(mieru_mihomo_yaml_for_node "$name" "$user" "$pass" "$proto" "$port" "$advertise_host" "$advertise_port")${NC}"
    if command -v qrencode >/dev/null 2>&1; then
        local l
        while IFS= read -r l; do
            [[ -z "$l" ]] && continue
            echo; qrencode -t ANSIUTF8 "$l"
        done <<< "$MIERU_LAST_LINK"
    fi
    hr
}

# 重新配置已有 Mieru 节点 (端口/协议/账号/MTU), 不影响其他节点
reconfigure_mieru_node() {
    local tag="$1" extra old_proto old_port old_user old_pass old_mtu old_advertise_host old_advertise_port name tx
    [[ -n "$tag" ]] || return 1
    extra="$(jq -r --arg t "$tag" '.nodes[$t].extra_tag // empty' "$STATE" 2>/dev/null)"
    [[ -n "$extra" ]] || { err "未找到节点配置: $tag"; return 1; }
    old_user="$(printf '%s' "$extra" | jq -r '.user')"
    old_pass="$(printf '%s' "$extra" | jq -r '.pass')"
    old_proto="$(printf '%s' "$extra" | jq -r '.proto')"
    old_port="$(printf '%s' "$extra" | jq -r '.port')"
    old_mtu="$(printf '%s' "$extra" | jq -r '.mtu // 1400')"
    old_advertise_host="$(printf '%s' "$extra" | jq -r '.advertise_host // ""')"
    old_advertise_port="$(printf '%s' "$extra" | jq -r '.advertise_port // ""')"
    tx="$(mieru_tx_snapshot)" || { err "创建 Mieru 状态快照失败。"; return 1; }

    warn "Profile / 客户端模式是 mita 全局设置, 本次修改会同步影响全部 Mieru 节点。"
    ask_mieru_profile
    ask_mieru_node_params "$old_proto" "$old_port" "$old_user" "$old_pass" "$old_mtu"
    local proto="$REPLY_MIERU_PROTO" port="$REPLY_MIERU_PORT" user="$REPLY_MIERU_USER"
    local pass="$REPLY_MIERU_PASS" mtu="$REPLY_MIERU_MTU" new_tag
    ask_name "$(jq -r --arg t "$tag" '.nodes[$t].name' "$STATE" 2>/dev/null)"; name="$REPLY_NAME"
    if [[ -n "$old_advertise_host" || -n "$old_advertise_port" ]]; then
        if ! mieru_valid_display_host "$old_advertise_host" \
           || ! [[ "$old_advertise_port" =~ ^[0-9]+$ ]] \
           || (( old_advertise_port < 1 || old_advertise_port > 65535 )) \
           || [[ "$proto" == "BOTH" && "$old_advertise_port" -ge 65535 ]]; then
            warn "原客户端展示入口不适用于新的协议设置, 请重新确认。"
            ask_mieru_display_endpoint "$old_advertise_host" "$old_advertise_port" "$port" "$proto"
            old_advertise_host="$REPLY_MIERU_ADVERTISE_HOST"
            old_advertise_port="$REPLY_MIERU_ADVERTISE_PORT"
        fi
    fi
    new_tag="mieru-${port}"

    if ! delete_state_node_only "$tag" \
       || ! mieru_save_node "$new_tag" "$name" "$user" "$pass" "$proto" "$port" "$mtu" \
            "$old_advertise_host" "$old_advertise_port" \
       || ! mieru_apply_from_state; then
        mieru_tx_rollback "$tx" || true
        return 1
    fi
    mieru_tx_commit "$tx"
    MIERU_LAST_LINK="$(jq -r --arg t "$new_tag" '.nodes[$t].link // empty' "$STATE" 2>/dev/null)"
    fw_deny_pairs "$(mieru_port_pairs "$old_proto" "$old_port")"
    fw_allow_pairs "$(mieru_port_pairs "$proto" "$port")"
    echo; hr; ok "  ✅ Mieru 节点已更新!"; hr
    echo -e "${GREEN}${MIERU_LAST_LINK}${NC}"
    hr
}

set_mieru_node_endpoint() {
    local tag="$1" extra proto port current_host current_port advertise_host advertise_port tx tmp
    [[ -n "$tag" ]] || return 1
    extra="$(jq -r --arg t "$tag" '.nodes[$t].extra_tag // empty' "$STATE" 2>/dev/null)"
    [[ -n "$extra" ]] || { err "未找到节点配置: $tag"; return 1; }
    proto="$(printf '%s' "$extra" | jq -r '.proto')"
    port="$(printf '%s' "$extra" | jq -r '.port')"
    current_host="$(printf '%s' "$extra" | jq -r '.advertise_host // ""')"
    current_port="$(printf '%s' "$extra" | jq -r '.advertise_port // ""')"
    tx="$(mieru_tx_snapshot)" || { err "创建 Mieru 状态快照失败。"; return 1; }

    ask_mieru_display_endpoint "$current_host" "$current_port" "$port" "$proto"
    advertise_host="$REPLY_MIERU_ADVERTISE_HOST"; advertise_port="$REPLY_MIERU_ADVERTISE_PORT"
    tmp="$(mktemp)" || { mieru_tx_commit "$tx"; return 1; }
    if ! jq --arg t "$tag" --arg host "$advertise_host" --argjson port "${advertise_port:-null}" '
        .nodes[$t].extra_tag |= ((fromjson + {advertise_host:$host, advertise_port:$port}) | tojson)
    ' "$STATE" > "$tmp" || ! mv "$tmp" "$STATE"; then
        rm -f "$tmp"
        mieru_tx_restore "$tx" 2>/dev/null || true
        mieru_tx_commit "$tx" 2>/dev/null || true
        err "更新 Mieru 展示入口失败。"
        return 1
    fi
    chmod 600 "$STATE"
    if ! mieru_refresh_saved_clients; then
        mieru_tx_restore "$tx" 2>/dev/null || true
        mieru_tx_commit "$tx" 2>/dev/null || true
        err "刷新 Mieru 客户端配置失败, 已恢复原展示入口。"
        return 1
    fi
    mieru_tx_commit "$tx"
    MIERU_LAST_LINK="$(jq -r --arg t "$tag" '.nodes[$t].link // empty' "$STATE" 2>/dev/null)"
    ok "Mieru 客户端展示入口已更新, 服务端监听与防火墙未改变。"
    echo -e "${GREEN}${MIERU_LAST_LINK}${NC}"
}

ask_mieru_release_channel() {
    mieru_load_opts
    local c def=1
    [[ "$MIERU_CHANNEL" == "latest" ]] && def=2
    ui_sub "Mieru 版本通道"
    ui_item 1 "Stable v${MIERU_STABLE_VERSION}" "已验证, 推荐"
    ui_item 2 "Latest" "跟随上游最新 release"
    read -rp "$(ui_prompt "选择" "回车=${def}")" c
    [[ "${c:-$def}" == "2" ]] && MIERU_CHANNEL="latest" || MIERU_CHANNEL="stable"
    mieru_save_opts
}

install_mieru() {
    ui_deploy_head "Mieru" "高抗审查底层协议"
    ...
    if mieru_installed; then
        warn "检测到 Mieru 已安装 (当前版本: $(mieru_version))。"
        if confirm "是否新增 Mieru 节点 (不下载最新版)?" "y"; then
            add_mieru_node
            return $?
        fi
        confirm "是否继续下载并覆盖安装所选版本?" "n" || return 0
    fi

    ask_mieru_release_channel
    local tag ver
    if [[ "$MIERU_CHANNEL" == "stable" ]]; then
        tag="v${MIERU_STABLE_VERSION}"
    else
        info "获取 Mieru 上游最新版本..."
        tag="$(gh_latest_tag "$MITA_REPO")"
        [[ -z "$tag" ]] && { err "无法获取 Mieru 版本 (GitHub 访问失败)。"; return 1; }
    fi
    ver="${tag#v}"

    local tmp url; tmp="$(mktemp -d)"
    if command -v dpkg >/dev/null 2>&1 && pm_has_systemd; then
        ...
        verify_sha256 "$tmp/mita.deb" "${url}.sha256.txt" || { rm -rf "$tmp"; return 1; }
        if ! DEBIAN_FRONTEND=noninteractive dpkg -i "$tmp/mita.deb" >/dev/null 2>&1; then
            DEBIAN_FRONTEND=noninteractive apt-get install -f -y >/dev/null 2>&1 \
                || { err "mita deb 包安装失败。"; rm -rf "$tmp"; return 1; }
        fi
    elif command -v rpm >/dev/null 2>&1 && pm_has_systemd; then
        ...
        verify_sha256 "$tmp/mita.rpm" "${url}.sha256.txt" || { rm -rf "$tmp"; return 1; }
        if ! rpm -Uvh --force "$tmp/mita.rpm" >/dev/null 2>&1; then
            command -v dnf >/dev/null 2>&1 && dnf install -y "$tmp/mita.rpm" >/dev/null 2>&1 \
                || { err "mita rpm 包安装失败。"; rm -rf "$tmp"; return 1; }
        fi
    else
        # 通用 tar.gz (Alpine/OpenRC 或无包管理器时): 手动安装二进制 + 服务
        ...
        verify_sha256 "$tmp/mita.tar.gz" "${url}.sha256.txt" || { rm -rf "$tmp"; return 1; }
        tar -xzf "$tmp/mita.tar.gz" -C "$tmp" || { err "解压失败。"; rm -rf "$tmp"; return 1; }
        local extracted; extracted="$(find "$tmp" -type f -name mita | head -n1)"
        [[ -n "$extracted" ]] || { err "未找到 mita 可执行文件。"; rm -rf "$tmp"; return 1; }
        install -m 755 "$extracted" /usr/local/bin/mita
        mieru_ensure_account
        if pm_has_systemd; then
            mkdir -p "$(dirname "$MITA_SERVICE_FILE")"
...
[Service]
Type=simple
User=$(id mita >/dev/null 2>&1 && echo mita || echo root)
Group=$(id mita >/dev/null 2>&1 && echo mita || echo root)
...
EOF
        ...
        pm_write_openrc_service "$MITA_SERVICE" "mita proxy server" "/usr/local/bin/mita" "run" \
            "$(id mita >/dev/null 2>&1 && printf 'command_user="mita"\nrespawn\nrespawn_delay 5' || printf 'respawn\nrespawn_delay 5')"
        systemctl daemon-reload
    fi
    rm -rf "$tmp"
    ...
    ok "Mieru (mita) ${tag} 安装完成。"
    mieru_harden_permissions
    mieru_migrate_legacy
    # 覆盖安装后重新下发已有节点配置, 保证多节点不丢失
    mieru_managed_exists && mieru_apply_from_state >/dev/null 2>&1
    add_mieru_node
}

# 输出全部 Mieru 托管节点的链接 / Mihomo 配置 / 二维码
mieru_show_link() {
    mieru_migrate_legacy
    mieru_managed_exists || { warn "未找到 Mieru 节点。"; return 0; }
    mieru_load_opts
    local row tag name user pass proto port mtu advertise_host advertise_port link effective_mtu
    effective_mtu="$(mieru_effective_mtu)"
    while IFS=$'\t' read -r tag name user pass proto port mtu advertise_host advertise_port; do
        mtu="$effective_mtu"
        [[ -z "$tag" ]] && continue
        echo
        echo -e "${BOLD}[Mieru] ${name}${NC}  ${DIM}(${tag})${NC}"
        echo -e "${BOLD}用户: ${NC}${user}   ${BOLD}密码: ${NC}${pass}   ${BOLD}协议: ${NC}${proto}   ${BOLD}MTU: ${NC}${mtu}   ${BOLD}伪装: ${NC}${MIERU_TRAFFIC:-conservative}"
        [[ -n "$advertise_host" ]] && echo -e "${BOLD}客户端入口: ${NC}$(mieru_display_url_host "$advertise_host"):${advertise_port}  ${DIM}(仅输出层)${NC}"
        link="$(mieru_links_for_node "$name" "$user" "$pass" "$proto" "$port" "$mtu" "$advertise_host" "$advertise_port")"
        ...
        | [.key, .value.name, (.value.extra_tag | fromjson | .user), (.value.extra_tag | fromjson | .pass),
           (.value.extra_tag | fromjson | .proto), (.value.extra_tag | fromjson | .port),
           (.value.extra_tag | fromjson | .mtu // 1400), (.value.extra_tag | fromjson | .advertise_host // ""),
           (.value.extra_tag | fromjson | .advertise_port // "")] | @tsv' "$STATE" 2>/dev/null)
    echo -e "${DIM}官方客户端 JSON: ${MIERU_CLIENT_DIR}/*.json${NC}"
...
uninstall_mieru() {
    ...
    if [[ -f "$STATE" ]]; then
        ...
        while IFS=$'\t' read -r proto port; do
            [[ -z "$port" ]] && continue
            fw_deny_pairs "$(mieru_port_pairs "$proto" "$port")"
        done < <(jq -r '.nodes | to_entries[] | select(.value.core == "mieru-managed")
            | [(.value.extra_tag | fromjson | .proto), (.value.extra_tag | fromjson | .port)] | @tsv' "$STATE" 2>/dev/null)
        local tmp; tmp="$(mktemp)"
    ...
    pm_remove_openrc_service "$MITA_SERVICE"
    rm -f "$MITA_SERVICE_FILE" "$MIERU_STATE"; rm -rf /etc/mita "$MIERU_CLIENT_DIR"
    rmdir "${MIERU_CLIENT_DIR%/current}" 2>/dev/null || true
...
install_wireguard() {
    ui_deploy_head "WireGuard" "现代轻量 VPN"
...
wg_show_link() {
    ...
    # shellcheck disable=SC1090
    source "$WG_STATE"
    local host raw_host; host="$(server_host)"; raw_host="$(server_address)"
...
EOF
    ...
    echo -e "${GREEN}"
    printf '  - name: %s\n    type: wireguard\n    server: %s\n    port: %s\n    ip: %s\n    private-key: %s\n    public-key: %s\n    udp: true\n' \
        "$(yaml_quote "WireGuard")" "$(yaml_quote "$raw_host")" "$WG_PORT" "$(yaml_quote "$WG_CLIENT_ADDR")" \
        "$(yaml_quote "$WG_CLIENT_PRIV")" "$(yaml_quote "$WG_SERVER_PUB")"
    echo -e "${NC}"
    if command -v qrencode >/dev/null 2>&1; then
        echo
...
view_all_nodes() {
    ...
    if [[ "$(node_count)" == "0" ]]; then
        ...
        local host; host="$(server_host)"
        ui_head "全部节点  ${G_SEP}  共 $(node_count) 个" "$C_ACC"
        printf '%b\n' "   ${C_KEY}当前节点地址${NC}  ${BOLD}${C_OKC}${host}${NC}   ${C_DIM2}可在「编辑节点配置」改为域名${NC}"
        local t n=1
        while IFS= read -r t; do
            [[ -z "$t" ]] && continue
            local type name link detail dline
            ...
            echo
            ui_card_top "$(printf '%02d' "$n")  ${G_SEP}  ${type}  ${G_SEP}  ${name}" "$C_ACC2"
            ui_card_row "${BOLD}${C_OKC}${link}${NC}"
            ui_card_gap
            while IFS= read -r dline; do
                [[ -z "$dline" ]] && continue
                if [[ "$dline" == *:* ]]; then
                    # 字段名与字段值分色, 建立"标签弱、数据强"的阅读层次
                    ui_card_row "${C_KEY}${dline%%:*}${NC}${C_DIM2}:${NC}${C_VAL}${dline#*:}${NC}"
                else
                    ui_card_row "${C_MUT}${dline}${NC}"
                fi
            done <<< "$(pm_format_detail_text "$detail")"
            ui_card_end
            n=$((n + 1))
        done <<< "$(jq -r '.nodes | keys[]' "$STATE")"
    fi
    if snell_installed; then
        echo; ui_head "Snell 独立服务" "$C_VIO"; snell_show_link
    fi
    # Mieru 节点已在上方通用列表输出; 这里补充 Mihomo 配置与二维码
    if mieru_managed_exists; then
        echo; ui_head "Mieru 客户端配置" "$C_VIO"; mieru_show_link
    fi
    if [[ -f "$WG_STATE" ]] && grep -q '^WG_CLIENT_PRIV=' "$WG_STATE" 2>/dev/null; then
        echo; ui_head "WireGuard 客户端配置" "$C_VIO"; wg_show_link
    fi
}
...
manage_nodes() {
    ...
    # Mieru 节点已全部纳入 state.json (core=mieru-managed), 与其他节点一同按 tag 管理
    if [[ -f "$WG_STATE" ]] && grep -q '^WG_CLIENT_PRIV=' "$WG_STATE" 2>/dev/null; then
        local wp
    ...
    ui_head "删除节点  ${G_SEP}  共 ${total} 个" "$C_ERRC"
    local i n=1
    for i in "${!tags[@]}"; do
        ui_node_row "$n" "${types[$i]}" "${names[$i]}"
        ((n++))
    done
    for i in "${!specials[@]}"; do
        ui_node_row "$n" "${special_types[$i]}" "${special_names[$i]}" "${special_ports[$i]}"
        ((n++))
    done
    ui_item_back
    local choice
    read -rp "$(ui_prompt_warn "选择要删除的节点编号" "0-${total}")" choice
...
delete_node() {
    ...
    if [[ "$core" == "snell-v6-managed" ]]; then
        delete_snell_v6_managed_node "$tag"
        return $?
    fi
    ...
    local del_pairs=""
    if [[ "$core" == "xray" ]]; then
        del_pairs="$(jq -r --arg t "$tag" '.inbounds[] | select(.tag==$t) | (if ((.streamSettings.network // "") == "kcp") then "udp" else "tcp" end) + " " + (.port|tostring)' "$cfg" 2>/dev/null)"
    else
        del_pairs="$(jq -r --arg t "$tag" '.inbounds[] | select(.tag==$t) | (if ((.type // "") | test("hysteria2|tuic|naive")) then "udp" else "tcp" end) + " " + (.listen_port|tostring)' "$cfg" 2>/dev/null)"
    fi
...
delete_mieru_managed_node() {
    local tag="$1" extra proto port
    extra="$(jq -r --arg t "$tag" '.nodes[$t].extra_tag // empty' "$STATE" 2>/dev/null)"
    delete_state_node_only "$tag"
    mieru_apply_from_state || return 1
    if [[ -n "$extra" ]]; then
        proto="$(printf '%s' "$extra" | jq -r 'try (.proto // empty) catch empty')"
        port="$(printf '%s' "$extra" | jq -r 'try (.port // empty) catch empty')"
        [[ -n "$port" ]] && fw_deny_pairs "$(mieru_port_pairs "$proto" "$port")"
    fi
    ok "Mieru 节点已删除。"
}
...
delete_snell_v6_runtime() {
    local port="$1" svc conf
    svc="$(snell_v6_service "$port")"; conf="$(snell_v6_conf "$port")"
...
delete_snell_v6_managed_node() {
    local tag="$1" port svc conf tmp
    port="${tag#snell-v6-}"
    [[ "$port" =~ ^[0-9]+$ ]] || { err "无法从 Snell v6 节点标签解析端口: ${tag}"; return 1; }
    svc="$(snell_v6_service "$port")"; conf="$(snell_v6_conf "$port")"
    delete_snell_v6_runtime "$port"
    if [[ -f "$STATE" ]]; then
        tmp="$(mktemp)"
...
set_node_host() {
    ...
    local h tmp
    read -rp "$(ui_prompt "请输入节点地址 域名或 IP, 直接回车清除")" h
    tmp="$(mktemp)"; jq --arg h "$h" '.meta.host = $h' "$STATE" > "$tmp" && mv "$tmp" "$STATE"
    if [[ -n "$h" ]]; then ok "节点地址已设置为: $h"; else ok "已恢复自动 IP。"; fi
    rebuild_links
    if mieru_managed_exists; then
        mieru_refresh_saved_clients || warn "Mieru 客户端链接 / JSON 刷新失败, 请重新查看节点后确认。"
    fi
    ok "已同步更新全部节点链接的地址部分。"
}
...
rebuild_links() {
    ...
    done <<< "$(jq -r '.nodes | to_entries[] | select(.value.core != "mieru-managed") | .key' "$STATE" 2>/dev/null)"
}
...
clash_node_yaml() {
    local type="$1" name="$2" link="$3" y_name
    y_name="$(yaml_quote "$name")"
    case "$type" in
        VLESS-Reality-Vision|VLESS-XHTTP-Reality|VLESS-gRPC-Reality|VLESS-Encryption|VLESS-Encryption-XHTTP|VLESS-WS-TLS|VLESS-gRPC-TLS|VLESS-XHTTP-TLS)
            clash_hostport "$link"
            local uuid="${link#vless://}"; uuid="${uuid%%@*}"
            local security net sni pbk sid flow svc path enc allow_insecure host_header pcs
            ...
            allow_insecure="$(clash_q "$link" allowInsecure)"; host_header="$(clash_q "$link" host)"; pcs="$(clash_q "$link" pcs)"
            printf '  - name: %s\n    type: vless\n    server: %s\n    port: %s\n    uuid: %s\n    udp: true\n    network: %s\n' \
                "$y_name" "$(yaml_quote "$CL_HOST")" "$CL_PORT" "$(yaml_quote "$uuid")" "$net"
            [[ -n "$flow" ]] && printf '    flow: %s\n' "$(yaml_quote "$flow")"
            [[ -n "$enc" ]] && printf '    encryption: %s\n' "$(yaml_quote "$enc")"
            if [[ "$security" == reality ]]; then
                printf '    tls: true\n    servername: %s\n    client-fingerprint: chrome\n    reality-opts:\n      public-key: %s\n      short-id: %s\n' \
                    "$(yaml_quote "$sni")" "$(yaml_quote "$pbk")" "$(yaml_quote "$sid")"
            elif [[ "$security" == tls ]]; then
                printf '    tls: true\n    servername: %s\n    skip-cert-verify: %s\n' \
                    "$(yaml_quote "$sni")" "$([[ "$allow_insecure" == "1" || -n "$pcs" ]] && echo true || echo false)"
            fi
            ...
            ;;
        VMess-WS|VMess-TCP|VMess-WS-TLS|VMess-gRPC-TLS)
            local js; js="$(printf '%s' "${link#vmess://}" | base64 -d 2>/dev/null)"
            [[ -z "$js" ]] && return
            local net tls sni path host header_type vmess_host
            ...
            header_type="$(echo "$js" | jq -r '.type // "none"')"
            [[ "$net" == tcp && "$header_type" == http ]] && net=http
            vmess_host="$(echo "$js" | jq -r .add)"; vmess_host="${vmess_host#[}"; vmess_host="${vmess_host%]}"
            printf '  - name: %s\n    type: vmess\n    server: %s\n    port: %s\n    uuid: %s\n    alterId: 0\n    cipher: auto\n    udp: true\n    network: %s\n' \
                "$y_name" "$(yaml_quote "$vmess_host")" "$(echo "$js" | jq -r .port)" "$(yaml_quote "$(echo "$js" | jq -r .id)")" "$net"
            if [[ "$tls" == "tls" ]]; then
            ...
            ;;
        Trojan-Reality|Trojan-TCP-TLS|Trojan-WS-TLS|Trojan-gRPC-TLS)
            clash_hostport "$link"
            local pw="${link#trojan://}"; pw="${pw%%@*}"
            local security net sni pbk sid svc path allow_insecure host_header pcs
            security="$(clash_q "$link" security)"; net="$(clash_q "$link" type)"; [[ -z "$net" ]] && net=tcp
            sni="$(clash_q "$link" sni)"; pbk="$(clash_q "$link" pbk)"; sid="$(clash_q "$link" sid)"
            svc="$(clash_q "$link" serviceName)"; path="$(clash_q "$link" path)"
            allow_insecure="$(clash_q "$link" allowInsecure)"; host_header="$(clash_q "$link" host)"; pcs="$(clash_q "$link" pcs)"
            printf '  - name: %s\n    type: trojan\n    server: %s\n    port: %s\n    password: %s\n    udp: true\n    network: %s\n    tls: true\n    sni: %s\n' \
                "$y_name" "$(yaml_quote "$CL_HOST")" "$CL_PORT" "$(yaml_quote "$pw")" "$net" "$(yaml_quote "$sni")"
            if [[ "$security" == reality ]]; then
                printf '    client-fingerprint: chrome\n    reality-opts:\n      public-key: %s\n      short-id: %s\n' \
                    "$(yaml_quote "$pbk")" "$(yaml_quote "$sid")"
            else
                printf '    skip-cert-verify: %s\n' "$([[ "$allow_insecure" == "1" || -n "$pcs" ]] && echo true || echo false)"
            fi
            if [[ "$net" == ws && -n "$path" ]]; then
                printf '    ws-opts:\n      path: %s\n' "$(yaml_quote "$path")"
                [[ -n "$host_header" ]] && printf '      headers:\n        Host: %s\n' "$(yaml_quote "$host_header")"
        ...
        Shadowsocks|SS2022)
            ...
            printf '  - name: %s\n    type: ss\n    server: %s\n    port: %s\n    cipher: %s\n    password: %s\n    udp: true\n' \
                "$y_name" "$(yaml_quote "$CL_HOST")" "$CL_PORT" "$(yaml_quote "${dec%%:*}")" "$(yaml_quote "${dec#*:}")"
        ...
        HTTP)
            ...
            printf '  - name: %s\n    type: http\n    server: %s\n    port: %s\n    username: %s\n    password: %s\n' \
                "$y_name" "$(yaml_quote "$CL_HOST")" "$CL_PORT" "$(yaml_quote "${ui%%:*}")" "$(yaml_quote "${ui#*:}")"
            ;;
        Hysteria2)
            clash_hostport "$link"
            ...
            printf '  - name: %s\n    type: hysteria2\n    server: %s\n    port: %s\n    password: %s\n    sni: %s\n    skip-cert-verify: true\n    alpn:\n      - h3\n' \
                "$y_name" "$(yaml_quote "$CL_HOST")" "$CL_PORT" "$(yaml_quote "$pw")" "$(yaml_quote "$sni")"
            ;;
        TUIC)
            clash_hostport "$link"
            ...
            printf '  - name: %s\n    type: tuic\n    server: %s\n    port: %s\n    uuid: %s\n    password: %s\n    sni: %s\n    skip-cert-verify: true\n    udp-relay-mode: native\n    congestion-controller: bbr\n    alpn:\n      - h3\n' \
                "$y_name" "$(yaml_quote "$CL_HOST")" "$CL_PORT" "$(yaml_quote "${ui%%:*}")" "$(yaml_quote "${ui#*:}")" "$(yaml_quote "$sni")"
            ;;
        AnyTLS)
            clash_hostport "$link"
...
node_get_port() {
    ...
    if [[ "$core" == "mieru-managed" ]]; then
        ...
        printf '%s\n' "${tag#snell-}"
    elif [[ "$core" == "snell-v6-managed" ]]; then
        printf '%s\n' "${tag#snell-v6-}"
...
change_node_uuid() {
    ...
    read -rp "$(ui_prompt "新 UUID" "回车随机生成")" uuid
...
change_node_password() {
    ...
    core="$(jq -r --arg t "$tag" '.nodes[$t].core // empty' "$STATE")"
    type="$(jq -r --arg t "$tag" '.nodes[$t].type // empty' "$STATE")"
    read -rp "$(ui_prompt "新密码" "回车随机生成")" new
    if [[ -z "$new" ]]; then
        if [[ "$type" == "SS2022" ]]; then
    ...
    case "$type" in
        Trojan-*|HTTP|Hysteria2|TUIC|AnyTLS|NaïveProxy)
            [[ "$new" == *":"* || "$new" == *"/"* || "$new" == *"?"* || "$new" == *"&"* || "$new" == *"%"* ]] && {
                err "该协议密码不能包含 :、/、?、&、% 。"
                return 1
            }
...
change_node_path() {
    ...
    if [[ "$net" == "grpc" ]]; then
        read -rp "$(ui_prompt "新的 gRPC serviceName")" new
        key="serviceName"
    else
        read -rp "$(ui_prompt "新的 Path" "回车随机")" new
...
change_node_sni() {
    ...
    read -rp "$(ui_prompt "新的 SNI/Host")" new
    [[ -z "$new" ]] && { warn "未修改。"; return 0; }
    tmp="$(mktemp)"
...
change_node_method() {
    ...
    ui_sub "选择加密方式"
    if [[ "$type" == "SS2022" ]]; then
        ui_item 1 "2022-blake3-aes-128-gcm"
        ui_item 2 "2022-blake3-aes-256-gcm"
        ui_item 3 "2022-blake3-chacha20-poly1305"
        read -rp "$(ui_prompt "请选择" "1-3")" choice
        case "$choice" in
        ...
        ui_item 1 "aes-128-gcm"
        ui_item 2 "aes-256-gcm"
        ui_item 3 "chacha20-poly1305"
        read -rp "$(ui_prompt "请选择" "1-3, 默认2")" choice
        case "$choice" in
...
change_node_obfs_type() {
    ...
    core="$(jq -r --arg t "$tag" '.nodes[$t].core // empty' "$STATE")"
    type="$(jq -r --arg t "$tag" '.nodes[$t].type // empty' "$STATE")"
    [[ "$core" == "xray" && "$type" == "VMess-TCP" ]] || { warn "仅 VMess TCP 支持修改伪装类型。"; return 0; }
    net="$(node_stream_network "$tag" "$core")"
    ui_sub "选择伪装类型"
    [[ "$net" == "tcp" ]] || { warn "当前节点不是 VMess TCP。"; return 0; }
    ui_item 1 "none" "不伪装"
    ui_item 2 "http" "伪装成 HTTP 流量"
    read -rp "$(ui_prompt "请选择" "1-2")" choice
    case "$choice" in 2) new="http" ;; 1|"") new="none" ;; *) err "无效选项。"; return 1 ;; esac
    cfg="$XRAY_CONFIG"; tmp="$(mktemp)"
    jq --arg t "$tag" --arg v "$new" '(.inbounds[]|select(.tag==$t).streamSettings.tcpSettings.header.type)=$v' "$cfg" > "$tmp"
    core_apply_file "$core" "$tmp" || return 1
    link="$(jq -r --arg t "$tag" '.nodes[$t].link' "$STATE")"
...
pick_node() {
    ...
    for i in "${!tags[@]}"; do
        ...
        ui_node_row "$n" "$type" "$name" "${port:-未知}"
        ((n++))
    done
    for type in "${specials[@]}"; do
        ...
        ui_node_row "$n" "独立协议" "$name" "$port"
        ((n++))
    done
    ui_item_back
    local c; read -rp "$(ui_prompt "选择节点编号" "0-$((n-1))")" c
    [[ "$c" == "0" || -z "$c" ]] && return 1
...
edit_special_node() {
    case "$1" in
        snell)
            ...
            ;;
        mieru)
            warn "Mieru 节点请在节点列表中直接选择进行编辑 (支持多节点共存)。"
            ;;
        wireguard)
...
edit_single_node() {
    ...
    EDIT_LABELS=()
    EDIT_FUNCS=()
    if [[ "$core" == "snell-managed" || "$core" == "snell-v6-managed" || "$core" == "snell-shadowtls" || "$core" == "wireguard-managed" ]]; then
        add_edit_option "修改备注名" "change_node_name"
    elif [[ "$core" == "mieru-managed" ]]; then
        add_edit_option "修改备注名" "change_node_name"
        add_edit_option "修改客户端展示入口" "set_mieru_node_endpoint"
        add_edit_option "重新配置 (端口/协议/账号/MTU)" "reconfigure_mieru_node"
    else
        ...
        case "$type" in
            ...
            VMess-TCP) add_edit_option "修改伪装类型" "change_node_obfs_type" ;;
        esac
    fi

    ui_head "编辑节点  ${G_SEP}  ${type}  ${G_SEP}  ${name}" "$C_WARNC"
    ui_note "当前端口 ${port:-未知}"
    for i in "${!EDIT_LABELS[@]}"; do
        ui_item "$((i+1))" "${EDIT_LABELS[$i]}" "" "$C_WARNC"
    done
    ui_item_back
    read -rp "$(ui_prompt "请选择" "0-${#EDIT_LABELS[@]}")" c
    [[ "$c" == "0" || -z "$c" ]] && return 0
...
edit_node_menu() {
    ui_head "编辑节点配置" "$C_WARNC"
    ui_item 1 "修改节点地址, 域名或 IP" "批量重写全部分享链接" "$C_WARNC"
    ui_item 2 "选择节点并编辑配置"     "端口/UUID/密码/SNI 等" "$C_WARNC"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-2")" c
    case "$c" in
        1) set_node_host ;;
...
uninstall_menu() {
    ui_head "卸载" "$C_ERRC"
    ui_note "卸载操作不可撤销, 请确认已备份需要保留的节点链接"
    ui_item 1 "卸载 sing-box" "同时删除其名下节点" "$C_WARNC"
    ui_item 2 "卸载 Xray"     "同时删除其名下节点" "$C_WARNC"
    ui_item 3 "卸载 Snell"    ""                   "$C_WARNC"
    ui_item 4 "卸载 Mieru"    ""                   "$C_WARNC"
    ui_item 5 "卸载 WireGuard" ""                  "$C_WARNC"
    ui_item 6 "卸载端口流量计费管理" "清理 nftables/tc 规则" "$C_WARNC"
    ui_item 7 "全部卸载"      "清空内核 · 节点 · 计费与定时任务" "$C_ERRC"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-7")" c
    case "$c" in
...
uninstall_all() {
    ...
    rm -f "$SB_BIN" "$XRAY_BIN" "$SNELL_BIN" "$SNELL_V6_BIN" "$SHADOWTLS_BIN" /usr/local/bin/snell-server-v4 /usr/local/bin/snell-server-v5 /usr/local/bin/mita /usr/bin/mita
    rm -f "$SHORTCUT" /tmp/pm_check.log
    rm -f "$WG_CONF" "$SNELL_STATE" "$MIERU_STATE" "$WG_STATE"
    rm -rf "$SB_DIR" "$XRAY_DIR" "$XRAY_SHARE" "$(dirname "$SNELL_CONF")" "$SNELL_V6_DIR" /etc/mita "${MIERU_CLIENT_DIR%/current}" "$STATE_DIR"
...
restart_all() {
    ...
    if [[ -f "$STATE" ]]; then
        ...
        while IFS= read -r t; do
            [[ -z "$t" ]] && continue
            port="${t#snell-v6-}"
            svc="$(snell_v6_service "$port")"
            systemctl restart "$svc" 2>/dev/null
            if systemctl is-active --quiet "$svc" 2>/dev/null; then
                ok "Snell v6 ${port} 服务运行中。"
            else
                err "Snell v6 ${port} 启动失败! 最近日志:"
                journalctl -u "$svc" -n 10 --no-pager 2>/dev/null | sed 's/^/    /'
            fi
            ((count++))
        done <<< "$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-v6-managed") | .key' "$STATE" 2>/dev/null)"
...
core_service_block() {
    ...
    printf '%b\n' "\n  ${BOLD}${C_TXT}${name}${NC}   ${status}"
    [[ -n "$version" ]] && ui_kv "版本" "$version" 6
    [[ -n "$service" ]] && ui_kv "服务" "$service" 6
    [[ -n "$config" ]]  && ui_kv "配置" "$config" 6
    [[ -n "$extra" ]]   && ui_kv "其他" "$extra" 6
    return 0
}

core_not_installed_block() {
    local name="$1"
    printf '%b\n' "\n  ${BOLD}${C_MUT}${name}${NC}   $(ui_badge none)"
}
...
show_core_service_info() {
    ...
    if mieru_installed; then
        local mcount
        mcount="$(jq -r '[.nodes | to_entries[] | select(.value.core == "mieru-managed")] | length' "$STATE" 2>/dev/null)"
        core_service_block "Mieru" "$(service_state_text "$MITA_SERVICE")" "$(mieru_version)" "$MITA_SERVICE" "" "节点数: ${mcount:-0}"
    fi
    ...
    ui_head "节点概览" "$C_WARNC"
    if [[ -f "$STATE" ]]; then
        local snell_ports snell_v6_ports stls_ports mieru_ports wg_peers
        snell_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-managed") | .key | sub("^snell-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$snell_ports" ]] && ui_kv "Snell 节点" "端口 ${snell_ports}" 14
        snell_v6_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-v6-managed") | .key | sub("^snell-v6-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$snell_v6_ports" ]] && ui_kv "Snell v6 RC" "端口 ${snell_v6_ports}" 14
        stls_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-shadowtls") | .key | sub("^snell-shadowtls-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$stls_ports" ]] && ui_kv "Snell+STLS" "端口 ${stls_ports}" 14
        mieru_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "mieru-managed") | .key | sub("^mieru-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$mieru_ports" ]] && ui_kv "Mieru 节点" "端口 ${mieru_ports}" 14
        wg_peers="$(jq -r '[.nodes | to_entries[] | select(.value.core == "wireguard-managed")] | length' "$STATE" 2>/dev/null)"
        [[ -n "$wg_peers" && "$wg_peers" != "0" ]] && ui_kv "WG Peers" "数量 ${wg_peers}" 14
    fi
    if shadowtls_installed; then
        ui_kv "ShadowTLS" "$(shadowtls_version)  ${G_SEP}  ${SHADOWTLS_BIN}" 14
    fi
    ui_kv "节点数量" "$(node_count)" 14
}

core_manage_menu() {
    clear 2>/dev/null || true
    ui_refresh_cols
    ui_screen "内核与服务管理" "安装 · 更新 · 重启"
    show_core_service_info
    ui_head "操作" "$C_WARNC"
    ui_item 1 "安装 / 更新 Xray 内核"     "VLESS/VMess/Trojan/SS 系" "$C_WARNC"
    ui_item 2 "安装 / 更新 sing-box 内核" "Hysteria2/TUIC/AnyTLS 系" "$C_WARNC"
    ui_item 3 "重启所有服务"              "按内核逐个重启并校验"     "$C_WARNC"
    ui_item_back
    hr
    local c; read -rp "$(ui_prompt "请选择" "0-3")" c
    case "$c" in
...
script_update_menu() {
    ui_head "更新脚本" "$C_WARNC"
    ui_item 1 "手动同步最新脚本" "从 GitHub 拉取并重启菜单" "$C_WARNC"
    ui_item 2 "自动同步脚本" "当前: $( [[ "$(auto_update_setting)" == "true" ]] && echo 开启 || echo 关闭 )" "$C_WARNC"
    ui_item_back
    local c
    read -rp "$(ui_prompt "请选择" "0-2")" c
    case "$c" in
        1) update_script ;;
...
vless_menu() {
    ui_head "VLESS 协议" "$C_ACC"
    ui_item 1 "VLESS-Reality-Vision"   "推荐首选 · 无需域名"
    ui_item 2 "VLESS-XHTTP-Reality"    "Xray 独有 · 抗主动探测"
    ui_item 3 "VLESS-gRPC-Reality"     "多路复用 · 长连接稳定"
    ui_item 4 "VLESS-Encryption"       "后量子加密 · 新版内核"
    ui_item 5 "VLESS-Encryption-XHTTP" "后量子 + XHTTP 传输"
    ui_item 6 "VLESS-WS-TLS"           "可套 CDN · 需要域名"
    ui_item 7 "VLESS-gRPC-TLS"         "可套 CDN · 需要域名"
    ui_item 8 "VLESS-XHTTP-TLS"        "可套 CDN · 需要域名"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-8")" c
    case "$c" in
        ...
        8) add_vless_tls xhttp ;;
        *) return 0 ;;
    esac
}

finalmask_menu() {
    ui_head "FinalMask 抗审查" "$C_VIO"
    ui_note "需要 Xray v26.3.27+ , 面向强干扰网络环境"
    ui_item 1 "VLESS-Encryption-XHTTP-FinalMask" "流量整形 + 后量子加密" "$C_VIO"
    ui_item 2 "VLESS-Encryption-FinalMask sudoku" "sudoku 密码模式"     "$C_VIO"
    ui_item 3 "FullStack REALITY+XHTTP+FinalMask" "全栈叠加 · 最强伪装" "$C_VIO"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-3")" c
    case "$c" in
...
vmess_menu() {
    ui_head "VMess 协议" "$C_VIO"
    ui_item 1 "VMess-TCP"       "裸 TCP · 可选 HTTP 伪装" "$C_VIO"
    ui_item 2 "VMess-mKCP"      "UDP 加速 · 抗丢包"       "$C_VIO"
    ui_item 3 "VMess-WebSocket" "无 TLS · 适合内网中转"   "$C_VIO"
    ui_item 4 "VMess-WS-TLS"    "可套 CDN · 需要域名"     "$C_VIO"
    ui_item 5 "VMess-gRPC-TLS"  "可套 CDN · 需要域名"     "$C_VIO"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-5")" c
    case "$c" in
        1) add_vmess_plain tcp ;;
        2) add_vmess_plain kcp ;;
        3) add_vmess_ws ;;
        4) add_vmess_tls ws ;;
        5) add_vmess_tls grpc ;;
        *) return 0 ;;
    esac
}

trojan_menu() {
    ui_head "Trojan 协议" "$C_PINK"
    ui_item 1 "Trojan-Reality" "无需域名 · 借用真实站点" "$C_PINK"
    ui_item 2 "Trojan-TCP-TLS" "经典模式 · 需要证书"     "$C_PINK"
    ui_item 3 "Trojan-WS-TLS"  "可套 CDN · 需要域名"     "$C_PINK"
    ui_item 4 "Trojan-gRPC-TLS" "可套 CDN · 需要域名"    "$C_PINK"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-4")" c
    case "$c" in
...
proxy_menu() {
    ui_head "SOCKS5 / HTTP" "$C_ACC2"
    ui_note "明文代理, 建议仅在内网或配合其他加密隧道使用"
    ui_item 1 "SOCKS5"     "通用代理 · 支持账号密码" "$C_ACC2"
    ui_item 2 "HTTP Proxy" "浏览器/系统代理"         "$C_ACC2"
    ui_item_back
    local c; read -rp "$(ui_prompt "请选择" "0-2")" c
    case "$c" in
        1) add_socks ;;
...
ptm_report_text_to_rich_html() {
    local text="$1" line label value html=""
    local title_seen=false table_open=false
    while IFS= read -r line || [ -n "$line" ]; do
        if [ "$title_seen" = false ]; then
            [ -z "$line" ] && continue
            html+="<h4>$(ptm_html_escape "$line")</h4><hr/>"
            title_seen=true
            continue
        fi
        [[ "$line" == ━* ]] && continue
        ...
        if [[ "$line" == *:* ]]; then
            ...
            if [ "$table_open" = false ]; then
                html+="<table>"
                table_open=true
            fi
            html+="<tr><td><b>$(ptm_html_escape "$label")</b></td><td>:</td><td>$(ptm_html_escape "$value")</td></tr>"
        else
            if [ "$table_open" = true ]; then
                html+="</table>"
                table_open=false
            fi
            html+="<p>$(ptm_html_escape "$line")</p>"
        fi
    done <<< "$text"
    [ "$table_open" = true ] && html+="</table>"
    printf '%s' "$html"
}
...
ptm_prompt_report_schedule() {
    ...
    ui_sub "$prompt"
    ui_kv "当前设置" "$(ptm_report_schedule_label "$current")" 10
    if [[ "$allow_inherit" == "true" ]]; then
        ...
        ui_item 4 "每月最后一天推送"
        ui_item 5 "关闭该用户概览"
        read -e -p "$(ui_prompt "请选择" "1-5, 留空不改")" choice
        case "$choice" in
        ...
        ui_item 3 "每月最后一天推送"
        ui_item 4 "关闭概览推送"
        read -e -p "$(ui_prompt "请选择" "1-4, 留空不改")" choice
        case "$choice" in
            1) normalized="daily" ;;
            2) normalized="weekly" ;;
...
ptm_build_port_plain_message() {
...

    {
        printf '%s\n' "$title"
        printf '━━━━━━━━━━━━━━━━\n'
        ptm_report_line "端口" "$port"
        ptm_report_line "备注" "${remark:-无}"
        ptm_report_line "状态" "$status"
        ptm_report_line "计费模式" "$billing_mode"
        ptm_report_line "已用流量" "$usage"
        ptm_report_line "流量配额" "$quota"
        ptm_report_line "剩余流量" "$remaining"
        ptm_report_line "使用比例" "${usage_percent}%"
        ptm_report_line "到期时间" "$expire"
        ptm_report_line "流量重置日期" "$reset_day"
    }
}
...
ptm_build_port_user_message() {
...

    {
        printf '%s\n' "$title"
        printf '━━━━━━━━━━━━━━━━\n'
        ptm_report_line "节点" "${remark:-未设置备注}"
        ptm_report_line "状态" "$status"
        ptm_report_line "计费模式" "$billing_mode"
        ptm_report_line "已用流量" "$usage"
        ptm_report_line "流量配额" "$quota"
...
ptm_send_telegram_pre_to_chat() {
    local chat_id="$1" text="$2"
    local enabled bot_token escaped json_body response
    enabled=$(jq -r '.notify.telegram.enabled // false' "$PTM_CONFIG_FILE" 2>/dev/null)
    [ "$enabled" = "true" ] || return 1
    bot_token=$(jq -r '.notify.telegram.bot_token // ""' "$PTM_CONFIG_FILE" 2>/dev/null)
    [ -z "$bot_token" ] || [ "$bot_token" = "null" ] || [ -z "$chat_id" ] || [ "$chat_id" = "null" ] || [ -z "$text" ] && return 1

    escaped=$(ptm_html_escape "$text")
    json_body=$(jq -n --arg chat_id "$chat_id" --arg text "<pre>${escaped}</pre>" \
...
ptm_send_telegram_rich_report_to_chat() {
    ptm_send_telegram_pre_to_chat "$1" "$2"
}
...
ptm_send_telegram_long() {
    ...
    if [ "${#text}" -le 3500 ]; then
        ptm_send_telegram_rich_report_to_chat "$chat_id" "$text"
        return $?
    fi
    local chunk sent=false
    while [ -n "$text" ]; do
        chunk="${text:0:3500}"
        if ptm_send_telegram_pre_to_chat "$chat_id" "$chunk"; then
            sent=true
        fi
        text="${text:3500}"
    done
...
ptm_build_ports_summary_text() {
    ...
    for port in $ports; do
        ...
        reset_day=$(ptm_format_reset_day_text "$reset_day")
        usage_percent=$(ptm_get_port_usage_percent "$port")
        [ -z "$remark" ] || [ "$remark" = "null" ] && remark="无"
        msg+=$'\n'
        msg+="$(ptm_report_line "端口" "$port")"$'\n'
        msg+="$(ptm_report_line "备注" "$remark")"$'\n'
        msg+="$(ptm_report_line "状态" "$status")"$'\n'
        msg+="$(ptm_report_line "计费模式" "$billing_mode")"$'\n'
        msg+="$(ptm_report_line "已用流量" "$usage")"$'\n'
        msg+="$(ptm_report_line "流量配额" "$quota")"$'\n'
        msg+="$(ptm_report_line "剩余流量" "$remaining")"$'\n'
        msg+="$(ptm_report_line "使用比例" "${usage_percent}%")"$'\n'
        msg+="$(ptm_report_line "到期时间" "$expire")"$'\n'
        msg+="$(ptm_report_line "流量重置日期" "$reset_day")"$'\n'
    done
    echo "$msg"
}
...
ptm_build_user_ports_summary_text() {
    ...
    for port in $ports; do
        ...
        reset_day=$(ptm_format_reset_day_text "$reset_day")
        usage_percent=$(ptm_get_port_usage_percent "$port")
        [ -z "$remark" ] || [ "$remark" = "null" ] && remark="未设置备注"
        msg+=$'\n'
        msg+="$(ptm_report_line "序号" "$index")"$'\n'
        msg+="$(ptm_report_line "节点" "$remark")"$'\n'
        msg+="$(ptm_report_line "状态" "$status")"$'\n'
        msg+="$(ptm_report_line "计费模式" "$billing_mode")"$'\n'
        msg+="$(ptm_report_line "已用流量" "$usage")"$'\n'
        msg+="$(ptm_report_line "流量配额" "$quota")"$'\n'
    ...
    for port in $(ptm_get_active_ports); do
        ...
        if [ "$today_epoch" -ge "$warning_epoch" ] && [ "$today_epoch" -lt "$expire_epoch" ]; then
            ...
            if [ "$last_warning_target" != "$expire_date" ]; then
                ...
                local admin_msg
                admin_msg="租期预警"$'\n'"━━━━━━━━━━━━━━━━"$'\n'
                admin_msg+="$(ptm_report_line "端口" "$port")"$'\n'
                admin_msg+="$(ptm_report_line "到期时间" "$expire_date")"$'\n'
                ptm_send_telegram_to_admin "$admin_msg"
            fi
        fi
        ...
        if [ "$today_epoch" -gt "$expire_epoch" ]; then
            ...
            if ptm_is_port_rules_exist "$port"; then
                ...
                admin_msg+="$(ptm_report_line "端口" "$port")"$'\n'
                admin_msg+="$(ptm_report_line "到期时间" "$expire_date")"$'\n'
                admin_msg+="$(ptm_report_line "状态" "已停机")"$'\n'
                ptm_send_telegram_to_admin "$admin_msg"
            fi
            ptm_block_port "$port"
...
ptm_cleanup_expired_port() {
    ...
    admin_msg="自动清理"$'\n'"━━━━━━━━━━━━━━━━"$'\n'
    admin_msg+="$(ptm_report_line "端口" "$port")"$'\n'
    admin_msg+="$(ptm_report_line "备注" "${remark}")"$'\n'
    admin_msg+="$(ptm_report_line "到期时间" "${expire_date}")"$'\n'
    admin_msg+="$(ptm_report_line "状态" "已回收")"$'\n'
    ptm_send_telegram_to_admin "$admin_msg"
...
发送 /cancel 取消。"
        ...
        remark)
            ...
            ptm_send_telegram_long "$chat_id" "$(ptm_build_port_plain_message "$port" "用户绑定信息")"
            ptm_send_telegram_rich_report_to_chat "$target_chat" "$(ptm_build_port_user_message "$port" "绑定成功")" || true
            ;;
        *)
            ptm_telegram_clear_session "$chat_id"
...
ptm_telegram_renew_port() {
    ...
    if ptm_do_renew_months "$port" "$months" >/dev/null 2>&1; then
        ptm_send_telegram_long "$chat_id" "$(ptm_build_port_plain_message "$port" "续期成功")"
    ...
    case "$expire_date" in
        permanent|forever|永久)
            if ptm_update_config ".ports.\"$port\".expiration_date = \"\""; then
                ...
                ptm_send_telegram_long "$chat_id" "$(ptm_build_port_plain_message "$port" "已设置为永久")"
            else
        ...
        *)
            ...
            if ptm_do_set_expiration "$port" "$expire_date" >/dev/null 2>&1; then
                ptm_send_telegram_long "$chat_id" "$(ptm_build_port_plain_message "$port" "到期日已更新")"
...
ptm_menu_add_port() {
    ptm_init_config
    ptm_check_dependencies
    ui_head "快速开通端口" "$C_ACC2"
    ui_note "依次引导: 添加端口 → 设置重置日期 → 设置租期 → 设置邮箱/TG"

    ui_step 1 4 "添加端口监控"
    ui_note "格式: 单端口 40001 / 端口段 8000-8100 / 端口组 101,102,105 (共享流量统计)"
    ptm_prompt_monitor_port
    ...
    ui_sub "统计模式"
    ui_item 1 "双向流量统计" "推荐 · (入站 + 出站) × 2"
    ui_item 2 "仅出站统计"   "出站 × 2"
    ui_item 3 "CN Premium 内网中转" "(入站 + 出站) × 1"
    read -e -p "$(ui_prompt "请选择统计模式" "1-3, 回车=1")" billing_choice
    ...
    ui_note "按回车继续"
    read -r _
    ui_note "为端口 ${port} 设置月重置日期, 0 代表不重置, 1-31 为每月重置日"
    read -e -p "$(ui_prompt "月重置日" "0-31")" reset_day_input
    ...
    ui_note "按回车继续"
    read -r _
    read -e -p "$(ui_prompt "是否设置用户邮箱" "y/N, 可后续补充")" email_choice
    ...
    echo
    ui_rule "$G_HH"
    printf '%b\n' "  ${BOLD}${C_OKC}${G_OK} 快速开通流程完成${NC}   ${C_DIM2}端口 ${port} 已进入监控与计费${NC}"
    ui_rule "$G_H"
    break_end
}
...
ptm_render_port_table() {
    ...
    local port
    for port in $ports; do
        local remark billing_mode quota_limit expire_date status usage remaining reset_day usage_percent
        remark=$(jq -r ".ports.\"$port\".remark // \"\"" "$PTM_CONFIG_FILE")
        billing_mode=$(jq -r ".ports.\"$port\".billing_mode // \"double\"" "$PTM_CONFIG_FILE")
        billing_mode=$(ptm_format_billing_mode "$billing_mode")
        quota_limit=$(jq -r ".ports.\"$port\".quota.monthly_limit // \"unlimited\"" "$PTM_CONFIG_FILE")
        ...
        remaining=$(ptm_get_port_remaining_text "$port")
        reset_day=$(jq -r ".ports.\"$port\".quota.reset_day // \"未设置\"" "$PTM_CONFIG_FILE")
        reset_day=$(ptm_format_reset_day_text "$reset_day")
        usage_percent=$(ptm_get_port_usage_percent "$port")
        [ -z "$remark" ] || [ "$remark" = "null" ] && remark="无"
        [ "$quota_limit" = "unlimited" ] && quota_limit="无限制"
        [ -z "$expire_date" ] || [ "$expire_date" = "null" ] && expire_date="永久"
        echo
        ui_card_top "端口 ${port}  ${G_SEP}  ${remark}" "$C_ACC"
        ui_card_row "${C_KEY}状态${NC} ${status}    ${C_KEY}计费${NC} ${C_VAL}${billing_mode}${NC}"
        ui_card_row "${C_KEY}用量${NC} $(ui_bar "$usage_percent" 18) ${C_VAL}${usage_percent}%${NC}   ${C_VAL}${usage}${NC} ${C_DIM2}/${NC} ${C_VAL}${quota_limit}${NC}"
        ui_card_row "${C_KEY}剩余${NC} ${C_VAL}${remaining}${NC}"
        ui_card_row "${C_KEY}到期${NC} ${C_VAL}${expire_date}${NC}    ${C_KEY}重置日${NC} ${C_VAL}${reset_day}${NC}"
        ui_card_end
    done
}
...
ptm_lease_prompt_and_apply() {
    ...
    ui_sub "租期设置"
    ui_kv "当前到期日" "$current_expire" 12
    ui_item 1 "增加 1 个月"
    ui_item 2 "增加 3 个月" "季付"
    ui_item 3 "增加 6 个月" "半年"
    ui_item 4 "增加 1 年"
    ui_item 5 "手动输入到期日期"
    ui_item 6 "清除租期" "设置为永久"
    ui_item_back "跳过"
    read -e -p "$(ui_prompt "请选择续费时长" "0-6")" duration_choice
...
ptm_menu_configure_email() {
    ptm_init_config
    ui_head "邮件通知设置 (Resend API)" "$C_ACC2"
...
ptm_menu_configure_telegram() {
    ptm_init_config
    ui_head "Telegram Bot 通知设置" "$C_ACC2"
...
ptm_menu_bind_telegram_chat() {
    ptm_init_config
    ui_head "绑定端口 Telegram 用户" "$C_ACC2"
    ...
    if ptm_update_config ".ports.\"$port\".telegram_chat_id = $chat_json"; then
        ...
        ptm_send_telegram_rich_report_to_chat "$user_chat" "$(ptm_build_port_user_message "$port" "绑定成功")" || true
    else
...
ptm_menu_configure_notify() {
    ...
    while true; do
        ...
        ptm_report_line "邮箱通知" "$email_enabled"
        ptm_report_line "Telegram" "$tg_enabled"
        ptm_report_line "管理员" "${tg_admin:-未设置}"
        ptm_report_line "概览推送" "$tg_report/$(ptm_report_schedule_label "$tg_schedule")"
        ptm_report_line "服务" "$tg_service"
        echo ""
        ui_item 1 "邮件通知设置"
        ui_item 2 "Telegram Bot 设置"
        ui_item 3 "绑定端口 Telegram 用户"
        ui_item 4 "设置用户概览推送周期"
        ui_item 5 "发送 Telegram 测试通知"
        ui_item 6 "启动/重启 Telegram Bot 服务"
        ui_item 7 "停止 Telegram Bot 服务"
        ui_item_back "返回"
        local choice
        read -e -p "$(ui_prompt "请选择" "0-7")" choice
        case "$choice" in
...
ptm_menu_diagnose() {
    ...
    for port in $ports; do
        ...
        [ -z "$email" ] || [ "$email" = "null" ] && diag+="⚠️未配置客户邮箱 "
        local tg_chat
        tg_chat=$(ptm_get_telegram_chat_for_port "$port")
        [ -z "$tg_chat" ] || [ "$tg_chat" = "null" ] && diag+="⚠️未绑定TG "
        [ "$ok" = true ] && [ -z "$diag" ] && diag="✅正常"
        echo
        ui_card_top "端口 ${port}" "$C_ACC"
        ui_card_row "${C_KEY}诊断结果${NC}  ${C_VAL}${diag}${NC}"
        ui_card_end
    done
    echo ""
...
ptm_menu_port_monitoring() {
    while true; do
        clear
        ui_refresh_cols
        ui_screen "端口监控管理" "开通 · 删除 · 合并"
        ui_item 1 "快速开通端口"
        ui_item 2 "删除端口监控"
        ui_item 3 "合并端口为组"
        ui_item_back "返回"
        local choice
        read -e -p "$(ui_prompt "请选择" "0-3")" choice
        case "$choice" in
            1) ptm_menu_add_port ;;
...
ptm_menu_limits() {
    while true; do
        clear
        ui_refresh_cols
        ui_screen "端口限制设置管理" "带宽 · 配额 · 租期"
        ui_item 1 "设置端口带宽限制（速率控制）"
        ui_item 2 "设置端口流量配额（总量控制）"
        ui_item 3 "管理端口租期（自动到期停机）"
        ui_item_back "返回"
        local choice
        read -e -p "$(ui_prompt "请选择" "0-3")" choice
        case "$choice" in
...
ptm_menu_reset_mgmt() {
    while true; do
        clear
        ui_refresh_cols
        ui_screen "流量重置管理" "月重置日 · 立即重置"
        ui_item 1 "重置流量月重置日设置"
        ui_item 2 "立即重置"
        ui_item_back "返回"
        local choice
        read -e -p "$(ui_prompt "请选择" "0-2")" choice
        case "$choice" in
...
ptm_menu() {
    ...
    while true; do
        ...
        ui_refresh_cols
        ui_screen "端口流量计费与到期管理" "nftables 计数 · tc 限速 · 到期停机"
        printf '%b\n' "   ${C_KEY}监控端口${NC} ${BOLD}${C_TXT}${port_count}${NC} ${C_DIM2}个${NC}    ${C_KEY}端口总流量${NC} ${BOLD}${C_TXT}${daily_total}${NC}"
        ptm_render_port_table
        ui_head "管理操作" "$C_WARNC"
        ui_item 1 "添加/删除端口监控" "开通 · 删除 · 合并端口组" "$C_WARNC"
        ui_item 2 "端口限制设置管理"   "带宽 · 配额 · 租期"       "$C_WARNC"
        ui_item 3 "流量重置管理"       "月重置日 · 立即重置"      "$C_WARNC"
        ui_item 4 "通知管理"           "邮件 · Telegram Bot"      "$C_WARNC"
        ui_item 5 "配置诊断"           "依赖 · 定时任务 · 通知"   "$C_WARNC"
        ui_item_back "返回主菜单"
        hr
        local choice
        read -e -p "$(ui_prompt "请选择" "0-5")" choice
        case $choice in
...
#  十四、状态面板 / 主菜单
# ===========================================================================
# 顶部品牌条: 渐变光带 + 逐字渐变标题 + 环境元信息
ui_banner() {
    local meta="v${SCRIPT_VERSION}   ${G_SEP}   ${OS_ID:-linux}/${ARCH:-unknown}   ${G_SEP}   快捷命令 pm"
    ui_rule "$G_HH"
    printf '%b\n' "  ${BOLD}$(ui_grad_text 'PROXY  NODE  MANAGER')${NC}"
    printf '%b\n' "  ${C_MUT}Sing-box ${C_VIO}×${C_MUT} Xray${NC}  ${C_DIM2}${G_SEP}${NC}  ${C_MUT}多协议节点部署与运维控制台${NC}"
    printf '%b\n' "  ${C_DIM2}${meta}${NC}"
    ui_rule "$G_H"
}
...
show_status() {
    ui_head "运行状态" "$C_ACC"
    if sb_installed; then
        if sb_running; then ui_svc_row "sing-box" on "$(sb_version)"
        else ui_svc_row "sing-box" off "$(sb_version)"; fi
    else ui_svc_row "sing-box" none; fi
    if xray_installed; then
        if xray_running; then ui_svc_row "Xray" on "$(xray_version)"
        else ui_svc_row "Xray" off "$(xray_version)"; fi
    else ui_svc_row "Xray" none; fi
    if snell_installed; then
        local snell_ver=""; snell_load_state && snell_ver="v${SNELL_VERSION}"
        if snell_running; then ui_svc_row "Snell" on "$snell_ver"
        else ui_svc_row "Snell" off "$snell_ver"; fi
    fi
    if [[ -x "$SNELL_V6_BIN" ]] || snell_v6_managed_exists; then
        local v6_ports; v6_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-v6-managed") | .key | sub("^snell-v6-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$v6_ports" ]] && ui_svc_row "Snell v6 RC" set "端口 ${v6_ports}"
    fi
    if mieru_installed; then
        local mieru_ver; mieru_ver="$(mieru_version)"
        if mieru_running; then ui_svc_row "Mieru" on "$mieru_ver"
        else ui_svc_row "Mieru" off "$mieru_ver"; fi
    fi
    if wg_installed; then
        local wg_ver; wg_ver="$(wg_version)"
        if wg_running; then ui_svc_row "WireGuard" on "$wg_ver"
        else ui_svc_row "WireGuard" off "$wg_ver"; fi
    fi
    if [[ -f "$STATE" ]]; then
        local snell_ports snell_v6_ports stls_ports mieru_ports wg_peers
        snell_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-managed") | .key | sub("^snell-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$snell_ports" ]] && ui_svc_row "Snell 节点" set "端口 ${snell_ports}"
        snell_v6_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-v6-managed") | .key | sub("^snell-v6-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$snell_v6_ports" ]] && ui_svc_row "Snell v6 RC" set "端口 ${snell_v6_ports}"
        stls_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "snell-shadowtls") | .key | sub("^snell-shadowtls-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$stls_ports" ]] && ui_svc_row "Snell+STLS" set "端口 ${stls_ports}"
        mieru_ports="$(jq -r '.nodes | to_entries[] | select(.value.core == "mieru-managed") | .key | sub("^mieru-"; "")' "$STATE" 2>/dev/null | paste -sd, -)"
        [[ -n "$mieru_ports" ]] && ui_svc_row "Mieru 节点" set "端口 ${mieru_ports}"
        wg_peers="$(jq -r '[.nodes | to_entries[] | select(.value.core == "wireguard-managed")] | length' "$STATE" 2>/dev/null)"
        [[ -n "$wg_peers" && "$wg_peers" != "0" ]] && ui_svc_row "WG Peers" set "数量 ${wg_peers}"
    fi
    ui_pad "节点总数" 12
    printf '%b\n' "   ${C_TXT}节点总数${NC}${UI_PAD}${BOLD}${C_ACC}$(node_count)${NC} ${C_DIM2}个已保存节点${NC}"
}
...
show_menu() {
    ...
    show_status
    ui_head "安装协议" "$C_ACC"
    ui_item2  1 "VLESS 协议"        2 "FinalMask 抗审查"
    ui_item2  3 "VMess 协议"        4 "Trojan 协议"
    ui_item2  5 "Shadowsocks"       6 "SOCKS5 / HTTP"
    ui_item2  7 "Hysteria2"         8 "TUIC v5"
    ui_item2  9 "AnyTLS"           10 "ShadowTLS v3"
    ui_item2 11 "NaïveProxy"       12 "Snell"
    ui_item2 13 "Mieru"            14 "WireGuard"
    ui_head "节点与系统管理" "$C_WARNC"
    ui_item2 15 "查看全部节点"     16 "编辑节点配置"           "$C_WARNC"
    ui_item2 17 "删除指定节点"     18 "端口流量计费与到期管理" "$C_WARNC"
    ui_item2 19 "内核与服务管理"   20 "更新脚本"               "$C_WARNC"
    ui_item2 21 "卸载"              22 "大陆直连屏蔽"            "$C_WARNC"
    ui_item 23 "出站 IP 协议优先"    "IPv4/IPv6 解析优先级 (Xray+sing-box)" "$C_WARNC"
    echo
    ui_item_back "退出"
    echo
    ui_rule "$G_H"
...
main_loop() {
    while true; do
        show_menu
        read -rp "$(ui_prompt "请输入选项" "0-23")" choice
        echo
        case "$choice" in
            1)  vless_menu ;;
            2)  finalmask_menu ;;
            ...
            21) uninstall_menu ;;
            22) configure_xray_cn_block ;;
            23) configure_outbound_strategy ;;
            0)  ok "感谢使用, 再见!"; exit 0 ;;
            *)  err "无效选项, 请输入 0-23。" ;;
        esac
        pause
