...
发送 /cancel 取消。"
        ...
        remark)
            ...
            ptm_send_telegram_pre_to_chat "$target_chat" "$(ptm_build_port_user_message "$port" "绑定成功")" || true
            ;;
        *)
            ptm_telegram_clear_session "$chat_id"
...
ptm_menu_bind_telegram_chat() {
    ...
    if ptm_update_config ".ports.\"$port\".telegram_chat_id = $chat_json"; then
        ...
        ptm_send_telegram_pre_to_chat "$user_chat" "$(ptm_build_port_user_message "$port" "绑定成功")" || true
    else
