...
EOF
    ...
    local pc; read -rp "$(echo -e "${CYAN}选择 (默认: 1): ${NC}")" pc
    local proto
    ...
    ask_int_default "Mieru MTU" "1400" 1280 9000; local mtu="$REPLY_VALUE"
...
install_wireguard() {
    ...
    ask_plain_default "出口网卡" "$iface"; iface="$REPLY_VALUE"
    ask_plain_default "WireGuard 服务端内网 IP" "10.66.66.1"; local s_addr="$REPLY_VALUE"
    ask_plain_default "WireGuard 客户端内网 IP" "10.66.66.2"; local c_addr="$REPLY_VALUE"
    ask_plain_default "WireGuard 客户端 DNS" "1.1.1.1, 8.8.8.8"; local dns="$REPLY_VALUE"
...
main_loop() {
    while true; do
        show_menu
        read -rp "$(echo -e "${CYAN}请输入选项 [0-20]: ${NC}")" choice
        echo
        case "$choice" in
            1)  vless_menu ;;
            2)  finalmask_menu ;;
