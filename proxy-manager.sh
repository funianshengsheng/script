...
ptm_send_telegram_rich_report_to_chat() {
    local chat_id="$1" text="$2"
    local enabled bot_token html json_body response
    enabled=$(jq -r '.notify.telegram.enabled // false' "$PTM_CONFIG_FILE" 2>/dev/null)
    [ "$enabled" = "true" ] || return 1
    bot_token=$(jq -r '.notify.telegram.bot_token // ""' "$PTM_CONFIG_FILE" 2>/dev/null)
    [ -z "$bot_token" ] || [ "$bot_token" = "null" ] || [ -z "$chat_id" ] || [ "$chat_id" = "null" ] || [ -z "$text" ] && return 1

    html=$(ptm_report_text_to_rich_html "$text")
    [ -n "$html" ] || return 1
    json_body=$(jq -n --arg chat_id "$chat_id" --arg html "$html" \
        '{chat_id: $chat_id, rich_message: {html: $html, skip_entity_detection: true}}')
    response=$(curl -s --connect-timeout 10 --max-time 30 \
        -X POST "https://api.telegram.org/bot${bot_token}/sendRichMessage" \
        -H "Content-Type: application/json" -d "$json_body" 2>/dev/null)
    if echo "$response" | jq -e '.ok == true' >/dev/null 2>&1; then
        ptm_log_notification "[TG富文本] 发送成功: chat_id=${chat_id}"
        return 0
    fi
    ptm_log_notification "[TG富文本] 发送失败，回退等宽消息: chat_id=${chat_id}"
    ptm_send_telegram_pre_to_chat "$chat_id" "$text"
}
