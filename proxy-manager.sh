...
ptm_report_line() {
    local label="$1" value="${2:-}" pad="" fw_space="　"
    case "$label" in
        端口|备注|状态|节点|序号|服务) pad="${fw_space}${fw_space}${fw_space}${fw_space}" ;;
        管理员|到期日) pad="${fw_space}${fw_space}${fw_space}" ;;
        计费模式|已用流量|流量配额|剩余流量|使用比例|到期时间|邮箱通知|概览推送|诊断结果) pad="${fw_space}${fw_space}" ;;
        流量重置日期) pad="" ;;
