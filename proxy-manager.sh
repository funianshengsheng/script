...
add_vless_tls() {
    ...
    echo -e "\n${MAGENTA}${BOLD}${label}${NC} 自签证书模式, 分享链接将携带证书指纹"
    ask_port "监听端口"; local port="$REPLY_PORT"
...
ask_mieru_traffic_pattern() {
    ...
    echo -e "${CYAN}流量伪装 (mita v3.28.0+ 支持):${NC}"
    echo -e "  ${GREEN}1.${NC} 关闭   ${GREEN}2.${NC} 保守 ${DIM}(推荐)${NC}   ${GREEN}3.${NC} 激进 ${DIM}(抗封锁更强, 开销更大)${NC}"
    read -rp "$(echo -e "${CYAN}选择 (回车使用: ${def}): ${NC}")" c
    case "${c:-$def}" in
        1) MIERU_TRAFFIC="off" ;;
...
ask_mieru_node_params() {
    ...
    read -rp "$(echo -e "${CYAN}选择 (回车使用: ${def_choice}): ${NC}")" pc
...
vless_menu() {
    ...
    echo -e "  ${GREEN}8.${NC} VLESS-XHTTP-TLS"
    echo -e "  ${GREEN}0.${NC} 返回"
    local c; read -rp "$(echo -e "${CYAN}请选择: ${NC}")" c
    case "$c" in
...
vmess_menu() {
    ...
    echo -e "  ${GREEN}3.${NC} VMess-WebSocket"
    echo -e "  ${GREEN}4.${NC} VMess-WS-TLS"
    echo -e "  ${GREEN}5.${NC} VMess-gRPC-TLS"
    echo -e "  ${GREEN}0.${NC} 返回"
    local c; read -rp "$(echo -e "${CYAN}请选择: ${NC}")" c
    case "$c" in
...
ptm_render_port_table() {
    ...
    for port in $ports; do
        ...
        echo "━━━━━━━━━━━━━━━━"
        ptm_report_line "端口" "$port"
        ptm_report_line "备注" "$remark"
        ptm_report_line "状态" "$status"
        ptm_report_line "计费模式" "$billing_mode"
        ptm_report_line "已用流量" "$usage"
        ptm_report_line "流量配额" "$quota_limit"
        ptm_report_line "剩余流量" "$remaining"
        ptm_report_line "使用比例" "${usage_percent}%"
        ptm_report_line "到期时间" "$expire_date"
        ptm_report_line "流量重置日期" "$reset_day"
    done
}
