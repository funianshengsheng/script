...
CONFIG_FILE="/etc/aether-ipv6-ddns/config"
...
# 实例选择:
# 1 Hong-Kong       -> wg_interface=tw
# 2 Dallas          -> wg_interface=wg0
# 3 Manassas        -> wg_interface=wg0
# 4 Los-Angeles     -> wg_interface=wg0
# 5 New-Jersey      -> wg_interface=wg0
# 6 Los-Angeles(CO) -> wg_interface=wg0
...
STATE_DIR="/var/lib/aether-ipv6-ddns"
LAST_IP_FILE="${STATE_DIR}/last_ipv6"
ALERT_STATE_FILE="${STATE_DIR}/last_alert_state"

SCRIPT_INSTALL_PATH="/usr/local/bin/aether-ipv6-ddns.sh"

VERBOSE=1
LOG_DIR="/var/log/aether-ipv6-ddns"
LOG_FILE="${LOG_DIR}/aether-ipv6-ddns.log"
LOG_MAX_SIZE_MB=10
...
rotate_logs_if_needed() {
  ...
  if [[ "$current_size" =~ ^[0-9]+$ ]] && (( current_size >= max_bytes )); then
    ...
    rotated_file="${LOG_DIR}/aether-ipv6-ddns-${ts}.log"
...
cleanup_old_logs_once_per_day() {
  ...
  while IFS= read -r _; do
    ((deleted_count++)) || true
  done < <(find "$LOG_DIR" -type f -name 'aether-ipv6-ddns*.log' -mtime +"$LOG_RETENTION_DAYS" -print -delete 2>/dev/null || true)
...
HK_WG_INTERFACE="${HK_WG_INTERFACE}"
US_WG_INTERFACE="${US_WG_INTERFACE}"
CFEOF
...
get_target_wg_interface() {
  case "$INSTANCE_PROFILE" in
    "Hong-Kong")
      echo "$HK_WG_INTERFACE"
      ;;
    "Dallas"|"Manassas"|"Los-Angeles"|"New-Jersey"|"Los-Angeles(CO)")
      echo "$US_WG_INTERFACE"
      ;;
    *)
      warn "未知实例 INSTANCE_PROFILE=$INSTANCE_PROFILE，默认按 Hong-Kong 处理"
      echo "$HK_WG_INTERFACE"
      ;;
  esac
}
...
send_tg_success() {
  ...
  local now_sh instance_name
  now_sh="$(TZ="${DISPLAY_TZ}" date '+%Y-%m-%d %H:%M:%S %Z')"
  instance_name="$(instance_label)"
  send_tg_message "✨ *${instance_name} IPv6 DDNS 更新成功*
...
send_tg_failure() {
  ...
  now_sh="$(TZ="${DISPLAY_TZ}" date '+%Y-%m-%d %H:%M:%S %Z')"
  instance_name="$(instance_label)"
  send_tg_message "❌ *${instance_name} IPv6 DDNS 故障告警*
...
send_tg_recovery() {
  ...
  now_sh="$(TZ="${DISPLAY_TZ}" date '+%Y-%m-%d %H:%M:%S %Z')"
  instance_name="$(instance_label)"
  send_tg_message "✅ *${instance_name} IPv6 DDNS 故障恢复*
...
install_systemd_timer() {
  if [[ "$ENABLE_AUTO_TIMER" != "1" ]]; then
    log_info "已禁用自动安装 systemd timer"
    return 0
  fi

  if [[ "$EUID" -ne 0 ]]; then
    warn "安装 systemd timer 需要 root，已跳过"
    return 0
  fi

  if ! command -v systemctl >/dev/null 2>&1; then
    warn "未找到 systemctl，已跳过 timer 安装"
    return 0
  fi
...
Description=Run Aether IPV6 DDNS every ${interval} minutes
...
WantedBy=timers.target"
  ...
  if [[ "$(cat "$SERVICE_FILE" 2>/dev/null || true)" != "$new_service_content" ]]; then
    echo "$new_service_content" > "$SERVICE_FILE"
    needs_reload=1
  fi

  if [[ "$(cat "$TIMER_FILE" 2>/dev/null || true)" != "$new_timer_content" ]]; then
  ...
  systemctl enable --now "${SERVICE_NAME}.timer" >/dev/null 2>&1 || {
    warn "启用 ${SERVICE_NAME}.timer 失败"
    return 0
  }
  log_ok "systemd timer 已启用，执行间隔: ${interval} 分钟"

  local ddns_cmd_content
  ddns_cmd_content="#!/usr/bin/env bash
exec ${SCRIPT_INSTALL_PATH} console"

  if [[ ! -f /usr/local/bin/ddns ]] || [[ "$(cat /usr/local/bin/ddns 2>/dev/null || true)" != "$ddns_cmd_content" ]]; then
    echo "$ddns_cmd_content" > /usr/local/bin/ddns
    chmod +x /usr/local/bin/ddns
    log_ok "快捷命令已安装: 输入 'ddns' 打开控制台"
  fi

  rm -f /usr/local/bin/tw 2>/dev/null || true
}
...
run_setup() {
  ...
  echo
  printf "%s" "$C_MAGENTA"; print_line
  printf "%s%s  Aether IPV6 DDNS 配置向导%s\n" "$C_BOLD" "$C_CYAN" "$C_RESET"
  printf "%s" "$C_MAGENTA"; print_line
  printf "%s" "$C_RESET"
  ...
  while true; do
    local cur_token=""
    _is_placeholder "$CF_API_TOKEN" || cur_token="$CF_API_TOKEN"
  ...
  while true; do
    local cur_zone=""
    _is_placeholder "$CF_ZONE_ID" || cur_zone="$CF_ZONE_ID"
  ...
  while true; do
    local cur_record=""
    _is_placeholder "$CF_RECORD_NAME" || cur_record="$CF_RECORD_NAME"
    CF_RECORD_NAME="$(_prompt_input "* DDNS 域名 (如 ddns.example.com)" "$cur_record")"
  ...
  printf "%s%s[ 实例选择 ]%s\n" "$C_BOLD" "$C_YELLOW" "$C_RESET"
  echo
  printf "  请选择你的实例[默认为Hong-Kong]\n"
  printf "  1. Hong-Kong\n"
  printf "  2. Dallas\n"
  printf "  3. Manassas\n"
  printf "  4. Los-Angeles\n"
  printf "  5. New-Jersey\n"
  printf "  6. Los-Angeles(CO)\n"
  echo
...
6) INSTANCE_PROFILE="Los-Angeles(CO)" ;;
    "Hong-Kong") INSTANCE_PROFILE="Hong-Kong" ;;
    "Dallas") INSTANCE_PROFILE="Dallas" ;;
    "Manassas") INSTANCE_PROFILE="Manassas" ;;
    "Los-Angeles") INSTANCE_PROFILE="Los-Angeles" ;;
    "New-Jersey") INSTANCE_PROFILE="New-Jersey" ;;
    "Los-Angeles(CO)") INSTANCE_PROFILE="Los-Angeles(CO)" ;;
  ...
  local instance_show target_wg_interface
  instance_show="$(instance_label)"
  target_wg_interface="$(get_target_wg_interface)"
  ...
  print_kv_plain "实例名称" "$instance_show"
  print_kv_plain "目标接口" "$target_wg_interface"
...
maybe_prompt_setup() {
  if _is_placeholder "$CF_API_TOKEN" || _is_placeholder "$CF_ZONE_ID" || _is_placeholder "$CF_RECORD_NAME"; then
...
run_uninstall() {
  ...
  echo
  printf "%s" "$C_MAGENTA"; print_line
  printf "%s%s  Aether IPV6 DDNS 完整卸载%s\n" "$C_BOLD" "$C_RED" "$C_RESET"
  printf "%s" "$C_MAGENTA"; print_line
  printf "%s" "$C_RESET"
  echo
  printf "  将要删除以下内容：\n"
  print_kv "systemd timer" "${C_YELLOW}${SERVICE_NAME}.timer${C_RESET}"
  print_kv "systemd service" "${C_YELLOW}${SERVICE_NAME}.service${C_RESET}"
  print_kv "脚本文件" "${C_YELLOW}${SCRIPT_INSTALL_PATH}${C_RESET}"
  print_kv "快捷命令" "${C_YELLOW}/usr/local/bin/ddns${C_RESET}"
  ...
  if command -v systemctl >/dev/null 2>&1; then
    systemctl disable --now "${SERVICE_NAME}.timer" 2>/dev/null && log_ok "timer 已停止" || warn "timer 停止失败（可能未安装）"
  ...
  rm -f /usr/local/bin/ddns && log_ok "已删除: /usr/local/bin/ddns" || true
  rm -f /usr/local/bin/tw 2>/dev/null || true
...
console_show_status() {
  local current_ipv6="" last_ip="" alert_state="" timer_status="" last_run=""
  local instance_show="" target_wg_interface=""

  [[ -f "$LAST_IP_FILE" ]] && last_ip="$(cat "$LAST_IP_FILE" 2>/dev/null || true)"
  [[ -z "$last_ip" ]] && last_ip="无记录"

  current_ipv6="$(ip -6 addr show scope global 2>/dev/null | grep -oP '(?<=inet6 )[0-9a-f:]+' | grep -v '^fe80' | head -1 || true)"
  [[ -z "$current_ipv6" ]] && current_ipv6="未检测到"
  ...
  echo
  printf "%s" "$C_MAGENTA"; print_line
  printf "%s%s  Aether IPV6 DDNS 状态总览%s\n" "$C_BOLD" "$C_CYAN" "$C_RESET"
  printf "%s" "$C_MAGENTA"; print_line
  printf "%s" "$C_RESET"
  ...
  print_kv "本机全局 IPv6" "${C_CYAN}${current_ipv6}${C_RESET}"
...
run_once() {
  ...
  log_banner "${ICON_RUN} 开始执行 Aether IPV6 DDNS 同步任务"
  ...
  local target_wg_interface instance_show
  target_wg_interface="$(get_target_wg_interface)"
  instance_show="$(instance_label)"
  log_info "当前实例: ${instance_show}"
  log_info "目标 wg_interface: ${target_wg_interface}"
  ...
  target_ipv6="$(echo "$api_json" | jq -r --arg wg "$target_wg_interface" '
    (
      [
        (.lease // empty),
        ((.leases // [])[]?)
  ...
  [[ -n "$target_ipv6" && "$target_ipv6" != "null" ]] || {
    err "API 中未找到实例 ${INSTANCE_PROFILE} 对应 wg_interface=${target_wg_interface} 的 IPv6"
    _log_write "$api_json"
    return 1
  }

  validate_ipv6 "$target_ipv6" || {
    err "获取到的 IPv6 格式无效: $target_ipv6"
    return 1
  }

  log_ok "获取到目标 IPv6: $(short_ipv6 "$target_ipv6")"
  test_target_ipv6_connectivity "$target_ipv6" || return 1
  ...
  if [[ "$record_count" -eq 0 ]]; then
    ...
    [[ "$create_success" == "true" ]] || {
      ...
      _log_write "$create_json"
      return 1
    }

    echo "$target_ipv6" > "$LAST_IP_FILE"
    chmod 600 "$LAST_IP_FILE" >/dev/null 2>&1 || true
    log_ok "已创建 AAAA: $CF_RECORD_NAME -> $(short_ipv6 "$target_ipv6")"
    send_tg_success "${last_local_ip:-不存在}" "$target_ipv6"
    [[ "$previous_alert_state" == "failure" ]] && send_tg_recovery "$target_ipv6"
    mark_success_state
  ...
  if [[ "$cf_current_ip" == "$target_ipv6" ]]; then
    echo "$target_ipv6" > "$LAST_IP_FILE"
    chmod 600 "$LAST_IP_FILE" >/dev/null 2>&1 || true
  ...
  log_info "新值: $(short_ipv6 "$target_ipv6")"
  ...
  [[ "$update_success" == "true" ]] || {
    ...
    _log_write "$update_json"
    return 1
  }

  echo "$target_ipv6" > "$LAST_IP_FILE"
  chmod 600 "$LAST_IP_FILE" >/dev/null 2>&1 || true
  log_ok "已更新 AAAA: $CF_RECORD_NAME -> $(short_ipv6 "$target_ipv6")"
  send_tg_success "$cf_current_ip" "$target_ipv6"
  [[ "$previous_alert_state" == "failure" ]] && send_tg_recovery "$target_ipv6"
  mark_success_state
