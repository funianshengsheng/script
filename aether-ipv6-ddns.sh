...
install_systemd_timer() {
  ...
  if [[ "$EUID" -ne 0 ]]; then
    warn "安装 systemd timer 需要 root，已跳过"; return 0
  fi
...
WantedBy=timers.target"
  ...
  systemctl enable --now "${SERVICE_NAME}.timer" >/dev/null 2>&1 || {
    warn "启用 ${SERVICE_NAME}.timer 失败"; return 0
  }
...
exec ${SCRIPT_INSTALL_PATH} console"
  ...
  if [[ ! -f /usr/local/bin/ddns ]] || \
     [[ "$(cat /usr/local/bin/ddns 2>/dev/null)" != "$ddns_cmd_content" ]]; then
...
run_uninstall() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    systemctl disable --now "${SERVICE_NAME}.timer"   2>/dev/null \
...
console_show_status() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    if systemctl is-active --quiet "${SERVICE_NAME}.timer" 2>/dev/null; then
      ...
      last_run="$(systemctl show "${SERVICE_NAME}.service" \
...
run_once() {
  ...
  validate_ipv6 "$target_ipv6" || {
    err "获取到的 IPv6 格式无效: $target_ipv6"; return 1
  }
