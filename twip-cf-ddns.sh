...
_prompt_input() {
  ...
  if [[ "$secret" == "1" ]]; then
    ...
    [[ -n "$current" ]] && printf " %s[当前: %s]%s" "$C_DIM" "$(mask_text "$current")" "$C_RESET"
    printf ": "
    read -rs input; echo
  else
    printf "  %s%s%s" "$C_BOLD" "$prompt" "$C_RESET"
    [[ -n "$current" ]] && printf " %s[当前: %s]%s" "$C_DIM" "$current" "$C_RESET"
    printf ": "
    read -r input
  fi
  if [[ -z "$input" && -n "$current" ]]; then echo "$current"
  else echo "$input"; fi
}
...
run_setup() {
  ...
  printf "  %-24s %s\n" "CF API Token:"  "$(mask_text "$CF_API_TOKEN")"
  printf "  %-24s %s\n" "CF Zone ID:"    "$(mask_text "$CF_ZONE_ID")"
  printf "  %-24s %s\n" "DDNS 域名:"     "$CF_RECORD_NAME"
  printf "  %-24s %s\n" "TTL:"           "$CF_TTL"
  printf "  %-24s %s\n" "CF 代理:"       "$CF_PROXIED"
  printf "  %-24s %s\n" "TG Bot Token:"  "$( ! _is_placeholder "$TG_BOT_TOKEN" && mask_text "$TG_BOT_TOKEN" || echo "未配置" )"
  printf "  %-24s %s\n" "TG Chat ID:"    "$( ! _is_placeholder "$TG_CHAT_ID"   && echo "$TG_CHAT_ID"        || echo "未配置" )"
  printf "  %-24s %s\n" "时区:"          "$DISPLAY_TZ"
  printf "  %-24s %s\n" "定时间隔:"      "${TIMER_INTERVAL_MINUTES} 分钟"
  printf "  %-24s %s\n" "VM UUID:"       "$( [[ -n "$VM_UUID" ]] && echo "$VM_UUID" || echo "自动检测" )"
...
run_uninstall() {
  ...
  printf "%s" "$C_MAGENTA"; print_line; printf "%s" "$C_RESET"; echo
  printf "  将要删除以下内容：\n"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "systemd timer:"  "twip-cf-ddns.timer"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "systemd service:" "twip-cf-ddns.service"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "脚本文件:"        "$SCRIPT_INSTALL_PATH"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "快捷命令:"        "/usr/local/bin/tw"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "配置文件:"        "$CONFIG_FILE"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "状态目录:"        "$STATE_DIR"
  printf "  ${C_YELLOW}%-36s${C_RESET}%s\n" "日志目录:"        "$LOG_DIR（二次确认）"
  echo
  printf "%s⚠ 此操作不可逆，确认完整卸载？[y/N]: %s" "$C_RED$C_BOLD" "$C_RESET"
  read -r confirm; confirm="${confirm:-N}"
  ...
  read -r del_log; del_log="${del_log:-N}"
  if [[ "${del_log^^}" == "Y" ]]; then
...
console_show_status() {
  ...
  printf "%s" "$C_MAGENTA"; print_line; printf "%s" "$C_RESET"
  printf "  %-20s %b\n" "域名:"          "${C_BOLD}${CF_RECORD_NAME}${C_RESET}"
  printf "  %-20s %b\n" "定时任务:"      "$timer_status"
  printf "  %-20s %s\n" "上次执行:"      "$last_run"
  printf "  %-20s %b\n" "告警状态:"      "$alert_state"
  printf "  %-20s %b\n" "本机全局 IPv6:" "${C_CYAN}${tw_ipv6}${C_RESET}"
  printf "  %-20s %b\n" "上次同步 IP:"   "${C_CYAN}${last_ip}${C_RESET}"
  printf "  %-20s %s\n" "配置文件:"      "$CONFIG_FILE"
  printf "  %-20s %s\n" "日志文件:"      "$LOG_FILE"
  printf "  %-20s %s\n" "时区:"          "$DISPLAY_TZ"
  printf "%s" "$C_MAGENTA"; print_line; printf "%s" "$C_RESET"
...
console_main() {
  ...
  while true; do
    ...
    case "$choice" in
      1) console_view_log;    printf "\n%s按 Enter 继续...%s" "$C_DIM" "$C_RESET"; read -r _ ;;
